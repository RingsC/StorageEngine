 ...
 */
static THREAD_LOCAL bool atexit_callback_setup = false;
...
static THREAD_LOCAL int on_proc_exit_index,
			on_shmem_exit_index;
...
proc_exit(int code)
{
    //extern void	UnregisteThreadInfo();
...
#ifdef PROFILE_PID_DIR
	{
		...
		if (IsAutoVacuumWorkerProcess())
			...
			snprintf(gprofDirName, 32, "gprof/%d", (int) pthread_self());
