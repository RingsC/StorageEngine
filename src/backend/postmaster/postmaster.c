...
THREAD_LOCAL char	   *UnixSocketDir;
THREAD_LOCAL char	   *ListenAddresses;
...
*/
THREAD_LOCAL int			ReservedBackends;
...
THREAD_LOCAL bool		EnableSSL = false;
THREAD_LOCAL bool		SilentMode = false; /* silent_mode */

THREAD_LOCAL int			PreAuthDelay = 0;
THREAD_LOCAL int			AuthenticationTimeout = 60;

THREAD_LOCAL bool		log_hostname;		/* for ps display and logging */
THREAD_LOCAL bool		Log_connections = false;
THREAD_LOCAL bool		Db_user_namespace = false;

THREAD_LOCAL bool		enable_bonjour = false;
THREAD_LOCAL char	   *bonjour_name;
THREAD_LOCAL bool		restart_after_crash = true;
...
THREAD_LOCAL bool ErrorCauseExit = false;
#endif //FOUNDER_XDB_SE

static bool FatalError = false; /* T if recovering from backend crash */
static bool RecoveryError = false;		/* T if WAL recovery failed */

extern THREAD_LOCAL bool		am_walsender ;
/*
...
THREAD_LOCAL bool		ClientAuthInProgress = false;		/* T during new-client
                                                            * authentication */
...
*/
static THREAD_LOCAL unsigned int random_seed = 0;
...
THREAD_LOCAL HANDLE		PostmasterHandle;
#endif
...
/* ugly kluge */
//typedef struct LWLockPadded LWLockPadded;	/* ugly kluge */
/*
...
PostmasterMain(int argc, char *argv[])
{
    ...
    */
    ...
    pg_signal(SIGHUP, SIGHUP_handler);	/* reread config file and have
                                        * children do same */
    pg_signal(SIGINT, pmdie);	/* send SIGTERM and shut down */
    pg_signal(SIGQUIT, pmdie);	/* send SIGQUIT and die */
    pg_signal(SIGTERM, pmdie);	/* wait for children and shut down */
    pg_signal(SIGALRM, SIG_IGN); /* ignored */
    pg_signal(SIGPIPE, SIG_IGN); /* ignored */
    pg_signal(SIGUSR1, sigusr1_handler); /* message from child process */
    pg_signal(SIGUSR2, dummy_handler);	/* unused, reserve for children */
    pg_signal(SIGCHLD, reaper);	/* handle child termination */
    pg_signal(SIGTTIN, SIG_IGN); /* ignored */
    pg_signal(SIGTTOU, SIG_IGN); /* ignored */
    /* ignore SIGXFSZ, so that ulimit violations work like disk full */
#ifdef SIGXFSZ
    pg_signal(SIGXFSZ, SIG_IGN); /* ignored */
#endif
...
pmdaemonize(void)
{
    ...
    */
    ...
    MyProcPid = PostmasterPid = pthread_self();		/* reset PID vars to child */
...
ServerLoop(void)
{
    ...
    for (;;)
    {
        ...
        */
        ...
        {
            pg_sleep(100000L); /* 100 msec seems reasonable */
            selres = 0;
        }
        else
        {
    ...
    if (PG_PROTOCOL_MAJOR(proto) >= 3)
    {
        ...
        */
        ...
        while (offset < len)
        {
            ...
            valoffset = offset + (uint32)strlen(nameptr) + 1;
            if (valoffset >= len)
            ...
            offset = valoffset + (uint32)strlen(valptr) + 1;
        }
    ...
    if (Db_user_namespace)
    {
        ...
        */
        ...
        else
        {
            ...
            db_user = (char *)palloc(strlen(port->user_name) +
    ...
    */
    ...
    case CAC_STARTUP:
        ereport(FATAL,
            ...
            errmsg("the storage engine is starting up")));
        break;
    case CAC_SHUTDOWN:
        ereport(FATAL,
            (errcode(ERRCODE_CANNOT_CONNECT_NOW),
            errmsg("the storage engine is shutting down")));
        break;
    case CAC_RECOVERY:
        ereport(FATAL,
            (errcode(ERRCODE_CANNOT_CONNECT_NOW),
            errmsg("the storage engine is in recovery mode")));
        break;
    case CAC_TOOMANY:
        ereport(FATAL,
...
ConnCreate(int serverFd)
{
    ...
    if (NULL == (port = (Port *) calloc(1, sizeof(Port))))
    {
        ereport(ERROR,
...
PostmasterStateMachine(void)
{
    ...
    */
    if (Shutdown > NoShutdown && pmState == PM_NO_CHILDREN)
    {
        if (FatalError)
        {
            ereport(LOG, (errmsg("abnormal storage engine shutdown")));
            ExitPostmaster(1);
        }
        else
        {
...
BackendInitialize(Port *port)
{
    ...
    */
    if (PreAuthDelay > 0)
        pg_sleep(PreAuthDelay * 1000000L);
    ...
    */
    pg_signal(SIGTERM, startup_die);
    pg_signal(SIGQUIT, startup_die);
    pg_signal(SIGALRM, startup_die);
...
#else
    param = (BackendParameters *)MapViewOfFile(paramHandle, FILE_MAP_WRITE, 0, 0, sizeof(BackendParameters));
#endif //FOUNDER_XDB_SE
...
#else
    childinfo = (win32_deadchild_waitinfo *)malloc(sizeof(win32_deadchild_waitinfo));
#endif //FOUNDER_XDB_SE
    if (!childinfo)
        ereport(FATAL,
        (errcode(ERRCODE_OUT_OF_MEMORY),
...
#ifdef FOUNDER_XDB_SE
int
fxdb_thread_create(fxdb_thread_t *th,
                   void *(*start)(void*),
                   void *arg);
pid_t
fxdb_backend_forkexec();

pid_t
fxdb_ThreadCreate(void *(*start)(void*), void *args);

static bool
fxdb_save_backend_variables(BackendParameters *param);
static pid_t
fxdb_MainStarter(void *params);
#ifndef FOUNDER_XDB_SE
int
fxdb_SubPostmasterMain(int argc, char *argv[]);
#endif
void*
...
#ifndef FOUNDER_XDB_SE
static void
fxdb_read_backend_variables(char *id);
static void
fxdb_restore_backend_variables(BackendParameters * param);
void
fxdb_save_thread_variables(BackendParameters *param,BackendId id,ThreadType tt);
void
fxdb_read_thread_variables(BackendParameters *param);
#endif //FOUNDER_XDB_SE
                   ...
                   void *arg)
{
...
#ifdef WIN32
    {
        ...
        th->os_handle = (HANDLE)_beginthreadex(NULL,	/* no security info */
            size,				/* statck size */
            (unsigned(__stdcall*)(void*)) start,/* start routine */
            arg,			/* arg list */
            0,				/* run */
            &th->thid);		/* thread id */
...
#else
    {
#define thread_stack_size 1024L
        pthread_attr_t attr;
        size_t	size;

        pthread_attr_init(&attr);
        size = thread_stack_size * 1024L;

        pthread_attr_setstacksize(&attr, size);

        rc = 0 == pthread_create(&th->os_handle,
            &attr,
            start,
            arg);
        th->thid = th->os_handle;
...
fxdb_thread_t *
fxdb_Thread_Create(void *(*start)(void*), void *args)
{   
    int			free_index = -1;
    fxdb_thread_t * thread=(fxdb_thread_t *)palloc0(sizeof(fxdb_thread_t));
    ...
    {
	    /* create the child */
	    if (fxdb_thread_create(thread, start, args))
	    {
	        return thread;
	    }
...
void 
fxdb_close_handle(fxdb_thread_t *t)
{
	Assert(t != NULL);
...
void set_bgwriter_thid(pid_t thid)
{
    BgWriterPID = thid;
}

pid_t get_bgwriter_thid()
{
    return BgWriterPID;
}
...
fxdb_SubPostmasterMain(int argc, char *argv[])
{
    ...
    /* MyProcPid has been initialized in ThreadInit*/
    /* MyProcPid = ThreadGetId(); */		/* reset MyProcPid */

#ifndef WIN32
    /* 
    * K_MT: In Linux, created thread does not inherit current dir, so we do it 
    * ourselves explicitely here. 
    */
    ChangeToDataDir();
#endif
    ...
    fxdb_read_backend_variables(argv[2]);

    /* 
    * This is a hack. You'd better move this to even earlier stage in case
    * we failed before pg_init(). Different from processed backend, we have
    * to close connections ourselves to prevent endless wait in client side.
    */
    MyProcPort = &port;

    /* Check we got appropriate args */
    if (argc < 3)
        elog(FATAL, "invalid  invocation");
...
fxdb_SubPostmaster_Main(void *params)
{
	...
	extern void PostPortAssignAppname(Port *port);
    ...
    /* Setup essential subsystems (to ensure elog() behaves sanely) */
    MemoryContextInit(true);
	if (((BackendParameters*)params)->MyThreadType == replha ||
		((BackendParameters*)params)->MyThreadType == walreceiver ||
		((BackendParameters*)params)->MyThreadType == walsender ||
		((BackendParameters*)params)->MyThreadType == vacuum_worker ||
		((BackendParameters*)params)->MyThreadType == xlogstarter)
		on_proc_exit(fxdb_free_topmemory, 0);
    ...
    /* Run backend or appropriate child */
    if (((BackendParameters*)params)->MyThreadType == backend)
    {
		pg_signal(SIGALRM, handle_sig_alarm);	/* timeout conditions */
        /* Restore basic shared memory pointers */
        InitShmemAccess(UsedShmemSegAddr);
        /* Need a PGPROC to run CreateSharedMemoryAndSemaphores */
        InitProcess();
    ...
    if(((BackendParameters*)params)->MyThreadType == bgwriter
		||((BackendParameters*)params)->MyThreadType == walreceiver
		||((BackendParameters*)params)->MyThreadType == xlogstarter
		|| ((BackendParameters*)params)->MyThreadType == walwriter)
    {
        Mode = BootstrapProcessing;
        InitShmemAccess(UsedShmemSegAddr);
        InitAuxiliaryProcess();
        CreateSharedMemoryAndSemaphores(false, 54322);		
		}

    if(((BackendParameters*)params)->MyThreadType == walsender)
    {
        /* Restore basic shared memory pointers */
        InitShmemAccess(UsedShmemSegAddr);
        /* Need a PGPROC to run CreateSharedMemoryAndSemaphores */
        ...
        InitProcessPhase2();
        InitBufferPoolBackend();
        MyProcPort = ((BackendParameters*)params)->port;
		am_walsender = true;
		ProcessStartupPacket(MyProcPort, false);
		PostPortAssignAppname(MyProcPort);
		ClientAuthentication(MyProcPort);
    }
		if(((BackendParameters*)params)->MyThreadType == vacuum_launcher)
		{
		...
		if(((BackendParameters*)params)->MyThreadType == vacuum_worker)
		{
			Mode = NormalProcessing;
			AutovacuumWorkerIAm();
			/* Restore basic shared memory pointers */
		...
		if(((BackendParameters*)params)->MyThreadType == pgstat)
		{
			extern char *pgstat_stat_tmpname;
			extern char *pgstat_stat_filename;

			pgstat_stat_tmpname = (char*)"fxdb_stat_tmp/pgstat.tmp";
			pgstat_stat_filename = (char*)"fxdb_stat_tmp/pgstat.stat";
	...
	on_proc_exit(fxdb_close_xlogfile, 0);
    Mode = NormalProcessing;
...
{
    extern void AbortOutOfAnyTransaction();
...
fxdb_save_backend_variables(BackendParameters *param)
{
    ...
    param->LWLockArray = LWLockArray;
    param->ProcStructLock = ProcStructLock;
    // param->AppReservedConnLock = AppReservedConnLock;
    param->ProcGlobal = ProcGlobal;
    param->AuxiliaryProcs = AuxiliaryProcs;
    param->PgStartTime = PgStartTime;
    //  param->LicenseMaxConnections = LicenseMaxConnections;
...
fxdb_read_backend_variables(char *id)
{
    BackendParameters *param = (BackendParameters *) id;

    fxdb_restore_backend_variables(param);
}
#endif //FOUNDER_XDB_SE
...
{
    MyCancelKey = param->MyCancelKey;
	UsedShmemSegID = (unsigned long)param->UsedShmemSegID;
    UsedShmemSegAddr = param->UsedShmemSegAddr;
    SetDataDir(param->DataDir);
    ShmemLock = param->ShmemLock;
    ShmemVariableCache = param->ShmemVariableCache;
    LWLockArray = param->LWLockArray;
    ProcStructLock = param->ProcStructLock;
    //   AppReservedConnLock = param->AppReservedConnLock;
    ProcGlobal = param->ProcGlobal;
    AuxiliaryProcs = param->AuxiliaryProcs;
    PgStartTime = param->PgStartTime;
...
fxdb_save_thread_variables(BackendParameters *param,BackendId id,ThreadType tt)
{
	param->MyThreadType = tt;
	//param->MyBackendId = id;
	//param->MyCancelKey = MyCancelKey;
	//param->UsedShmemSegID = UsedShmemSegID;
	//param->UsedShmemSegAddr = UsedShmemSegAddr;
	//strlcpy(param->DataDir, DataDir, MAXPGPATH);
...
static
void ReceiverLsnrShutdownHandler(SIGNAL_ARGS)
{
	Shutdown = FastShutdown;
}

static
void ProcessLsnrSignals()
{
	pg_process_signals();
}

extern void fxdb_walsender_forkexec(Port * port);

void * waitfor_walreceiver(void * param)
{
	IsPostmasterEnvironment = true;

	MemoryContextInit(true);

	pg_signal(SIGTERM, ReceiverLsnrShutdownHandler);

	ereport(LOG,
		(errmsg("walreceiverLsnr_handle started : %d\n", (int)pthread_self())));
    ...
    if (status != STATUS_OK)
        ereport(ERROR,
                ...
                errmsg("could not create listen socket")));
	        	...
	        	if (FD_ISSET(ListenSocket[i], &rmask))
	        	 {
			           ...
			           port = ConnCreate((int)ListenSocket[i]);
			         if (port)
			         {
				         fxdb_walsender_forkexec(port);
			         }
	        	 }
		   }
...
void KillStorageEngine(int errCode)
{
	interruptCode = errCode;
	bInterrupt = true;
}
#endif //FOUNDER_XDB_SE
