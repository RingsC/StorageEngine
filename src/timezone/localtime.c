...
static THREAD_LOCAL char wildabbr[] = WILDABBR;

static THREAD_LOCAL const char gmt[] = "GMT";
...
/* GMT timezone */
static THREAD_LOCAL struct state gmtmem;

#define gmtptr		(&gmtmem)


static THREAD_LOCAL int	gmt_is_set = 0;
...
#else
	return (struct pg_tm*)localtime((const time_t*)timep);
#endif
}
		...
		const struct state * sp, struct pg_tm * tmp)
{
	...
	}
	{
		...
		seconds = (long)(tdays * SECSPERDAY + 0.5);
