...
typedef struct BTScanPosData
{
	...
	/* 
	 * effective when the nbtree is used as a cluster index, otherwise NULL. 
	 * stores a leaf page's all datums whose key matches the scankeys, it must always 
	 * be assigned to a memory chunk of BLCKSZ bytes so that it's always big enough.
	 *
	 * offsets[i] is the total NO. of bytes of dataitem-0, dataitem-1,..., to dataitem-i, in datumBlock memory. 
	 * offsets[MaxIndexTuplesPerPage] is the NO. of data items in datumBlock and also the NO. of filled slots in offset array.
	 *
	 * We have to support mark/restore, thus have to put the two fields in this structure rather than the general IndexScanDescData.
	 */
	char        datumBlock[BLCKSZ];
    uint16         offsets[MaxIndexTuplesPerPage + 1];
	 ...
	 * 
	 * These fileds are also effective when this is a cluster index.
	 */
...
{
	bool isInUse;// whether this struct is used, for allocation in current txn's BTScanOpaqueData array.
	...
	int		    killedItems[MaxIndexTuplesPerPage];	/* currPos.items indexes of killed items */
	...
	struct BTScanOpaqueData *next;// if preallocated objs in txn-state-data isn't enough, more will be allocated as needed and linked and appended to the last preallocated one.
...
extern Datum btbuildcluster(PG_FUNCTION_ARGS);
 ...
 */
extern ScanKey _bt_mkscankey(Relation rel, IndexTuple itup, int* keynum);
extern ScanKey _bt_mkscankey_nodata(Relation rel);
extern void _bt_freeskey(ScanKey skey);
extern void _bt_freeskeys(ScanKey skey, int num);
...
extern BTSpool *_bt_spoolinit_kb(Relation index, bool isunique, int btKbyte);
...
extern void btbuildCallback(Relation index,
							HeapTuple htup,
							Datum *values,
							bool *isnull,
							bool tupleIsAlive,
