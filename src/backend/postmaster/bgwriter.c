...
typedef struct
{
	pthread_t	bgwriter_pid;	/* PID of bgwriter (0 if not started) */
...
static THREAD_LOCAL BgWriterShmemStruct *BgWriterShmem;
...
static THREAD_LOCAL volatile sig_atomic_t checkpoint_requested = false;
static THREAD_LOCAL volatile sig_atomic_t shutdown_requested = false;

/*
 * Private state
 */
static THREAD_LOCAL bool am_bg_writer = false;

static THREAD_LOCAL bool ckpt_active = false;

/* these values are valid when ckpt_active is true: */
static THREAD_LOCAL pg_time_t ckpt_start_time;
static THREAD_LOCAL XLogRecPtr ckpt_start_recptr;
static THREAD_LOCAL double ckpt_cached_elapsed;

static THREAD_LOCAL pg_time_t last_checkpoint_time;
static THREAD_LOCAL pg_time_t last_xlog_switch_time;
static THREAD_LOCAL bool firstSnapShot = false;
...
BackgroundWriterMain(void)
{
...
#else
	ereport(LOG,
		(errmsg("bgwriter_handle started:%d\n", (int)MyProcPid)));
	 ...
	 */
	pg_signal(SIGHUP, BgSigHupHandler);	/* set flag to read config file */
	pg_signal(SIGINT, ReqCheckpointHandler);		/* request checkpoint */
	pg_signal(SIGTERM, SIG_IGN); /* ignore SIGTERM */
	pg_signal(SIGQUIT, bg_quickdie);		/* hard crash time */
	pg_signal(SIGALRM, SIG_IGN);
	pg_signal(SIGPIPE, SIG_IGN);
	pg_signal(SIGUSR1, SIG_IGN); /* reserve for ProcSignal */
	pg_signal(SIGUSR2, ReqShutdownHandler);		/* request shutdown */
...
//	PG_SETMASK(&UnBlockSig);
	...
	for (;;)
	{
		...
		elapsed_secs = (int)(now - last_checkpoint_time);
		...
		if (do_checkpoint)
		{
			...
			extern void GetFirstSnapShot(void);;
			if(!RecoveryInProgress())
			{
				if(!firstSnapShot)
				{
...
{
	static THREAD_LOCAL int	absorb_counter = WRITES_PER_ABSORB;
...
#ifdef FOUNDER_XDB_SE
bool IsBgWriterReady()
{
	if(BgWriterShmem && BgWriterShmem->bgwriter_pid != 0)
	{
		return true;
	}
...
RequestCheckpoint(int flags)
{
	...
	for (ntries = 0;; ntries++)
	{
		...
		else if (pgkill(BgWriterShmem->bgwriter_pid, SIGINT) != 0)
		{
			if (ntries >= 20)	/* max wait 2.0 sec */
			{
				elog((flags & CHECKPOINT_WAIT) ? ERROR : LOG,
		...
		pg_sleep(100000L);		/* wait 0.1 sec, then retry */
	}
	...
	if (flags & CHECKPOINT_WAIT)
	{
		...
		if (new_failed != old_failed)
			ereport(ERROR,
                    (errcode(ERRCODE_BGWRITER_CHECK_POINT_REQUEST_FAILURE),
					...
					BlockNumber segno)
{
	...
	if (am_bg_writer)
		ereport(ERROR,
                ...
                errmsg("ForwardFsyncRequest must not be called in bgwriter")));
...
CompactBgwriterRequestQueue()
{
...
#else
    skip_slot = (bool *)palloc0(sizeof(bool) * BgWriterShmem->num_requests);
#endif //FOUNDER_XDB_SE
	...
	for (n = 0; n < BgWriterShmem->num_requests; ++n)
	{
		...
		slotmap = (BgWriterSlotMapping *)hash_search(htab, request, HASH_ENTER, &found);
		if (found)
		{
