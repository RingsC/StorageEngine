...
static THREAD_LOCAL MemoryContext MdCxt;		/* context for all md.c allocations */
...
static THREAD_LOCAL HTAB *pendingOpsTable = NULL;
static THREAD_LOCAL List *pendingUnlinks = NIL;

static THREAD_LOCAL CycleCtr mdsync_cycle_ctr = 0;
static THREAD_LOCAL CycleCtr mdckpt_cycle_ctr = 0;
...
mdsync(void)
{
	static THREAD_LOCAL bool mdsync_in_progress = false;
	...
	if (!pendingOpsTable)
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                errmsg("cannot sync without a pendingOpsTable")));
	...
	while ((entry = (PendingOperationEntry *) hash_seq_search(&hstat)) != NULL)
	{
		...
		if (hash_search(pendingOpsTable, &entry->tag,
			...
			ereport(ERROR,
                    (errcode(ERRCODE_DATA_CORRUPTED),
                    errmsg("pendingOpsTable corrupted")));
...
mdpostckpt(void)
{
	while (pendingUnlinks != NIL)
	{
		...
		extern void TmpOidSetBitMap(Oid, bool);
		extern bool is_toast_id(Oid);

		/* 
		 * Temp toast id not in this range.
		 */
		if(IsTmpOid(entry->rnode.node.relNode) &&
			!is_toast_id(entry->rnode.node.relNode))
		{
			LWLockAcquire(TempOidBitMapLock, LW_EXCLUSIVE);

			TmpOidSetBitMap(entry->rnode.node.relNode, false);
...
register_unlink(RelFileNodeBackend rnode)
{
	...
	else
	{
		...
		while (!ForwardFsyncRequest(rnode, MAIN_FORKNUM,
			...
			pg_sleep(10000L);	/* 10 msec seems a good number */
	}
}
...
BlockNumber getNumberOfPagesForFile(SMgrRelation reln, ForkNumber forknum)
{
