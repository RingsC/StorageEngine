...
static THREAD_LOCAL int	standbyWait_us = STANDBY_INITIAL_WAIT_US;
...
WaitExceedsMaxStandbyDelay(void)
{
	 ...
	 */
	pg_sleep(standbyWait_us);

	/*
	 * Progressively increase the sleep times, but not to more than 1s, since
	 * pg_sleep isn't interruptable on some platforms.
	 */
	standbyWait_us *= 2;
...
standby_redo(XLogRecPtr lsn, XLogRecord *record)
{
	...
	else if (info == XLOG_RUNNING_XACTS)
	{
		...
		ereport(PANIC,
                (errcode(ERRCODE_INTERNAL_ERROR),
                errmsg("relation_redo: unknown op code %u", info)));
}
...
#ifdef FOUNDER_XDB_SE
	if (NULL != CurrRunningXacts->xids)
	{
		pfree(CurrRunningXacts->xids);
		CurrRunningXacts->xids = NULL;
	}
#endif
