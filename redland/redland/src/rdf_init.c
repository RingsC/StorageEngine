...
#endif
#ifdef FOUNDER_XDB_SE
const static unsigned int BLANK_BASE_SEQUENCE_ID = 16;
const static unsigned int BLANK_COUNT_SEQUENCE_ID = 17;
                         ...
                         librdf_node* value) 
{
  ...
  if(librdf_uri_equals(feature, genid_base)) {
    if(!librdf_node_is_resource(value))
	  ...
	  #else
	  SequenceFactory* seqFacotry = SequenceFactory::getSequenceFactory();
  	  XdbSequence* base_sequence = seqFacotry->openSequence(BLANK_BASE_SEQUENCE_ID,-1);
	  base_sequence->reset(lid);
	  #endif
#ifdef WITH_THREADS
	...
	#else
	  SequenceFactory* seqFacotry = SequenceFactory::getSequenceFactory();
      XdbSequence* counter_sequence = seqFacotry->openSequence(BLANK_COUNT_SEQUENCE_ID,-1);
	  counter_sequence->reset(lid);
	#endif
#ifdef WITH_THREADS
      pthread_mutex_unlock(world->mutex);
#endif
...
librdf_world_get_genid(librdf_world* world)
{
  ...
  sprintf((char*)buffer, "r%lur%lur%lu", id, pid, counter);
#else
	unsigned long id, tmpid, counter, tmpcounter;
	size_t length;
	unsigned char *buffer;
	SequenceFactory* seqFacotry = SequenceFactory::getSequenceFactory();
	XdbSequence* pid_sequence = seqFacotry->openSequence(BLANK_BASE_SEQUENCE_ID,-1);
	XdbSequence* counter_sequence = seqFacotry->openSequence(BLANK_COUNT_SEQUENCE_ID,-1);
	tmpid = (id = (unsigned long)pid_sequence->getValue(0));
	tmpcounter = (counter = (unsigned long)counter_sequence->getValue(1));
	/* min length 1 + "r" + min length 1 + "r" \0 */
	length = 5;
	...
	sprintf((char*)buffer, "r%lur%lu", id, counter);
#endif
  return buffer;
}
