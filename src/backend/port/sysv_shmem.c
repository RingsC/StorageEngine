...
#define shmget(key, size, flag)		fxdb_shmget(key, size, flag)
#define shmdt(shmaddr)       		fxdb_shmdt(shmaddr)
#define shmat(memId, shmaddr, flag) fxdb_shmat(memId, shmaddr, flag)
#define shmctl(memId, cmd, buf)     fxdb_shmctl(memId, cmd, buf)
...
InternalIpcMemoryCreate(IpcMemoryKey memKey, Size size)
{
	...
	if (memAddress == (void *) -1)
		ereport(FATAL,
                (errcode(ERRCODE_INTERNAL_ERROR),
                errmsg("shmat(id=%d) failed: %m", shmid)));
...
PGSharedMemoryCreate(Size size, bool makePrivate, int port)
{
	...
	for (NextShmemSegID++;; NextShmemSegID++)
	{
		...
		if (hdr->creatorPID != pthread_self())
		{
			if (pgkill(hdr->creatorPID, 0) == 0 || errno != ESRCH)
			{
				shmdt(memAddress);
