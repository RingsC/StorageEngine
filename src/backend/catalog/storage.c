...
static THREAD_LOCAL PendingRelDelete *pendingDeletes = NULL; /* head of linked list */
#else
#define pendingDeletes (CurrentTransactionState->mTopLevelState->mPendingDeletes)
#endif
		...
		if (RelFileNodeEquals(rnode, pending->relnode))
		{
			...
			if (pending->atCommit)
				ereport(ERROR,
                        ...
                        errmsg("cannot preserve a delete-on-commit relation")));
...
smgr_redo(XLogRecPtr lsn, XLogRecord *record)
{
	...
	else if (info == XLOG_SMGR_TRUNCATE)
	{
		...
		ereport(PANIC,
                ...
                errmsg("smgr_redo: unknown op code %u", info)));
}
