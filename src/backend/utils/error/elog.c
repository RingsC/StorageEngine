...
/* GUC parameters */
THREAD_LOCAL int			Log_error_verbosity = PGERROR_VERBOSE;
THREAD_LOCAL char	   *Log_line_prefix = NULL;		/* format for extra log line info */
THREAD_LOCAL int			Log_destination = LOG_DESTINATION_STDERR;
...
static THREAD_LOCAL bool openlog_done = false;
static THREAD_LOCAL char *syslog_ident = NULL;
static THREAD_LOCAL int	syslog_facility = LOG_LOCAL0;
...
static THREAD_LOCAL ErrorData errordata[ERRORDATA_STACK_SIZE];

static THREAD_LOCAL int	errordata_stack_depth = -1; /* index of topmost active frame */

static THREAD_LOCAL int	recursion_depth = 0;	/* to detect actual recursion */
...
static THREAD_LOCAL char formatted_start_time[FORMATTED_TS_LEN];
static THREAD_LOCAL char formatted_log_time[FORMATTED_TS_LEN];
...
extern void send_message_to_log4cxx(const char* msg, int len, int elevel);
		 ...
		 const char *funcname, const char *domain)
{
	...
	if (recursion_depth++ > 0 && elevel >= ERROR)
	{
		 ...
		 */
		MemoryContextReset(ErrorContext, true);
...
#ifdef FOUNDER_XDB_SE
extern char* get_errno_errmsg( int &err_no )
{
    ErrorData  *edata = &errordata[errordata_stack_depth];

    /* we don't bother incrementing recursion_depth */
    CHECK_STACK_DEPTH();
    err_no = edata->sqlerrcode;

    char fmt[] = "err:%s  (File:%s Lineno:%d)";
    size_t len  = strlen(edata->message ) + sizeof(fmt) + 256;
    char *errmsg = (char*)talloc(len);
    sprintf(errmsg,fmt ,edata->message, edata->filename, edata->lineno);
    --errordata_stack_depth;
		FreeErrorData(edata);
    return errmsg;
}
 ...
 */
static THREAD_LOCAL int	save_format_errnumber;
static THREAD_LOCAL const char *save_format_domain;
...
static void
write_console_r2(const char *line, int len)
{
#ifdef WIN32
 ...
 */
static void
send_message_to_server_log_r2(ErrorData *edata)
{
	StringInfoData buf;
    ...
    //no need append \n when we use log4cxx as logging backend.-sunwf
	//appendStringInfoChar(&buf, '\n');
...
#else
            //write_console_r2(buf.data, buf.len);
	send_message_to_log4cxx(buf.data, buf.len, edata->elevel);
#endif
...
EmitErrorReport(void)
{
	...
	if (edata->output_to_server)
		send_message_to_server_log_r2(edata);
...
{
	static THREAD_LOCAL unsigned long seq = 0;
...
write_eventlog(int level, const char *line, int len)
{
	...
	static THREAD_LOCAL HANDLE evtHandle = INVALID_HANDLE_VALUE;
...
log_line_prefix(StringInfo buf, ErrorData *edata)
{
	...
	static THREAD_LOCAL long log_line_number = 0;

	/* has counter been reset in current process? */
	static THREAD_LOCAL int	log_my_pid = 0;
...
useful_strerror(int errnum)
{
	...
	static THREAD_LOCAL char errorstr_buf[48];
	const char *str;
