...
initialize_reloptions(void)
{
	...
	if (relOpts)
		pfree(relOpts);
	relOpts = (relopt_gen **)MemoryContextAlloc(TopMemoryContext,
...
add_reloption(relopt_gen *newoption)
{
	...
	if (num_custom_options >= max_custom_options)
	{
		...
		if (max_custom_options == 0)
		{
			...
			custom_options = (relopt_gen **)palloc(max_custom_options * sizeof(relopt_gen *));
		}
		else
		{
			max_custom_options *= 2;
			custom_options = (relopt_gen **)repalloc(custom_options,
					 ...
					 validate_string_relopt validator)
{
	...
	newoption = (relopt_string *)palloc0(sizeof(relopt_string) + default_len);
