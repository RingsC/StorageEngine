...
{
	printf(_("%s displays control information of a FounderXMLDB database cluster.\n\n"), progname);
	printf
		(
		 _(
		   "Usage:\n"
	...
	printf(_("\nIf no data directory (DATADIR) is specified, "
			 "the environment variable XMLDB_PATH\nis used.\n\n"));
...
main(int argc, char *argv[])
{
	...
	set_pglocale_pgservice(argv[0], PG_TEXTDOMAIN("fxdb_controldata"));
	...
	snprintf(ControlFilePath, MAXPGPATH, "%s/data/global/fxdb_control", DataDir);
	...
	printf(_("fxdb_control version number:            %u\n"),
		   ControlFile.pg_control_version);
	if (ControlFile.pg_control_version % 65536 == 0 && ControlFile.pg_control_version / 65536 != 0)
		printf(_("WARNING: possible byte ordering mismatch\n"
				 "The byte ordering used to store the fxdb_control file might not match the one\n"
	...
	printf(_("fxdb_control last modified:             %s\n"),
		   pgctime_str);
