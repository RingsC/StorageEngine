 ...
 */
static THREAD_LOCAL int	static_client_encoding = PG_SQL_ASCII;
static THREAD_LOCAL bool static_std_strings = false;
...
pqResultAlloc(PGresult *res, size_t nBytes, bool isBinary)
{
	...
	if (nBytes <= (size_t) res->spaceLeft)
	{
		...
		res->curOffset += (int)nBytes;
		res->spaceLeft -= (int)nBytes;
		return space;
	}
	...
	for (i = 0; i < nParams; i++)
	{
		if (paramValues && paramValues[i])
		{
			...
			else
			{
				...
				nbytes = (int)strlen(paramValues[i]);
			}
...
{
	return PQputnbytes(conn, s, (int)strlen(s));
}
...
{
	if (status < 0 || status >= (int)(sizeof pgresStatus / sizeof pgresStatus[0]))
		return libpq_gettext((char*)"invalid ExecStatusType code");
	return (char*)pgresStatus[status];
}
