...
/* GUC variables */
THREAD_LOCAL bool		zero_damaged_pages = false;
THREAD_LOCAL int			bgwriter_lru_maxpages = 100;
THREAD_LOCAL double		bgwriter_lru_multiplier = 2.0;
 ...
 */
THREAD_LOCAL int			target_prefetch_pages = 0;

/* local state for StartBufferIO and related functions */
static THREAD_LOCAL volatile BufferDesc *InProgressBuf = NULL;
static THREAD_LOCAL bool IsForInput;

/* local state for LockBufferForCleanup */
static THREAD_LOCAL volatile BufferDesc *PinCountWaitBuf = NULL;


static Buffer ReadBuffer_common(SMgrRelation reln, char relpersistence,
				  ForkNumber forkNum, BlockNumber blockNum,
				  ReadBufferMode mode, BufferAccessStrategy strategy,
				  bool *hit, BlockNumber threshold);
				   ...
				   ReadBufferMode mode, BufferAccessStrategy strategy)
{
	...
	buf = ReadBuffer_common(reln->rd_smgr, reln->rd_rel->relpersistence,
							...
							BlockNumberThresholdByRel(reln));
	if (hit)
						  ...
						  BufferAccessStrategy strategy)
{
	...
	return ReadBuffer_common(smgr, RELPERSISTENCE_PERMANENT, forkNum, blockNum,
							 mode, strategy, &hit, InvalidBlockNumber);
}
...
ReadBuffer_common(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
				  BlockNumber blockNum, ReadBufferMode mode,
				  BufferAccessStrategy strategy, bool *hit, BlockNumber threshold)
{
	...
	*hit = false;
	...
	if (isExtend)
	{
		...
		smgrextend(smgr, forkNum, blockNum, (char *) bufBlock, false, threshold);
	}
	else
	{
		/*
			...
			bool *foundPtr)
{
	...
	for (;;)
	{
		...
		if (oldFlags & BM_TAG_VALID)
		{
			...
			oldTag = buf->tag; 
#else
			ASSIGN_BUFFERTAG((oldTag),(buf->tag));
#endif // FOUNDER_XDB_SE
	 ...
	 */
			...
			buf->tag = newTag;
#else
			ASSIGN_BUFFERTAG(buf->tag,newTag);
	...
	if (buf->refcount != 0)
	{
		...
		if (PrivateRefCount[buf->buf_id] != 0)
			ereport(ERROR,
                    ...
                    errmsg("buffer is pinned in InvalidateBuffer")));
		WaitIO(buf);
		goto retry;
	}
...
BgBufferSync(void)
{
	...
	reusable_buffers_est = (int)((float) bufs_ahead / smoothed_density);
	...
	if (smoothed_alloc <= (float) recent_alloc)
		smoothed_alloc = (float)(recent_alloc * 1.0);
	else
...
#else
	RelFileNode tmpNode;
	ASSIGN_RNODE(tmpNode,buf->tag.rnode);
	path = relpathbackend(tmpNode, backend, buf->tag.forkNum);
#endif // FOUNDER_XDB_SE
			 ...
			 BlockNumber *blknum)
{
	...
	*rnode = bufHdr->tag.rnode;
	...
	ASSIGN_RNODE((*rnode),bufHdr->tag.rnode);
...
FlushBuffer(volatile BufferDesc *buf, SMgrRelation reln)
{
...

	{
		...
		RelFileNode tmpNode;
		ASSIGN_RNODE(tmpNode,buf->tag.rnode);
		reln = smgropen(tmpNode, InvalidBackendId);
#endif // FOUNDER_XDB_SE
					   ...
					   BlockNumber firstDelBlock)
{
	...
	if (rnode.backend != InvalidBackendId)
	{
		...
		return;
#else
		if ((TransactionId)rnode.backend == CurrentTopLevelState->xid)
			//DropRelFileNodeLocalBuffers(rnode.node, forkNum, firstDelBlock);
			return;
#endif //FOUNDER_XDB_SE
	}
...
UnlockBuffers(void)
{
	...
	if (buf)
	{
		...
		if ((buf->flags & BM_PIN_COUNT_WAITER) != 0 &&
			buf->wait_backend_pid == (int)MyProcPid)
			buf->flags &= ~BM_PIN_COUNT_WAITER;
...
LockBuffer(Buffer buffer, int mode)
{
	...
	if (mode == BUFFER_LOCK_UNLOCK)
		...
		ereport(ERROR,
                ...
                errmsg("unrecognized buffer lock mode: %d", mode)));
}
...
LockBufferForCleanup(Buffer buffer)
{
	...
	if (BufferIsLocal(buffer))
	{
		...
		if (LocalRefCount[-buffer - 1] != 1)
			ereport(ERROR,
                    (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    errmsg( "incorrect local pin count: %d",
				                     LocalRefCount[-buffer - 1])));
	...
	for (;;)
	{
		...
		if (bufHdr->flags & BM_PIN_COUNT_WAITER)
		{
			...
			ereport(ERROR,
                    (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    errmsg("multiple backends attempting to wait for pincount 1")));
		}
	...
	if (buf)
	{
		...
		else
		{
			...
			if (sv_flags & BM_IO_ERROR)
			{
				...
				RelFileNode tmpNode;
				ASSIGN_RNODE(tmpNode,buf->tag.rnode);
				path = relpathperm(tmpNode, buf->tag.forkNum);
#endif // FOUNDER_XDB_SE
...
shared_buffer_write_error_callback(void *arg)
{
	...
	if (bufHdr != NULL)
	{
		...
		BufferTag tmpTag;
		ASSIGN_BUFFERTAG(tmpTag,(bufHdr->tag));
		char *path = relpathperm(tmpTag.rnode, tmpTag.forkNum);
...
local_buffer_write_error_callback(void *arg)
{
	...
	if (bufHdr != NULL)
	{
		...
		BufferTag tmpTag;
		ASSIGN_BUFFERTAG(tmpTag,(bufHdr->tag));
		char	   *path = relpathbackend(tmpTag.rnode, MyBackendId,
										  tmpTag.forkNum);
#endif // FOUNDER_XDB_SE
