...
#ifdef FOUNDER_XDB_SE
static THREAD_LOCAL ResourceReleaseCallbackItem *ResourceRelease_callbacks = NULL;
#endif //FOUNDER_XDB_SE
...
ResourceOwnerForgetBuffer(ResourceOwner owner, Buffer buffer)
{
	if (owner != NULL)
	{
		...
		ereport(ERROR,
                ...
                errmsg( "buffer %d is not owned by resource owner %s",
			                 buffer, owner->name)));
	}
}
...
ResourceOwnerForgetCatCacheRef(ResourceOwner owner, HeapTuple tuple)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
            errmsg( "catcache reference %p is not owned by resource owner %s",
		             tuple, owner->name)));
}
#ifndef FOUNDER_XDB_SE
/*
...
ResourceOwnerForgetRelationRef(ResourceOwner owner, Relation rel)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
            errmsg( "relcache reference %s is not owned by resource owner %s",
...
ResourceOwnerForgetTupleDesc(ResourceOwner owner, TupleDesc tupdesc)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
            errmsg( "tupdesc reference %p is not owned by resource owner %s",
		             tupdesc, owner->name)));
}
...
ResourceOwnerForgetSnapshot(ResourceOwner owner, Snapshot snapshot)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
            errmsg( "snapshot reference %p is not owned by resource owner %s",
		             snapshot, owner->name)));
}
...
ResourceOwnerForgetFile(ResourceOwner owner, File file)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
            errmsg( "temporery file %d is not owned by resource owner %s",
		             file, owner->name)));
}
