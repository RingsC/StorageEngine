...
#ifdef FOUNDER_XDB_SE
/* true: if wal segment did not send to all of standbys, then  it can not remove or reuse */
bool        wal_force_send_before_removed = false;
#endif

THREAD_LOCAL int			XLOGbuffers = -1;
THREAD_LOCAL int			XLogArchiveTimeout = 0;
bool		XLogArchiveMode = false; //TODO:replace with option
char	   *XLogArchiveCommand = NULL;
bool		EnableHotStandby = false;
THREAD_LOCAL bool		fullPageWrites = true;
bool		log_checkpoints = true;
THREAD_LOCAL int			sync_method = DEFAULT_SYNC_METHOD;
int			wal_level = WAL_LEVEL_MINIMAL;//TODO:replace with option


#ifdef WAL_DEBUG
THREAD_LOCAL bool		XLOG_DEBUG = false;
#endif
extern pthread_t postmasterTid;

#ifdef FOUNDER_XDB_SE
extern THREAD_LOCAL MemoryContext CurrentMemoryContext;
...
static THREAD_LOCAL  XLogRecPtr LastRec;

/*
* Local copy of SharedRecoveryInProgress variable. True actually means "not
* known, need to check the shared state".
*/
static THREAD_LOCAL bool LocalRecoveryInProgress = true;

/*
* Local copy of SharedHotStandbyActive variable. False actually means "not
* known, need to check the shared state".
*/
static THREAD_LOCAL bool LocalHotStandbyActive = false;
...
*/
static THREAD_LOCAL int	LocalXLogInsertAllowed = -1;
...
static THREAD_LOCAL bool restoredFromArchive = false;
...
#else
static  bool recoveryTargetInclusive = false;
...
#ifdef FOUNDER_XDB_SE
TriggerFunc trigger_func = NULL;
#endif //FOUNDER_XDB_SE

/* if recoveryStopsHere returns true, it saves actual stop xid/time/name here */
static THREAD_LOCAL TransactionId recoveryStopXid;
static THREAD_LOCAL TimestampTz recoveryStopTime;
static THREAD_LOCAL char recoveryStopName[MAXFNAMELEN];
static  THREAD_LOCAL bool recoveryStopAfter;
...
static THREAD_LOCAL bool recoveryTargetIsLatest = false;
static THREAD_LOCAL List *expectedTLIs;
static THREAD_LOCAL TimeLineID curFileTLI;
...
*/
static THREAD_LOCAL XLogRecPtr ProcLastRecPtr = {0, 0};

THREAD_LOCAL XLogRecPtr	XactLastRecEnd = {0, 0};
...
* InitXLOGAccess.
*/
static THREAD_LOCAL XLogRecPtr RedoRecPtr;
...
*/
static THREAD_LOCAL XLogRecPtr RedoStartLSN = {0, 0};
...
#define copyXLogwrtRqst(x, y)   \
	do{  \
	copyXLogRecPtr(x.Write, y.Write); \
	copyXLogRecPtr(x.Flush, y.Flush);  \
	}while(0)     \
...
static THREAD_LOCAL XLogCtlData *XLogCtl = NULL;
...
#define INSERT_FREESPACE(Insert)  \
	(uint32)(XLOG_BLCKSZ - ((Insert)->currpos - (char *) (Insert)->currpage))

/* Construct XLogRecPtr value for current insertion point */
#define INSERT_RECPTR(recptr,Insert,curridx)  \
	( \
	(recptr).xlogid = (uint32)XLogCtl->xlblocks[curridx].xlogid, \
	(recptr).xrecoff = \
	(uint32)(XLogCtl->xlblocks[curridx].xrecoff - INSERT_FREESPACE(Insert)) \
	)
...
*/
static THREAD_LOCAL XLogwrtResult LogwrtResult = {{0, 0}, {0, 0}};
...
*/
static THREAD_LOCAL int	openLogFile = -1;
static THREAD_LOCAL uint32 openLogId = 0;
static THREAD_LOCAL uint32 openLogSeg = 0;
static THREAD_LOCAL uint32 openLogOff = 0;
...
*/
static THREAD_LOCAL int	readFile = -1;
static THREAD_LOCAL  uint32 readId = 0;
static THREAD_LOCAL uint32 readSeg = 0;
static THREAD_LOCAL uint32 readOff = 0;
static THREAD_LOCAL uint32 readLen = 0;
static THREAD_LOCAL int	readSource = 0;		/* XLOG_FROM_* code */

/*
* Keeps track of which sources we've tried to read the current WAL
* record from and failed.
*/
static THREAD_LOCAL int	failedSources = 0;	/* OR of XLOG_FROM_* codes */
...
*/
static THREAD_LOCAL TimestampTz XLogReceiptTime = 0;
static THREAD_LOCAL int	XLogReceiptSource = 0;		/* XLOG_FROM_* code */

/* Buffer for currently read page (XLOG_BLCKSZ bytes) */
static THREAD_LOCAL char *readBuf = NULL;

/* Buffer for current ReadRecord result (expandable) */
static  THREAD_LOCAL char *readRecordBuf = NULL;
static  THREAD_LOCAL uint32 readRecordBufSize = 0;

/* State information for XLOG reading */
static THREAD_LOCAL XLogRecPtr ReadRecPtr;	/* start of last record read */
static THREAD_LOCAL XLogRecPtr EndRecPtr;	/* end+1 of last record read */
static THREAD_LOCAL TimeLineID lastPageTLI = 0;
#ifdef FOUNDER_XDB_SE
static THREAD_LOCAL TimeLineID lastSegmentTLI = 0;

/*
 * When we are in standby mode, use lastReadPoint to record the read point while 
 * xlog reply. 
 * Can not use readOff do this, because readOff maybe not the real point it will
 * be read. More information see comment at switched_segment branch of XLogPageRead.
 */
static THREAD_LOCAL XLogRecPtr lastReadPoint = {0, 0};
#endif

static THREAD_LOCAL XLogRecPtr minRecoveryPoint;		/* local copy of
																			* ControlFile->minRecoveryPoint */
#ifdef FOUNDER_XDB_SE
static THREAD_LOCAL TimeLineID minRecoveryPointTLI;
#endif
static THREAD_LOCAL bool updateMinRecoveryPoint = true;
static THREAD_LOCAL bool reachedMinRecoveryPoint = false;

static THREAD_LOCAL bool InRedo = false;

/* Have we launched bgwriter during recovery? */
static THREAD_LOCAL bool bgwriterLaunched = false;

#ifdef FOUNDER_XDB_SE
/* Local copy of WalRcv->receiveTLI */
static THREAD_LOCAL TimeLineID receiveTLI = 0;
#endif
...
static THREAD_LOCAL volatile sig_atomic_t shutdown_requested = false;
static THREAD_LOCAL volatile sig_atomic_t promote_triggered = false;
...
static void ExecuteRecoveryCommand(char *command, const char *commandName,
																	 bool failOnerror);
...
void setInArchiveRecovery(bool isInRecovery);

#ifdef FOUNDER_XDB_SE
static void SetRecoveryTargetTLI(void);
static void ShutDownWalRcvHandler(SIGNAL_ARGS);
#endif //FOUNDER_XDB_SE
...
XLogInsert(RmgrId rmid, uint8 info, XLogRecData *rdata)
{
	...
	XLogRecord *record = 0;
	XLogContRecord *contrecord = 0;
	XLogRecPtr	RecPtr = {0};
	XLogRecPtr	WriteRqst = {0};
	uint32		freespace = 0;
	int			curridx = 0;
	XLogRecData *rdt = 0;
	...
	if (!XLogInsertAllowed())
		ereport(ERROR,
                (errcode(ERRCODE_XLOG_MAKE_NEW_WAL_DURING_RECOVERY),
                errmsg("cannot make new WAL entries during recovery")));

	/* info's high bits are reserved for use by me */
	if (info & XLR_INFO_MASK)
		ereport(PANIC,
                (errcode(ERRCODE_XLOG_ERROR),
                errmsg("invalid xlog info mask %02X", info)));
...
begin:;
			...
			if (i >= XLR_MAX_BKP_BLOCKS)
				ereport(PANIC,
                        (errcode(ERRCODE_OUT_OF_MEMORY),
                         errmsg("can backup at most %d blocks per xlog record",
                         XLR_MAX_BKP_BLOCKS)));
		}
	...
	*/
	if (len == 0 && !isLogSwitch)
		ereport(PANIC,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg("invalid xlog record length %u", len)));
...
#else
			copyXLogwrtRqst(xlogctl->LogwrtResult, LogwrtResult);
#endif //FOUNDER_XDB_SE			
...
#else
				copyXLogRecPtr(xlogctl->LogwrtRqst.Write, LogwrtResult.Write);
#endif //FOUNDER_XDB_SE				
			if (XLByteLT(xlogctl->LogwrtRqst.Flush, LogwrtResult.Flush))
#ifndef FOUNDER_XDB_SE			
				xlogctl->LogwrtRqst.Flush = LogwrtResult.Flush;
#else
				copyXLogRecPtr(xlogctl->LogwrtRqst.Flush, LogwrtResult.Flush);
#endif //FOUNDER_XDB_SE				
...
#else
			copyXLogRecPtr(xlogctl->LogwrtRqst.Write, WriteRqst);
#endif //FOUNDER_XDB_SE 			
		/* update local result copy while I have the chance */
#ifndef FOUNDER_XDB_SE		
		LogwrtResult = xlogctl->LogwrtResult;
#else
		copyXLogwrtRqst(LogwrtResult, xlogctl->LogwrtResult);
#endif //FOUNDER_XDB_SE		
		SpinLockRelease(&xlogctl->info_lck);
	}
...
#else
	if(NULL != PgArchHandler && IsUnderPostmaster)
	{
		pgkill(PgArchHandler->thid,SIGUSR1);
	}
#endif
}
...
#else
				copyXLogRecPtr(xlogctl->LogwrtRqst.Write, FinishedPageRqstPtr);
#endif //FOUNDER_XDB_SE 	
#ifndef FOUNDER_XDB_SE			
			LogwrtResult = xlogctl->LogwrtResult;
#else
...
#else
			if (write(openLogFile, from, (unsigned int)nbytes) != (int32)nbytes)
#endif //FOUNDER_XDB_SE
			{
...
#else
		copyXLogRecPtr(xlogctl->asyncXactLSN, asyncXactLSN);
#endif //FOUNDER_XDB_SE 		
	SpinLockRelease(&xlogctl->info_lck);
}
		...
		*/
...
#else
		copyXLogRecPtr(newMinRecoveryPoint, xlogctl->replayEndRecPtr);
		...
		if (XLByteLT(ControlFile->minRecoveryPoint, newMinRecoveryPoint))
		{
			...
			ereport(DEBUG2,
				...
				minRecoveryPoint.xlogid, minRecoveryPoint.xrecoff, newMinRecoveryPointTLI)));
#else
			ereport(DEBUG2,
...
#else
			copyXLogRecPtr(WriteRqstPtr, xlogctl->LogwrtRqst.Write);
#endif //FOUNDER_XDB_SE 	
#ifndef FOUNDER_XDB_SE		
		LogwrtResult = xlogctl->LogwrtResult;
#else
...
#else
		copyXLogRecPtr(WriteRqstPtr, xlogctl->asyncXactLSN);
#endif //FOUNDER_XDB_SE 		
		SpinLockRelease(&xlogctl->info_lck);
						 ...
						 bool *use_existent, bool use_lock)
{
	...
	*/
	...
	for (nbytes = 0; nbytes < (int)XLogSegSize; nbytes += XLOG_BLCKSZ)
	{
		errno = 0;
	...
	*use_existent = false;
	...
	elog(LOG, "done created new WAL file %08X%08X%08X", 
				ThisTimeLineID, installed_log, installed_seg);
						 ...
						 TimeLineID srcTLI, uint32 srclog, uint32 srcseg)
{
	...
	*/
	if (!InstallXLogFileSegment(&log, &seg, tmppath, false, NULL, false))
		ereport(ERROR,
                (errcode(ERRCODE_XLOG_INSTALL_SEGMENT_FILE_FAILURE),
                errmsg("InstallXLogFileSegment should not have failed")));
}
						 ...
						 int source, bool notfoundOk)
{
	...
	default:
		ereport(ERROR,
                (errcode(ERRCODE_XLOG_INVALID_XLOGREAD_SOURCE),
                errmsg("invalid XLogFileRead source %d", source)));
	}
...
#ifdef FOUNDER_XDB_SE
int
XLogArchiveOrRestoreFile(const char *filename, char *path, bool archive)
{
	int result = 0;
	...
	if (archive)
	{
		...
		if (stat(XLogArchiveCommand, &stat_buf) != 0 && errno == ENOENT)
		{
			if (mkdir(XLogArchiveCommand, S_IRWXU) < 0 && errno != EEXIST)
				ereport(ERROR,
						(errcode_for_file_access(),
		...
		snprintf(to,MAXPGPATH,"%s/%s",XLogArchiveCommand,filename);
		result = copyfile(path, to);
	}
	else
	{	
		char from[MAXPGPATH];
		
		snprintf(from,MAXPGPATH,"%s/%s",recoveryRestoreCommand,filename);
		result = copyfile(from,path);
	}
...
#else
	rc = XLogArchiveOrRestoreFile(xlogfname, xlogpath, false);
#endif
...
*/
static void
ExecuteRecoveryCommand(char *command, const char *commandName, bool failOnSignal)
{
	...
	while ((xlde = ReadDir(xldir, XLOGDIR)) != NULL)
	{
		...
		*/
			...
			strcmp(xlde->d_name + 8, lastoff + 8) <= 0)
		{
			...
			*/
#ifdef FOUNDER_XDB_SE
			if (WalRcvRunning() || XLogArchiveCheckDone(xlde->d_name))
#else
...
#endif
{
	...
	if (hdr->xlp_info & XLP_LONG_HEADER)
	{
		...
		if (longhdr->xlp_sysid != ControlFile->system_identifier)
		{
			...
			*/
			...
			ereport(emode_for_corrupt_record(emode, recaddr),
				(errmsg("WAL file is from different storage engine"),
				errdetail("WAL file storage engine identifier is %s, fxdb_control storage engine identifier is %s.",
	...
	*/
#ifdef FOUNDER_XDB_SE
	if (!tliInHistory(hdr->xlp_tli, expectedTLIs))
#else
	...
	*/
	...
	if (hdr->xlp_tli < (segmentonly ? lastSegmentTLI : lastPageTLI))
	{
		...
		XLogFileName(xlogfile, curFileTLI, readId, readSeg);
		ereport(emode_for_corrupt_record(emode, recaddr),
	...
	if (newtarget != recoveryTargetTLI)
	{
		...
		*/
		if (!list_member_int(newExpectedTLIs,
			...
			ereport(LOG,
			(errmsg("new timeline %u is not a child of storage engine timeline %u",
			newtarget,
			ThisTimeLineID)));
		else
		{
...
static void
SetRecoveryTargetTLI(void)
{
	TimeLineID  probeTLI = 2;
	bool        found = false;

	/* Only in archive recovery we need set recoveryTargetTLI */
	if (!InArchiveRecovery)
		return;
	
	while(existsTimeLineHistory(probeTLI))
	{
		found = true;
		probeTLI++;
	}
	...
	{
		/*
		 * Not found timeline histroy file, we assume that database system did not
		 * branched.
		 */
		recoveryTargetIsLatest = (recoveryTarget == RECOVERY_TARGET_UNSET);
		recoveryTargetTLI = 1;
		return ;
	}
	...
	{
		/*
		 * User not set any recovery target options, we used the latest tli as 
		 * the recovery target tli and we assume that user want to recovery
		 * to latest.
		 */
		recoveryTargetIsLatest = true;
		recoveryTargetTLI = probeTLI;
		return;
	}

	/*
	 * User set the time he want to recovery, decides the recoveryTargetTLI 
	 * according to the recoveryTargetTime.
	 */
	recoveryTargetIsLatest = false;
	if (recoveryTarget == RECOVERY_TARGET_TIME)
	{
		recoveryTargetTLI = DecideRecoveryTargetTLI(probeTLI,
								timestamptz_to_time_t(recoveryTargetTime));
		Assert(recoveryTargetTLI >= 1);
	}
}
...
WriteControlFile(void)
{
	...
	*/
	if (sizeof(ControlFileData) > PG_CONTROL_SIZE)
		ereport(PANIC,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg("sizeof(ControlFileData) is larger than PG_CONTROL_SIZE; fix either one")));
...
#ifdef FOUNDER_XDB_SE
void
SetControlFileNodeId(uint32 nodeId)
{
	LWLockAcquire(ControlFileLock, LW_EXCLUSIVE);
	ControlFile->nodeId = nodeId;
	UpdateControlFile();
	LWLockRelease(ControlFileLock);
}
uint32
GetControlFileNodeId(void)
{
	uint32 nodeId;
	
	LWLockAcquire(ControlFileLock, LW_SHARED);
	nodeId = ControlFile->nodeId;
	LWLockRelease(ControlFileLock);
...
{
	Assert(false);
	recoveryRestoreCommand = (char*)"copy  \"C:\\archive\\%f\" \"%p\"";
	recoveryEndCommand =(char*)"";
	archiveCleanupCommand = (char*)"";
	recoveryPauseAtTarget = false; 
	recoveryTargetXid = 0;
	recoveryTarget = RECOVERY_TARGET_XID;
	recoveryTargetTime = 0;
	recoveryTargetName = (char*)"";
	recoveryTargetInclusive = false;
	StandbyMode = true;	
	PrimaryConnInfo = (char*)"host=localhost port=5432 user=foo password=foopass";
	TriggerFile = (char*)"";
...
TimeLineID GetCurrentTimeLine( void )
{
...
StartupXLOG(void)
{
...
#ifdef FOUNDER_XDB_SE
	bool isStandbyMode = StandbyMode;
#endif //FOUNDER_XDB_SE
	...
	*/
	...
	if (ControlFile->state == DB_SHUTDOWNED)
		ereport(LOG,
		(errmsg("storage engine was shut down at %s",
		str_time(ControlFile->time))));
	else if (ControlFile->state == DB_SHUTDOWNED_IN_RECOVERY)
		ereport(LOG,
		(errmsg("storage engine was shut down in recovery at %s",
		str_time(ControlFile->time))));
	else if (ControlFile->state == DB_SHUTDOWNING)
		ereport(LOG,
		(errmsg("storage engine shutdown was interrupted; last known up at %s",
		str_time(ControlFile->time))));
	else if (ControlFile->state == DB_IN_CRASH_RECOVERY)
		ereport(LOG,
		(errmsg("storage engine was interrupted while in recovery at %s",
		str_time(ControlFile->time)),
		errhint("This probably means that some data is corrupted and"
		" you will have to use the last backup for recovery.")));
	else if (ControlFile->state == DB_IN_ARCHIVE_RECOVERY)
		ereport(LOG,
		(errmsg("storage engine was interrupted while in recovery at log time %s",
		str_time(ControlFile->checkPointCopy.time)),
		errhint("If this has occurred more than once some data might be corrupted"
		" and you might need to choose an earlier recovery target.")));
	else if (ControlFile->state == DB_IN_PRODUCTION)
		ereport(LOG,
		(errmsg("storage engine was interrupted; last known up at %s",
		str_time(ControlFile->time))));
	...
	*/
#ifdef FOUNDER_XDB_SE
	SetRecoveryTargetTLI();
#endif
	if (!InArchiveRecovery || 0 == recoveryTargetTLI)
	{
	...
	*/
#ifdef FOUNDER_XDB_SE
	if (tliOfPointInHistory(ControlFile->checkPoint, expectedTLIs) !=
			ControlFile->checkPointCopy.ThisTimeLineID)
	{
		 ...
		 */
		switchpoint = tliSwitchPoint(ControlFile->checkPointCopy.ThisTimeLineID, expectedTLIs);
		ereport(FATAL,
				(errcode(isStandbyMode ? ERRCODE_RECOVERY_TARGET_TIMELINE_DISMATCH : 0),
	...
	{
		XLogRecPtr  minRecPtr = {0, 0};
		
		minRecPtr = ControlFile->minRecoveryPoint;
		XLByteReduce(minRecPtr, 1);
		if (tliOfPointInHistory(minRecPtr, expectedTLIs) !=
	...
	if (!list_member_int(expectedTLIs,
		...
		ereport(FATAL,
		(errcode(ERRCODE_DATA_EXCEPTION),
		errmsg("requested timeline %u is not a child of storage engine timeline %u",
		recoveryTargetTLI,
		ControlFile->checkPointCopy.ThisTimeLineID)));
#endif
	...
	*/
	...
	if (InRecovery)
	{
		...
		*/
		...
		{
			ereport(LOG,
				(errmsg("storage engine was not properly shut down; "
...
#else
		copyXLogRecPtr(xlogctl->replayEndRecPtr, ReadRecPtr);
		xlogctl->replayEndTLI = ThisTimeLineID;
#endif //FOUNDER_XDB_SE 	
#ifndef FOUNDER_XDB_SE	
		xlogctl->recoveryLastRecPtr = ReadRecPtr;
#else
		copyXLogRecPtr(xlogctl->recoveryLastRecPtr,ReadRecPtr );
#endif //FOUNDER_XDB_SE		
				...
				*/
					...
					(record->xl_info & ~XLR_INFO_MASK) == XLOG_CHECKPOINT_SHUTDOWN)
				{
					...
					if (newTLI != ThisTimeLineID)
					{
						...
						ereport(LOG, (errmsg("xlog replay switch time line from %u to %u",
...
#else
				copyXLogRecPtr(xlogctl->replayEndRecPtr, EndRecPtr);
				xlogctl->replayEndTLI = ThisTimeLineID;
#endif //FOUNDER_XDB_SE 				
				recoveryPause = xlogctl->recoveryPause;
				SpinLockRelease(&xlogctl->info_lck);
...
#else
				copyXLogRecPtr(xlogctl->recoveryLastRecPtr, EndRecPtr);
#endif //FOUNDER_XDB_SE 				
				SpinLockRelease(&xlogctl->info_lck);
...
#ifdef FOUNDER_XDB_SE
	/*
	 * When standby switch to master, starup process had replayed all of xlog
	 * that received from master. So here we do this check. For high availability
	 * we Use WARNING error level if check fail.
	 */
	if (StandbyMode && !XLogRecPtrIsInvalid(lastReadPoint))
	{
		XLogRecPtr receivedUpto = GetWalRcvWriteRecPtr(NULL, NULL);

		if (!XLByteLE(receivedUpto, lastReadPoint))
			ereport(WARNING, (errmsg("standby don't redo all received xlog,"
									"received xlog at %X/%X replay xlog at %X/%X",
									receivedUpto.xlogid, receivedUpto.xrecoff,
									lastReadPoint.xlogid, lastReadPoint.xrecoff)));
	}
#endif
		...
		if (!XLogRecPtrIsInvalid(ControlFile->backupStartPoint))
			ereport(FATAL,
					...
					errmsg("WAL ends before end of online backup"),
					 errhint("Online backup started with pg_start_backup() must be ended with pg_stop_backup(),\
					 and all WAL up to that point must be available at recovery.")));
		else
			ereport(FATAL,
			        (errcode(ERRCODE_XLOG_ERROR),
	...
	*
	* In standby mode, we still need assign o new ID.  System can choose right
	* xlog to read in archive recovery if the archive xlogs were generated by 
	* 'old' master instance and 'new' master instance.
	*/
	if (InArchiveRecovery)
	{
#ifdef FOUNDER_XDB_SE
...
CheckRecoveryConsistency(void)
{
	...
	*/
	...
	//	SendPostmasterSignal(PMSIGNAL_BEGIN_HOT_STANDBY);
		pg_wakeup( postmasterTid, SIGUSR2);
	}
}
...
#else
	copyXLogRecPtr(RedoRecPtr, xlogctl->Insert.RedoRecPtr);
#endif //FOUNDER_XDB_SE 	
	SpinLockRelease(&xlogctl->info_lck);
...
#else
	copyXLogRecPtr( recptr, xlogctl->LogwrtRqst.Write);
#endif //FOUNDER_XDB_SE 	
	SpinLockRelease(&xlogctl->info_lck);
...
#else
	copyXLogRecPtr( recptr, xlogctl->LogwrtResult.Flush);
#endif //FOUNDER_XDB_SE	
	SpinLockRelease(&xlogctl->info_lck);
...
LogCheckpointEnd(bool restartpoint)
{
	...
	*/
	...
	longest_usecs = (int)(CheckpointStats.ckpt_longest_sync -
	...
	average_usecs = (int)(average_sync_time - (uint64) average_secs *1000000);
...
CreateCheckPoint(int flags)
{
	...
	*/
	{
		...
		RedoRecPtr = xlogctl->Insert.RedoRecPtr = checkPoint.redo;
#else
		RedoRecPtr.xlogid = xlogctl->Insert.RedoRecPtr.xlogid = checkPoint.redo.xlogid;
		RedoRecPtr.xrecoff = xlogctl->Insert.RedoRecPtr.xrecoff = checkPoint.redo.xrecoff;
#endif //FOUNDER_XDB_SE 		
		SpinLockRelease(&xlogctl->info_lck);
	}
	...
	*/
	...
	if (nInCommit > 0)
	{
		do
		{
			pg_sleep(10000L);	/* wait for 10 msec */
...
#ifdef FOUNDER_XDB_SE
	ereport(LOG, (errmsg("checkpoint info: redo at %X/%X, checkpoint at %X/%X",
					checkPoint.redo.xlogid, checkPoint.redo.xrecoff,
	...
	*/
	if (shutdown && !XLByteEQ(checkPoint.redo, ProcLastRecPtr))
		ereport(PANIC,
                (errcode(ERRCODE_XLOG_ERROR),
                errmsg("concurrent transaction log activity while storage engine is shutting down")));
	...
	if (_logId || _logSeg)
	{
		...
		*/
		if (wal_keep_segments > 0)
		{
			...
			if (log < (uint32)d_log || ((int32)log == d_log && seg == 0))
			{
				log = 0;
				seg = 1;
			}
...
#ifdef FOUNDER_XDB_SE
		if (wal_force_send_before_removed && 
			wal_level >= WAL_LEVEL_ARCHIVE &&
			max_wal_senders > 0)
		{
			XLogRecPtr minWalSendPtr;

			/*
			 * Before all of wal senders send the segment, it can not remove
			 * or reuse. It maybe remain a lot of xlog segments that can not
			 * been removed.
			 */
			minWalSendPtr = GetOldestWALSendPointer();
			if (minWalSendPtr.xlogid != 0 || minWalSendPtr.xrecoff != 0)
			{
				uint32		log;
				uint32		seg;
			
				XLByteToSeg(minWalSendPtr, log, seg);
				if (log < _logId || (log == _logId && seg < _logSeg))
				{
					_logId = log;
					_logSeg = seg;
...
CreateRestartPoint(int flags)
{
...
#else
	copyXLogRecPtr(lastCheckPointRecPtr, xlogctl->lastCheckPointRecPtr);
#endif //FOUNDER_XDB_SE 	
...
#else
	copyXLogRecPtr( xlogctl->Insert.RedoRecPtr, lastCheckPoint.redo);
#endif //FOUNDER_XDB_SE 	
	SpinLockRelease(&xlogctl->info_lck);
	LWLockRelease(WALInsertLock);
	...
	*/
#ifdef FOUNDER_XDB_SE
	if (WalRcvRunning() && (_logId || _logSeg))
#else
...
#endif
	{
		...
		endptr = GetWalRcvWriteRecPtr(NULL, NULL);
...
#else
	XLogFlush(XLogInsert(RM_XLOG_ID, XLOG_NEXTOID, &rdata));
#endif
...
checkTimeLineSwitch(XLogRecPtr lsn, TimeLineID newTLI)
{
	 ...
	 * decrease.
	 */
	if (newTLI < ThisTimeLineID || !tliInHistory(newTLI, expectedTLIs))
		ereport(PANIC,
...
get_sync_bit(int method)
{
	...
	*/
	...
	default:
		...
		ereport(ERROR,
                (errcode(ERRCODE_XLOG_UNKNOWN_WAL_SYNC_METHOD),
                errmsg("unrecognized wal_sync_method: %d", method)));
...
issue_xlog_fsync(int fd, uint32 log, uint32 seg)
{
	...
	default:
		ereport(PANIC,
                (errcode(ERRCODE_XLOG_UNKNOWN_WAL_SYNC_METHOD),
                errmsg("unrecognized wal_sync_method: %d", sync_method)));
		break;
	}
}
...
do_pg_start_backup(const char *backupidstr, bool fast, char **labelfile)
{
	...
	oldMem = MemoryContextSwitchTo(TopMemoryContext);
	#endif
...
#endif //FOUNDER_XDB_SE 		
		r = (int)fread(labelfile, statbuf.st_size, 1, lfp);
...
#ifdef FOUNDER_XDB_SE
void 
fxdb_close_xlogfile(int code, Datum arg)
{
	if(openLogFile > 0)
	{
		XLogFileClose();
...
#else
	copyXLogRecPtr(recptr, xlogctl->recoveryLastRecPtr);
#endif //FOUNDER_XDB_SE 	
	SpinLockRelease(&xlogctl->info_lck);
...
//		ProcessConfigFile(PGC_SIGHUP);
	...
	*/
	...
	if (shutdown_requested)
	{
		...
		ErrorCauseExit = true;
		ereport(ERROR, 
			(errmsg("thread exit : %d", (int)pthread_self())));
	}
#endif //FOUNDER_XDB_SE
...
StartupProcessMain(void)
{
	...
	*/
	pg_signal(SIGHUP, StartupProcSigHupHandler); /* reload config file */
	pg_signal(SIGINT, SIG_IGN);	/* ignore query cancel */
	pg_signal(SIGTERM, StartupProcShutdownHandler);		/* request shutdown */
	pg_signal(SIGQUIT, startupproc_quickdie);	/* hard crash time */
	if (EnableHotStandby)
		pg_signal(SIGALRM, handle_standby_sig_alarm);	/* ignored unless
																									* InHotStandby */
	else
		pg_signal(SIGALRM, SIG_IGN);
	pg_signal(SIGPIPE, SIG_IGN);
	pg_signal(SIGUSR1, StartupProcSigUsr1Handler);
	pg_signal(SIGUSR2, StartupProcTriggerHandler);
...
#else
	pg_signal(SIGCHLD, ShutDownWalRcvHandler);
#endif //FOUNDER_XDB_SE
...
//	PG_SETMASK(&UnBlockSig);
	...
	* successfully.
	*/
	pg_wakeup( postmasterTid, SIGUSR1);

#ifdef FOUNDER_XDB_SE
	if(NeedStartWalreceiverListener())
	{
		MarkStartWalreceiverListener(false);

		if (max_wal_senders > 0)
			startWalreceiverListener(true);
...
XLogPageRead(XLogRecPtr *RecPtr, int emode, bool fetching_ckpt,
						 bool randAccess)
{
	static THREAD_LOCAL XLogRecPtr receivedUpto = {0, 0};
	bool		switched_segment = false;
	uint32		targetPageOff;
	uint32		targetRecOff;
	uint32		targetId;
	uint32		targetSeg;
	static THREAD_LOCAL pg_time_t last_fail_time = 0;
...
retry:
...
#endif
				{
					...
					*/
					...
					if (havedata)
					{
						...
						* isn't changed.
						*
						* After receiving some WAL over streaming replication, 
						* try to open the file from the timeline we're currently 
						* recieving, not recoveryTargetTLI. They are usually the 
						* same, which is why wasn't noticed before, but you'd get
						* an error if there have been more than one timeline switch 
						* between the current point in WAL and the recovery target.
						*/
						if (readFile < 0)
						{
							readFile =
					...
					*/
					...
					if (!(sources & ~failedSources))
					{
						...
						*/
						...
						{
							pg_sleep((long)(1000000L * (5 - (now - last_fail_time))));
							now = (pg_time_t) time(NULL);
						}
						last_fail_time = now;
...
#else
						//then check for new master and start xlog streaming
						if (CheckForNewMaster() && PrimaryConnInfo)
						{
							XLogRecPtr ptr = fetching_ckpt ? RedoStartLSN : *RecPtr;
							TimeLineID tli = tliOfPointInHistory(ptr, expectedTLIs);
							...
							if (curFileTLI > 0 && tli < curFileTLI)
								elog(ERROR, "according to history file, WAL location %X/%X belongs to timeline %u, but previous recovered WAL file came from timeline %u",
								 		...
								 		tli, curFileTLI);
							curFileTLI = tli;
							RequestXLogStreaming(curFileTLI, 
												ptr, 
								PrimaryConnInfo);
							continue;
						} 
	...
	*/
	...
	{
		/*
		 * Save the read offset in standby mode, use it to check we had replay all
		 * of received xlog from wal receiver after we switch to new master.
		 */
		lastReadPoint.xlogid = readId;
		lastReadPoint.xrecoff = readSeg * XLogSegSize + targetPageOff;
	}
#endif
...
{
	struct stat stat_buf;
	static THREAD_LOCAL bool triggered = false;
...
void setInArchiveRecovery(bool isInRecovery)
{
	InArchiveRecovery = isInRecovery;
}

#ifdef FOUNDER_XDB_SE
void RequestRecoveryFinish()
{
	...
	{
		ereport(LOG,
			(errmsg("shut down recovery.")));

		pg_wakeup(xlog_handle->thid, SIGUSR2);
	}
}

void RegTriggerFunc(TriggerFunc func)
{
	trigger_func = func;
}
...
void RequestShutDownWalRcv()
{
	...
	if (xlog_handle != NULL)
		pgkill(xlog_handle->thid, SIGCHLD);
}

static void
ShutDownWalRcvHandler(SIGNAL_ARGS)
{
	ShutdownWalRcv();
}
