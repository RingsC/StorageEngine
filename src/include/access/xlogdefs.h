...
#define XLByteReduce(recptr, nbytes)						\
	do {													\
		if (recptr.xrecoff <= nbytes)              		    \
		{													\
			recptr.xlogid -= 1; 							\
			recptr.xrecoff									\
				= XLogFileSize + recptr.xrecoff - nbytes;	\
		}													\
		else												\
			recptr.xrecoff -= nbytes;						\
	} while (0)
...
extern int GetPostPort();
extern bool CurrentNodeSync();
