...
static THREAD_LOCAL const FormData_pg_attribute Desc_pg_class[Natts_pg_class] = {Schema_pg_class};
static THREAD_LOCAL const FormData_pg_attribute Desc_pg_attribute[Natts_pg_attribute] = {Schema_pg_attribute};
static THREAD_LOCAL const FormData_pg_attribute Desc_pg_proc[Natts_pg_proc] = {Schema_pg_proc};
static THREAD_LOCAL const FormData_pg_attribute Desc_pg_type[Natts_pg_type] = {Schema_pg_type};
static const FormData_pg_attribute Desc_pg_database[Natts_pg_database] = {Schema_pg_database};
static const FormData_pg_attribute Desc_pg_authid[Natts_pg_authid] = {Schema_pg_authid};
static const FormData_pg_attribute Desc_pg_auth_members[Natts_pg_auth_members] = {Schema_pg_auth_members};
static THREAD_LOCAL const FormData_pg_attribute Desc_pg_index[Natts_pg_index] = {Schema_pg_index};
#endif //FOUNDER_XDB_SE
/*
...
static THREAD_LOCAL HTAB *RelationIdCache;
 ...
 */
static THREAD_LOCAL long relcacheInvalsReceived = 0L;
 ...
 */
static THREAD_LOCAL List *initFileRelationIds = NIL;

/*
 * This flag lets us optimize away work in AtEO(Sub)Xact_RelationCache().
 */
//#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL bool need_eoxact_work = false;
//#else 
//#define need_eoxact_work (CurrentTransactionState->mTopLevelState->bNeed_eoxact_work)
//#endif
...
static THREAD_LOCAL HTAB *OpClassCache = NULL;
...
#ifndef FOUNDER_XDB_SE
static Relation RelationBuildDesc(Oid targetRelId, bool insertIt, Oid tableSpace = 0, IndexType indexType= BTREE_TYPE);
#else
Relation
RelationBuildDesc(Oid relationId, bool insertIt,bool pinIt,Oid dbid);
#endif
...
#else 
	relation->rd_att = single_attibute_tupDesc;
#endif //FOUNDER_XDB_SE											   
...
RelationParseRelOptions(Relation relation, HeapTuple tuple)
{
	...
	options = extractRelOptions(tuple,
								...
								relation->rel_kind == RELKIND_INDEX ?
	...
	if (options)
	{
		...
		relation->rd_options = MemoryContextAlloc(CacheMemoryContext,
											  VARSIZE(options));
#else
		relation->rd_options = (bytea *)MemoryContextAlloc(CacheMemoryContext,
												  VARSIZE(options));
#endif //FOUNDER_XDB_SE
...
RelationBuildTupleDesc(Relation relation)
{
	...
	while (HeapTupleIsValid(pg_attribute_tuple = systable_getnext(pg_attribute_scan)))
	{
...
#else
        Form_pg_attribute attp = &text_attribute;
#endif //FOUNDER_XDB_SE		
#ifndef FOUNDER_XDB_SE
		...
		if (attp->attnum <= 0 ||
			...
			ereport(ERROR,
                    ...
                    errmsg( "invalid attribute number %d for %s",
	...
	if (need != 0)
		ereport(ERROR,
                ...
                errmsg( "catalog is missing %d attribute(s) for relid %u",
 ...
 */
static Relation
RelationBuildDesc(Oid targetRelId, bool insertIt, Oid tableSpace, IndexType indexType)
{
	Relation	relation;
	Oid			relid;
...
RelationBuildDesc(Oid relationId,bool insertIt,bool pinIt,Oid dbid)
{
	...
	TupleDesc tupleDesc = single_attibute_tupDesc;	
	Oid tablespaceId ;
	Oid object_id = relationId;
    Colinfo colInfo = NULL;
	MtInfoData mtInfo;
	extern bool fxdb_get_metatable_info(Oid ,IndexType& ,char& ,Oid& ,Colinfo&, MtInfoData&);

	memset(&mtInfo,0,sizeof(MtInfoData));
	if (isMetaTable(relationId))
	{
		fxdb_get_metatable_info(relationId,indexType,relkind,tablespaceId,colInfo,mtInfo);
		mtInfo.database_id = DEFAULT_DATABASE_OID;
	}
	else if (is_toast_id(relationId))
	{
		if (!fxdb_get_mete_info(relationId,&mtInfo))
		{
			return NULL;
		}
		Relation rel = relation_open(reverse_id_from_toast(relationId),NoLock);
		if (NULL == rel)
		{
			return NULL;
		}

		if (is_toast_table_id(relationId))
		{
			extern TupleDesc get_toast_tuple_desc(void);

			relkind = RELKIND_TOASTVALUE;
			tupleDesc = get_toast_tuple_desc();
		}
		else
		{
			extern Colinfo getColInfo(Oid);
			extern Oid get_toast_index_colinfo();
			indexType = BTREE_TYPE;
			relkind = RELKIND_INDEX;
			colInfo = getColInfo(get_toast_index_colinfo());
		}
		relpersistence = rel->rd_rel->relpersistence;
		tablespaceId = rel->rd_node.spcNode;
		mtInfo.database_id = rel->rd_node.dbNode;
		relationId = mtInfo.rel_filenode;
	...
	else
	{
		...
		indexType = (IndexType)mtInfo.type;
		tablespaceId = mtInfo.spcid;
		colInfo = mtInfo.tableColInfo;
		relpersistence = mtInfo.relpersistence;
		relationId = mtInfo.rel_filenode;
		if (UNKNOWN_TYPE != indexType)
		{
			relkind = RELKIND_INDEX;
		}
	}
	char relname[1024];
	memset(relname, 0, 1024);
	sprintf(relname, "rel name %u", relationId);

	Relation newRel = RelationBuildLocalRelation(relname, 0
		, tupleDesc	, relationId, object_id, tablespaceId,mtInfo.database_id
		, false, false, relpersistence, colInfo,insertIt,pinIt);

	newRel->rd_createSubid = InvalidSubTransactionId;
	newRel->rel_kind = relkind;
	if (!IsSystemTable(relationId) && !is_toast_id(relationId))
	{
		newRel->rd_rel->reltoastrelid = get_toast_table_id(object_id);
		newRel->rd_rel->reltoastidxid = get_toast_index_id(object_id);
	}
	memcpy(&newRel->mt_info,&mtInfo,sizeof(MtInfoData));
		extern RelationAmInfo btree_aminfo ;
		extern RelationAmInfo hash_aminfo ;
		
	if ((0 != mtInfo.rel_filenode) && (!isMetaTable(relationId)) && (newRel->rd_node.relNode != mtInfo.rel_filenode))
		newRel->rd_node.relNode = mtInfo.rel_filenode;
	
	if ((BTREE_TYPE == indexType) || (BTREE_UNIQUE_TYPE == indexType) ||
	    indexType == BTREE_UNIQUE_CLUSTER_TYPE || indexType == BTREE_CLUSTER_TYPE
		|| indexType == LSM_TYPE || indexType == LSM_SUBTYPE)
	{
		newRel->rd_am = &btree_am;
		newRel->rd_aminfo = &btree_aminfo;
	}
	else if (HASH_TYPE == indexType)
	{
		newRel->rd_am = &hash_am;
		newRel->rd_aminfo = &hash_aminfo;
	}
	MemoryContext indexcxt = NULL;
	indexcxt = AllocSetContextCreate(CacheMemoryContext,
		RelationGetRelationName(newRel),
		ALLOCSET_SMALL_MINSIZE,
		ALLOCSET_SMALL_INITSIZE,
		ALLOCSET_SMALL_MAXSIZE);
	newRel->rd_indexcxt = indexcxt;
	/*
	* Insert newly created relation into relcache hash table, if requested.
	*/
 ...
 */
static void
RelationInitPhysicalAddr(Relation relation,Oid dbid)
{
		  ...
		  int natts, const FormData_pg_attribute *attrs)
{
	...
	relation->rel_kind = RELKIND_RELATION;
...
#endif
{
	 ...
	 */
#ifndef FOUNDER_XDB_SE	 
	rd = RelationBuildDesc(relationId, true, tableSpace, indexType);
#endif //FOUNDER_XDB_SE	
	if (RelationIsValid(rd))
		RelationIncrementReferenceCount(rd);
	return rd;
...
RelationReloadIndexInfo(Relation relation)
{
	...
	Assert(relation->rel_kind == RELKIND_INDEX &&
		   !relation->rd_isvalid);
...
#endif //FOUNDER_XDB_SE	
#ifdef FOUNDER_XDB_SE
	if (relation->rd_colinfo != NULL)
	{
		if (relation->rd_colinfo->col_number != NULL) 
			pfree(relation->rd_colinfo->col_number);
		if (relation->rd_colinfo->rd_comfunction != NULL)
			pfree(relation->rd_colinfo->rd_comfunction);
	    pfree(relation->rd_colinfo);
	}
	//free struct mt_info
	MtInfo_Free(relation->mt_info);
#endif //FOUNDER_XDB_SE
	pfree(relation);
}
...
RelationClearRelation(Relation relation, bool rebuild)
{
	 ...
	 */
	if(relation->rd_rel->relpersistence != 't')
	{
		Assert(rebuild ?
	...
	{
		RelationInitPhysicalAddr(relation,relation->rd_node.dbNode);

		if (relation->rel_kind == RELKIND_INDEX)
		{
	...
	else
	{
		...
		newrel = RelationBuildDesc(save_relid,false,false,relation->rd_node.dbNode);
		if (newrel == NULL)
		{
...
RelationForgetRelation(Oid rid)
{
	...
	if (!RelationHasReferenceCountZero(relation) && relation->rd_rel->relpersistence != 't')
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_IN_USE),
                errmsg("relation %u is still open", rid)));
...
AtEOXact_RelationCache(bool isCommit)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
...
#ifdef FOUNDER_XDB_SE
		if(relation->idx_max != (Datum)0)
		{
			pfree(DatumGetPointer(relation->idx_max));
			relation->idx_max = (Datum)0;
		}
		if(relation->idx_min != (Datum)0)
		{
			pfree(DatumGetPointer(relation->idx_min));
			relation->idx_min = (Datum)0;
		}
#endif
		/*
		...
		if (!IsBootstrapProcessingMode())
		{
			...
			{
				elog(LOG, "relation refcount leek:dbid:%d,relid:%d", relation->rd_node.dbNode, relation->rd_node.relNode);
			}
		}
#endif
						   ...
						   bool pinIt)
{
	...
	if (shared_relation != IsSharedRelation(relid))
		ereport(ERROR,
                ...
                errmsg( "shared_relation flag for \"%s\" does not match IsSharedRelation(%u)",
			                 relname, relid)));
...
#else 
    rel->rd_att = tupDesc;
	memset(&rel->mt_info,0,sizeof(MtInfoData));
#endif //FOUNDER_XDB_SE	
#ifdef FOUNDER_XDB_SE
	rel->rd_colinfo = NULL;
	if (colinfo != NULL)
	{ 
		rel->rd_colinfo = (Colinfo)palloc(sizeof(ColinfoData));
		memset(rel->rd_colinfo, 0, sizeof(ColinfoData));
		memcpy(rel->rd_colinfo, colinfo, sizeof(ColinfoData));
		rel->rd_colinfo->split_function = colinfo->split_function;
		if (colinfo->col_number)
		{
			rel->rd_colinfo->col_number = (size_t*)palloc(colinfo->keys * sizeof(size_t));
			memcpy(rel->rd_colinfo->col_number, colinfo->col_number, colinfo->keys * sizeof(size_t));
		}
		if (colinfo->rd_comfunction)
		{
			Assert(0 != colinfo->keys);
			rel->rd_colinfo->rd_comfunction = (CompareCallback*)palloc(colinfo->keys * sizeof(CompareCallback));
			memcpy(rel->rd_colinfo->rd_comfunction, colinfo->rd_comfunction, colinfo->keys * sizeof(CompareCallback));
		}
	}
...
//	rel->rd_att->tdrefcount = 1;	/* mark as refcounted */
...
#else
			rel->rd_backend = CurrentTopLevelState->xid;
#endif //FOUNDER_XDB_SE
			break;
		default:
			ereport(ERROR,
...
#else
	RelationGetRelid(rel) = object_id;
#endif //FOUNDER_XDB_SE
	...
	RelationInitPhysicalAddr(rel,dbid);
...
RelationSetNewRelfilenode(Relation relation, TransactionId freezeXid)
{
	...
	Assert((relation->rel_kind == RELKIND_INDEX ||
			relation->rel_kind == RELKIND_SEQUENCE) ?
	...
	if (relation->rel_kind != RELKIND_SEQUENCE)
	{
...
RelationSetNewRelfilenode(Relation relation, TransactionId freezeXid)
{
	...
	/* Allocate a new relfilenode */
	newrelfilenode = GetNewRelFileNode(relation->rd_node.spcNode, relation->rd_node.dbNode,
									   relation->rd_rel->relpersistence);
	...
	*/
	 ...
	 * update to the relation mapper.
	 */
	relation->rd_node.relNode = newrelfilenode;
	...
	memcpy(compare_data, &RelationGetRelid(relation), sizeof(compare_data));

	Datum value;
	value = fdxdb_string_formdatum(compare_data, sizeof(compare_data));
	ScanKeyData Scankey;
	Fdxdb_ScanKeyInitWithCallbackInfo(&Scankey, META_TABLE_COL_OID, BTEqualStrategyNumber, relid_compare, value);
	IndexScanDesc indscan = index_beginscan(rel, indrel, SnapshotNow, META_TABLE_COL_OID, 0, 0, 0);
	index_rescan(indscan, &Scankey, META_TABLE_COL_OID, NULL, 0);
	...
	{
		memcpy((void*)&self,(void*)&(tuple->t_self), sizeof(ItemPointerData));
		form_meta = fxdb_tuple_to_chars(tuple);
	}
	index_endscan(indscan);
	index_close(indrel, NoLock);

	size_t nSize = sizeof(Oid);
	*((Oid*)(form_meta+(META_TABLE_COL_FILENODE-1)*nSize)) = newrelfilenode;
	*((Oid*)(form_meta+(META_TABLE_COL_FROZENXID-1)*nSize)) = RecentXmin;
	time_t nowtime = time(NULL);
	*((time_t*)(form_meta+(META_TABLE_COL_TIME-1)*nSize)) = (pg_time_t)time;

	HeapTuple mtuple = fdxdb_heap_formtuple(form_meta, sizeof(Form_meta_table_data));
	simple_heap_update(rel, &self, mtuple);
	...
	relation->mt_info.rel_filenode = newrelfilenode;
	/*
	 * Mark the rel as having been given a new relfilenode in the current
	 * (sub) transaction.  This is a hint that can be used to optimize later
	 * operations on the rel in the same transaction.
		...
		if (relation->rd_rel->relowner == InvalidOid)
		{
			...
			if (relation->rd_rel->relowner == InvalidOid)
				ereport(ERROR,
                        ...
                        errmsg( "invalid relowner in pg_class entry for \"%s\"",
					                         RelationGetRelationName(relation))));
...
{
	static THREAD_LOCAL TupleDesc pgclassdesc = NULL;
#ifndef FOUNDER_XDB_SE
...
{
	static THREAD_LOCAL TupleDesc pgindexdesc = NULL;
#ifndef FOUNDER_XDB_SE
...
//	MemoryContext oldcxt;
	...
	indrel = heap_open(RelationRelationId, AccessShareLock);
	indscan = systable_beginscan(indrel, IndexIndrelidIndexId, true,
								 SnapshotNow, 1, &skey);
#else
	extern int relid_compare(const char*, size_t, const char*, size_t);

	heaprel = heap_open(RelationRelationId, AccessShareLock);
	indrel = index_open(MetaTableColThirdIndex, AccessShareLock);

	char compare_data[4];//compare string
	Oid iva_oid = RelationGetRelid(relation);
	memcpy(compare_data, &iva_oid, sizeof(iva_oid));
	...
	indscan = index_beginscan(heaprel, indrel, SnapshotNow, 1, 0, 0, 0);
	...
	MemoryContextSwitchTo(oldcxt);
#else
	while (HeapTupleIsValid(htup = index_getnext(indscan, ForwardScanDirection)))
	{
		bool should_free = false;
		Form_meta_table index = (Form_meta_table) fxdb_get_metastruct_ptr(htup, &should_free);

		/* Add index's OID to result list in the proper order */
		result = insert_ordered_oid(result, index->rel_id);
...
{
	const char *tblspcdir = "fxdb_tblspc";
	DIR		   *dir;
	struct dirent *de;
	char		path[MAXPGPATH];
...
void RelationCacheDeleteIfAny(Oid relid)
{
	Relation rel = NULL;
