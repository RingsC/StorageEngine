...
#include "rasqal.h"
#define RASQAL_MALLOC(type, size) MEMCONTEXT_MALLOC(type, size)
#define RASQAL_CALLOC(type, nmemb, size) MEMCONTEXT_CALLOC(type, nmemb, size)
#define RASQAL_REALLOC(type, ptr, size) MEMCONTEXT_REALLOC(type, ptr, size)
#define RASQAL_FREE(type, ptr)   MEMCONTEXT_FREE(type, ptr)
#endif
...
#define SortEntrySetWorkMem (10 * 1024 * 1024) /* Bytes */

int sort_compare_row_func(const char *str1, size_t len1, const char *str2, size_t len2);
void rasqal_sort_cmp_row_set_row(rasqal_sort_compare_row *cmp_row, rasqal_row *row);
void rasqal_sort_cmp_row_set_map(rasqal_sort_compare_row *cmp_row, rasqal_map *map);
rasqal_row *rasqal_sort_cmp_row_get_row(rasqal_sort_compare_row *cmp_row);
rasqal_map *rasqal_sort_cmp_row_get_map(rasqal_sort_compare_row *cmp_row);
size_t rasqal_sort_cmp_row_struct_size();
rasqal_sort_compare_row *rasqal_new_sort_cmp_row();
