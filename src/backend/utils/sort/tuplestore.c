...
tuplestore_set_eflags(Tuplestorestate *state, int eflags)
{
	...
	if (state->status != TSS_INMEM || state->memtupcount != 0)
		ereport(ERROR,
                (errcode(ERRCODE_TUPLE_STORE_CALL_SET_EFLAG_LATE),
                errmsg("too late to call tuplestore_set_eflags")));
...
tuplestore_alloc_read_pointer(Tuplestorestate *state, int eflags)
{
	...
	if (state->status != TSS_INMEM || state->memtupcount != 0)
	{
		if ((state->eflags | eflags) != state->eflags)
			ereport(ERROR,
                    (errcode(ERRCODE_TUPLE_STORE_REQUIRE_NEW_EFLAG_LATE),
                    errmsg("too late to require new tuplestore eflags")));
	}
...
tuplestore_select_read_pointer(Tuplestorestate *state, int ptr)
{
	...
	switch (state->status)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_TUPLE_STORE_INVALID_STATE),
                    errmsg("invalid tuplestore state")));
			break;
	}
	...
	switch (state->status)
	{
		...
		case TSS_READFILE:
			...
			if (BufFileSeek(state->myfile,
							...
							SEEK_SET) != 0)
				ereport(ERROR,
                        (errcode(ERRCODE_TUPLE_SEEK_EOF_FAILURE),
                        errmsg("tuplestore seek to EOF failed")));
			state->status = TSS_WRITEFILE;
			...
			*should_free = false;
			...
			else
			{
				...
				else
				{
					...
					{
						Assert(!state->truncated);
						readptr->current = state->memtupcount + 1;
					} else if (readptr->current < state->memtupdeleted)
					{
						Assert(!state->truncated);
						return NULL;
					}
		...
		case TSS_READFILE:
			*should_free = true;
							...
							SEEK_CUR) != 0)
			{
				...
				readptr->eof_reached = true;
				Assert(!state->truncated);
				if (BufFileSeek(state->myfile, state->writepos_file, 
					state->writepos_offset - sizeof(unsigned int), SEEK_SET) != 0)
					return NULL;
#endif //FOUNDER_XDB_SE
			}
...
tuplestore_rescan(Tuplestorestate *state)
{
	...
	switch (state->status)
	{
		...
		case TSS_READFILE:
			...
			if (BufFileSeek(state->myfile, 0, 0L, SEEK_SET) != 0)
				ereport(ERROR,
                        (errcode(ERRCODE_TUPLE_SEEK_START_FAILURE),
                        errmsg("tuplestore seek to start failed")));
			break;
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_TUPLE_STORE_INVALID_STATE),
	...
	/*if(readptr->eflags & EXEC_FLAG_BACKWARD)
	    readptr->eof_reached = true;*/
}
...
writetup_heap(Tuplestorestate *state, void *tup)
{
	...
	if (BufFileWrite(state->myfile, (void *) &tuplen,
		...
		ereport(ERROR,
                (errcode(ERRCODE_TUPLE_STORE_WRITE_FAILURE),
                errmsg("write failed")));
