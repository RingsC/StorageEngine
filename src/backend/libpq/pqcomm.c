 ...
 */
int		   THREAD_LOCAL Unix_socket_permissions;
char	   THREAD_LOCAL *Unix_socket_group;


/* Where the Unix socket file is */
static THREAD_LOCAL char sock_path[MAXPGPATH];
 ...
 * Message status
 */
static THREAD_LOCAL bool PqCommBusy;
static THREAD_LOCAL bool DoingCopyOut;
...
pq_init(void)
{
	...
	PqSendBuffer = (char *)MemoryContextAlloc(TopMemoryContext, PqSendBufferSize);
				 ...
				 pgsocket ListenSocket[], int MaxListen)
{
	...
	for (addr = addrs; addr; addr = addr->ai_next)
	{
		 ...
		 * connections.
		 */
		err = bind(fd, addr->ai_addr, (int)addr->ai_addrlen);
		if (err < 0)
		{
			extern char *inet_net_ntop(int af, const void *src, int bits,
                           char *dst, size_t size);
			char addrstr[MAXPGPATH] = {'\0'};

			if (addr->ai_family == AF_INET)
				inet_net_ntop(addr->ai_family, 
			...
			ereport(LOG,
					(errcode_for_socket_access(),
			/* translator: %s is IPv4, IPv6, or Unix */
					 errmsg("could not bind %s(addr %s:%d) socket: %m",
							familyDesc, 
							addrstr[0] == '\0' ? "???" : addrstr, portNumber),
 ...
 * --------------------------------
 */
extern THREAD_LOCAL int			pgwin32_noblock;
static void
...
pq_recvbuf(void)
{
	...
	for (;;)
	{
		...
		r = (int)secure_read(MyProcPort, PqRecvBuffer + PqRecvLength,
...
pq_getbyte_if_available(unsigned char *c)
{
	...
	r = (int)secure_read(MyProcPort, c, 1);
	if (r < 0)
	{
		/*
...
internal_flush(void)
{
	static THREAD_LOCAL int	last_reported_send_errno = 0;
	...
	while (bufptr < bufend)
	{
		...
		r = (int)secure_write(MyProcPort, bufptr, bufend - bufptr);
...
pq_putmessage_noblock(char msgtype, const char *s, size_t len)
{
	 ...
	 */
	required = PqSendPointer + 1 + 4 + (int)len;
	if (required > PqSendBufferSize)
	{
		PqSendBuffer = (char *)repalloc(PqSendBuffer, required);
