 ...
 */
static THREAD_LOCAL SlruCtlData OldSerXidSlruCtlData;
...
#define OldSerXidNextPage(page) (((page) >= (int)OLDSERXID_MAX_PAGE) ? 0 : (page) + 1)
...
static THREAD_LOCAL OldSerXidControl oldSerXidControl;
 ...
 * commitSeqNo is used.
 */
static THREAD_LOCAL SERIALIZABLEXACT *OldCommittedSxact;
...
#else
THREAD_LOCAL int			max_predicate_locks_per_xact = 64;		/* set by guc.c */
 ...
 * fixed upon creation.
 */
static THREAD_LOCAL PredXactList PredXact;

/*
 * This provides a pool of RWConflict data elements to use in conflict lists
 * between transactions.
 */
static THREAD_LOCAL RWConflictPoolHeader RWConflictPool;

/*
 * The predicate locking hash tables are in shared memory.
 * Each backend keeps pointers to them.
 */
static THREAD_LOCAL HTAB *SerializableXidHash;
static THREAD_LOCAL HTAB *PredicateLockTargetHash;
static THREAD_LOCAL HTAB *PredicateLockHash;
static THREAD_LOCAL SHM_QUEUE *FinishedSerializableTransactions;
 ...
 */
static THREAD_LOCAL const PREDICATELOCKTARGETTAG ScratchTargetTag = {0, 0, 0, 0, 0};
static THREAD_LOCAL uint32 ScratchTargetTagHash;
static THREAD_LOCAL int	ScratchPartitionLock;
 ...
 */
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL HTAB *LocalPredicateLockHash = NULL;
#else
#define LocalPredicateLockHash (CurrentTransactionState->mTopLevelState->mLocalPredicateLockHash)
#endif
 ...
 */
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL SERIALIZABLEXACT *MySerializableXact = InvalidSerializableXact;
static THREAD_LOCAL bool MyXactDidWrite = false;
#else
#define MySerializableXact (CurrentTransactionState->mTopLevelState->mMySerializableXact)
#define MyXactDidWrite (CurrentTransactionState->mTopLevelState->mMyXactDidWrite)
#endif //FOUNDER_XDB_SE
...
OldSerXidPagePrecedesLogically(int p, int q)
{
	 ...
	 */
	Assert(p >= 0 && p <= (int)OLDSERXID_MAX_PAGE);
	Assert(q >= 0 && q <= (int)OLDSERXID_MAX_PAGE);

	diff = p - q;
	if (diff >= (int)((OLDSERXID_MAX_PAGE + 1) / 2))
...
OldSerXidInit(void)
{
	...
	SimpleLruInit(OldSerXidSlruCtl, "OldSerXid SLRU Ctl",
				  NUM_OLDSERXID_BUFFERS, 0, OldSerXidLock, "fxdb_serial");
...
InitPredicateLocks(void)
{
	...
	PredXact = ShmemInitStruct("PredXactList",
							   ...
							   &found);
#else
	PredXact = (PredXactList)ShmemInitStruct("PredXactList",
							   PredXactListDataSize,
							   &found);
#endif // FOUNDER_XDB_SE
	if (!found)
...
#else
		PredXact->element = (PredXactListElement)ShmemAlloc(requestSize);
#endif // FOUNDER_XDB_SE
	...
	RWConflictPool = ShmemInitStruct("RWConflictPool",
									 ...
									 &found);
#else
	RWConflictPool = (RWConflictPoolHeader)ShmemInitStruct("RWConflictPool",
									 RWConflictPoolHeaderDataSize,
									 &found);
#endif // FOUNDER_XDB_SE
	if (!found)
...
#else
		RWConflictPool->element = (RWConflict)ShmemAlloc(requestSize);
#endif // FOUNDER_XDB_SE
...
GetPredicateLockStatusData(void)
{
...
#else
	for (i = 0; i < NUM_PREDICATELOCK_PARTITIONS; i++)
		LWLockAcquire((LWLockId)(FirstPredicateLockMgrLock + i), LW_SHARED);
#endif // FOUNDER_XDB_SE
	...
	while ((predlock = (PREDICATELOCK *) hash_seq_search(&seqstat)) != NULL)
	{
...
#else
	for (i = NUM_PREDICATELOCK_PARTITIONS - 1; i >= 0; i--)
		LWLockRelease((LWLockId)(FirstPredicateLockMgrLock + i));
#endif // FOUNDER_XDB_SE
...
#else
	if (!lockheld)
		LWLockAcquire((LWLockId)ScratchPartitionLock, LW_EXCLUSIVE);
#endif // FOUNDER_XDB_SE
	hash_search_with_hash_value(PredicateLockTargetHash,
								&ScratchTargetTag,
...
#else
	if (!lockheld)
		LWLockRelease((LWLockId)ScratchPartitionLock);
#endif // FOUNDER_XDB_SE
}
...
RemoveTargetIfNoLongerUsed(PREDICATELOCKTARGET *target, uint32 targettaghash)
{
	...
	rmtarget = hash_search_with_hash_value(PredicateLockTargetHash,
										   ...
										   HASH_REMOVE, NULL);
#else
	rmtarget = (PREDICATELOCKTARGET *)hash_search_with_hash_value(PredicateLockTargetHash,
										   &target->tag,
										   targettaghash,
										   HASH_REMOVE, NULL);
#endif // FOUNDER_XDB_SE
								  ...
								  bool removeOld)
{
	...
	oldtarget = hash_search_with_hash_value(PredicateLockTargetHash,
											...
											HASH_FIND, NULL);
#else
	oldtarget = (PREDICATELOCKTARGET *)hash_search_with_hash_value(PredicateLockTargetHash,
											&oldtargettag,
											oldtargettaghash,
											HASH_FIND, NULL);
#endif // FOUNDER_XDB_SE
	...
	if (oldtarget)
	{
		...
		newtarget = hash_search_with_hash_value(PredicateLockTargetHash,
												...
												HASH_ENTER_NULL, &found);
#else
		newtarget = (PREDICATELOCKTARGET *)hash_search_with_hash_value(PredicateLockTargetHash,
												&newtargettag,
												newtargettaghash,
												HASH_ENTER_NULL, &found);
#endif // FOUNDER_XDB_SE
...
#else
	for (i = 0; i < NUM_PREDICATELOCK_PARTITIONS; i++)
		LWLockAcquire((LWLockId)(FirstPredicateLockMgrLock + i), LW_EXCLUSIVE);
#endif // FOUNDER_XDB_SE
	LWLockAcquire(SerializableXactHashLock, LW_EXCLUSIVE);
...
#else
	while ((oldtarget = (PREDICATELOCKTARGET *) hash_seq_search(&seqstat)) != NULL)
#endif // FOUNDER_XDB_SE
	{
		PREDICATELOCK *oldpredlock;
		...
		if (transfer && heaptarget == NULL)
		{
			...
			heaptarget = hash_search_with_hash_value(PredicateLockTargetHash,
													 ...
													 HASH_ENTER, &found);
#else
			heaptarget = (PREDICATELOCKTARGET *)hash_search_with_hash_value(PredicateLockTargetHash,
													 &heaptargettag,
													 heaptargettaghash,
													 HASH_ENTER, &found);
#endif // FOUNDER_XDB_SE
...
ReleasePredicateLocks(bool isCommit)
{
	...
	/* may not be serializable during COMMIT/ROLLBACK PREPARED todo:check it - sunwf*/
...
#else
			predlock = (PREDICATELOCK *)hash_search_with_hash_value(PredicateLockHash, &tag,
								PredicateLockHashCodeFromTargetHashCode(&tag,
															  targettaghash),
												   HASH_ENTER_NULL, &found);
#endif // FOUNDER_XDB_SE
	...
	switch (htsvResult)
	{
		...
		default:
			...
			ereport(ERROR,
                    ...
                    errmsg("unrecognized return value from HeapTupleSatisfiesVacuum: %u", htsvResult)));
...
#else
	memset(&mypredlocktag, 0, sizeof(mypredlocktag));
#endif // FOUNDER_XDB_SE
...
#else
	while ((target = (PREDICATELOCKTARGET *) hash_seq_search(&seqstat)) != NULL)
#endif // FOUNDER_XDB_SE
...
#else
	for (i = NUM_PREDICATELOCK_PARTITIONS - 1; i >= 0; i--) {
		LWLockId tmpLockId = (LWLockId)(FirstPredicateLockMgrLock + i);
		LWLockRelease(tmpLockId);
	}
#endif // FOUNDER_XDB_SE
	LWLockRelease(SerializablePredicateLockListLock);
}
