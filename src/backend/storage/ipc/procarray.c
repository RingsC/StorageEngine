...
static THREAD_LOCAL ProcArrayStruct *procArray;

/*
 * Bookkeeping for tracking emulated transactions in recovery
 */
static THREAD_LOCAL TransactionId *KnownAssignedXids;
static THREAD_LOCAL bool *KnownAssignedXidsValid;
static THREAD_LOCAL TransactionId latestObservedXid = InvalidTransactionId;
 ...
 * KnownAssignedXids.
 */
static THREAD_LOCAL TransactionId standbySnapshotPendingXmin;

#ifdef XIDCACHE_DEBUG

/* counters for XidCache measurement */
static THREAD_LOCAL long xc_by_recent_xmin = 0;
static THREAD_LOCAL long xc_by_known_xact = 0;
static THREAD_LOCAL long xc_by_my_xact = 0;
static THREAD_LOCAL long xc_by_latest_xid = 0;
static THREAD_LOCAL long xc_by_main_xid = 0;
static THREAD_LOCAL long xc_by_child_xid = 0;
static THREAD_LOCAL long xc_by_known_assigned = 0;
static THREAD_LOCAL long xc_no_overflow = 0;
static THREAD_LOCAL long xc_slow_answer = 0;
...
static const int MAX_CONCURRENT_TRANSACTION = 512;
struct TransactionStateArray
{
    TransactionState base[MAX_CONCURRENT_TRANSACTION];
    int curSize;
...
static void DestoryTransactionArray( void )
{
	Assert(NULL != TransArray);
	pthread_mutex_destroy(&TransArray->mut);
	pfree(TransArray);
	TransArray = NULL;
}

void InitTransactionArray( void )
{
 	TransArray = (TransactionStateArray*)talloc(sizeof(TransactionStateArray));
    Assert(NULL != TransArray);
	memset(TransArray->base,0,sizeof(TransArray->base));
	TransArray->curSize = 0;
	pthread_mutex_init(&TransArray->mut,NULL);

#ifdef _DEBUG
	TransArray->nTotalAdded = 0;
    TransArray->nTotalRemoved = 0;
#endif

	atexit(DestoryTransactionArray);
}

void AddTransation2Array(TransactionState pTransState)
{
	Assert(NULL != pTransState);
	pthread_mutex_lock(&TransArray->mut);
	if(TransArray->curSize >= MAX_CONCURRENT_TRANSACTION)
	{
		pthread_mutex_unlock(&TransArray->mut);
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
				errmsg("Too many transactons run in concurrent")));
	}
	else
	{
		TransArray->base[TransArray->curSize] = pTransState;
		TransArray->curSize++;
#ifdef _DEBUG
        ++TransArray->nTotalAdded;
#endif
	}
	pthread_mutex_unlock(&TransArray->mut);
}

void AddTransation2ArrayInLock(TransactionState pTransState)
{
	Assert(NULL != pTransState);
	//pthread_mutex_lock(&TransArray->mut);
	if(TransArray->curSize >= MAX_CONCURRENT_TRANSACTION)
...
void RemoveTransactonFromArray(TransactionState pTransState, TransactionId xid)
{
	Assert(NULL != pTransState);
	bool found = false;
	pthread_mutex_lock(&TransArray->mut);
	for(int iTrans = 0; iTrans < TransArray->curSize; ++iTrans)
	{
		if(pTransState == TransArray->base[iTrans])
		{
			if(iTrans != (TransArray->curSize - 1))
			{
			    TransArray->base[iTrans] = TransArray->base[TransArray->curSize - 1];
			}
			TransArray->base[TransArray->curSize - 1] = NULL;
			--TransArray->curSize;
			found = true;
#ifdef _DEBUG
        ++TransArray->nTotalRemoved;
#endif
		}
	}

	AddPrepareXact2Array(xid);
	pthread_mutex_unlock(&TransArray->mut);
	Assert(found);
}

void RemoveLegacyTransaction( void )
{
    if(CurrentTransactionState != GetDefaultXact() && (NULL != CurrentTransactionState))
	{
		RemoveTransactonFromArray(CurrentTransactionState, InvalidTransactionId);
		CurrentTransactionState = GetDefaultXact();
    }
}
...
ProcArrayEndTransaction(PGPROC *proc, TransactionId latestXid)
{
	if (TransactionIdIsValid(latestXid))
	{
		...
		Assert(TransactionIdIsValid(proc->xid));
#else
        Assert(TransactionIdIsValid(ProcTopLevel->xid));
#endif
...
#else
		ProcTopLevel->xid = InvalidTransactionId;
		ProcTopLevel->lxid = InvalidLocalTransactionId;
		ProcTopLevel->xmin = InvalidTransactionId;
		ProcTopLevel->lockProc->vacuumFlags  &= ~PROC_VACUUM_STATE_MASK;
		ProcTopLevel->inCommit = false;
#endif
...
#else
		ProcTopLevel->subxids.nxids = 0;
		ProcTopLevel->subxids.overflowed = false;
#endif
...
#else
		Assert(ProcTopLevel->subxids.nxids == 0);
		Assert(ProcTopLevel->subxids.overflowed == false);
#endif
	}
}
...
ProcArrayApplyRecoveryInfo(RunningTransactions running)
{
	...
	xids = (TransactionId *)palloc(sizeof(TransactionId) * running->xcnt);
...
{
	static THREAD_LOCAL TransactionId *xids = NULL;
	int			nxids = 0;
	ProcArrayStruct *arrayP = procArray;
...
TransactionIdIsInProgress(TransactionId xid)
{
	...
	if (xids == NULL)
	{
		...
		xids = (TransactionId *) talloc0(maxxids * sizeof(TransactionId));
	...
	/* No shortcuts, gotta grovel through the array */
	pthread_mutex_lock(&TransArray->mut);
	for (i = 0; i < TransArray->curSize; i++)
	{
		TransactionState state = TransArray->base[i];
		TopLevelState *pTopLevel = state->mTopLevelState;
		TransactionId pxid;
		 ...
		 * Step 2: check the cached child-Xids arrays
		 */
		for (j = pTopLevel->subxids.nxids - 1; j >= 0; j--)
		{
			/* Fetch xid just once - see GetNewTransactionId */
			TransactionId cxid = pTopLevel->subxids.xids[j];
		 ...
		 * worry about.)
		 */
		if (pTopLevel->subxids.overflowed)
			xids[nxids++] = pxid;
...
TransactionIdIsActive(TransactionId xid)
{
	...
	for (i = 0; i < TransArray->curSize; i++)
	{
		...
		TransactionState state = TransArray->base[i];
		/* Fetch xid just once - see GetNewTransactionId */
		TransactionId pxid = state->mTopLevelState->xid;
...
#else
	pthread_mutex_lock(&TransArray->mut);
	for (index = 0; index < TransArray->curSize; index++)
	{
		TransactionState state = TransArray->base[index];
		TopLevelState* topLevel = state->mTopLevelState;
		if (ignoreVacuum && (topLevel->lockProc->vacuumFlags & PROC_IN_VACUUM))
			continue;


		if (allDbs ||
        	topLevel->databaseId == MyDatabaseId ||
			topLevel->databaseId == 0)		/* always include WalSender */
		{
			/* Fetch xid just once - see GetNewTransactionId */
			TransactionId xid = topLevel->xid;
			 ...
			 * Xid, that could determine some not-yet-set Xmin.
			 */
			xid = topLevel->xmin;	/* Fetch just once */
...
GetSnapshotData(Snapshot snapshot)
{
	...
	if (snapshot->xip == NULL)
	{
		...
		snapshot->xip = (TransactionId *)
			palloc0(arrayP->maxProcs * sizeof(TransactionId));
		...
		snapshot->subxip = (TransactionId *)
			palloc0(TOTAL_MAX_CACHED_SUBXIDS * sizeof(TransactionId));
		MemoryContextSwitchTo(oldcxt);
#endif //FOUNDER_XDB_SE
...
#else
		pthread_mutex_lock(&TransArray->mut);
		int nTransSize = TransArray->curSize;
		for(int iTrans = 0;iTrans < nTransSize; ++iTrans)
		{
			TransactionState pState = TransArray->base[iTrans];
			if(NULL == pState)
			{
				continue;
			}

			TopLevelState* pTopLevelState = pState->mTopLevelState;
			Assert(NULL != pTopLevelState);

			/* Update globalxmin to be the smallest valid xmin */
			TransactionId xid = pTopLevelState->xmin;	/* fetch just once */
			...
			if (TransactionIdIsNormal(xid))
			{
				if (TransactionIdFollowsOrEquals(xid, xmax))
					continue;
				if (pTopLevelState != CurrentTransactionState->mTopLevelState)
					snapshot->xip[count++] = xid;
				if (TransactionIdPrecedes(xid, xmin))
					xmin = xid;
			}
			 ...
			 * be later than their parent, so no need to check them against
			 * xmin.  We could filter against xmax, but it seems better not to
			 * do that much work while holding the pTopLevelStateArrayLock.
			 *
			 * The other backend can add more subxids concurrently, but cannot
			 * remove any.	Hence it's important to fetch nxids just once.
			 * Should be safe to use memcpy, though.  (We needn't worry about
			 ...
			 * Again, our own XIDs are not included in the snapshot.
			 */
			if (!suboverflowed && pTopLevelState != CurrentTransactionState->mTopLevelState)
			{
				if (pTopLevelState->subxids.overflowed)
					suboverflowed = true;
				else
				{
					int			nxids = pTopLevelState->subxids.nxids;

					if (nxids > 0)
					{
						memcpy(snapshot->subxip + subcount,
							   (void *) pTopLevelState->subxids.xids,
							   nxids * sizeof(TransactionId));
						subcount += nxids;
					}
				}
...
#else
	if (!TransactionIdIsValid(MyProcTopLevel->xmin))
		MyProcTopLevel->xmin = TransactionXmin = xmin;
#endif
	LWLockRelease(ProcArrayLock);
...
{
	/* result workspace */
	static THREAD_LOCAL RunningTransactionsData CurrentRunningXactsData;
	...
	for (index = 0; index < TransArray->curSize; index++)
	{
		TopLevelState *pTopLevel = TransArray->base[index]->mTopLevelState;
		//volatile PGPROC *proc = arrayP->procs[index];
		TransactionId xid;
		int			nxids;
...
#else
		nxids = pTopLevel->subxids.nxids;
		if (nxids > 0)
		{
			memcpy(&xids[count], (void *) pTopLevel->subxids.xids,
				   nxids * sizeof(TransactionId));
			count += nxids;
			subcount += nxids;
...
GetTransactionsInCommit(TransactionId **xids_p)
{
	...
	for (index = 0; index < TransArray->curSize; index++)
	{
		...
		if (proc->inCommit && TransactionIdIsValid(pxid))
			xids[nxids++] = pxid;
#else
		TransactionId pxid = pTopLevel->xid;
		if (pTopLevel->inCommit && TransactionIdIsValid(pxid))
			xids[nxids++] = pxid;
#endif
					  ...
					  int *nvxids)
{
	...
	/* allocate what's certainly enough result space */
	vxids = (VirtualTransactionId *)
		palloc(sizeof(VirtualTransactionId) * MAX_CONCURRENT_TRANSACTION);

	pthread_mutex_lock(&TransArray->mut);

	for (index = 0; index < TransArray->curSize; index++)
	{
		TransactionState transState = TransArray->base[index];
		TopLevelState *pTopLevel = transState->mTopLevelState;
		if (transState == CurrentTransactionState)
			continue;
		if (excludeVacuum & transState->mTopLevelState->lockProc->vacuumFlags)
			continue;

		if (allDbs /*|| proc->databaseId == MyDatabaseId*/)
		{
			/* Fetch xmin just once - might change on us */
			TransactionId pxmin = pTopLevel->xmin;
			...
			if (!TransactionIdIsValid(limitXmin) ||
				TransactionIdPrecedesOrEquals(pxmin, limitXmin))
			{
				VirtualTransactionId vxid = pTopLevel->vid;
...
{
	static THREAD_LOCAL VirtualTransactionId *vxids;
	ProcArrayStruct *arrayP = procArray;
	int			count = 0;
	int			index;
...
GetConflictingVirtualXIDs(TransactionId limitXmin, Oid dbOid)
{
	...
	{
		vxids = (VirtualTransactionId *)
			malloc(sizeof(VirtualTransactionId) * (MAX_CONCURRENT_TRANSACTION + 1));
	...
	for (index = 0; index < TransArray->curSize; index++)
	{
		...
		{
			/* Fetch xmin just once - can't change on us, but good coding */
			TransactionId pxmin = state->mTopLevelState->xmin;
...
#else
		if(ProcTopLevel->xid == InvalidTransactionId)
#endif
...
#define MAXAUTOVACPIDS	10		/* max autovacs to SIGTERM per iteration */
	pthread_t	autovac_pids[MAXAUTOVACPIDS];
	int			tries;
	...
	for (tries = 0; tries < 50; tries++)
	{
		...
		*nbackends = *nprepared = 0;
...
#else
				if ((ProcTopLevel->lockProc->vacuumFlags & PROC_IS_AUTOVACUUM) &&
#endif
		...
		for (index = 0; index < nautovacs; index++)
			(void) pgkill(autovac_pids[index], SIGTERM);	/* ignore any error */

		/* sleep, then try again */
		pg_sleep(100 * 1000L); /* 100ms */
	}
...
#define XidCacheRemove(i) \
	do { \
		MyProcTopLevel->subxids.xids[i] = MyProcTopLevel->subxids.xids[MyProcTopLevel->subxids.nxids - 1]; \
		MyProcTopLevel->subxids.nxids--; \
	} while (0)
						  ...
						  TransactionId latestXid)
{
	...
	for (i = nxids - 1; i >= 0; i--)
	{
...
#else
		for (j = MyProcTopLevel->subxids.nxids - 1; j >= 0; j--)
		{
			if (TransactionIdEquals(MyProcTopLevel->subxids.xids[j], anxid))
			{
				XidCacheRemove(j);
				break;
			}
		 ...
		 * debug warning.
		 */
		if (j < 0 && !MyProcTopLevel->subxids.overflowed)
			elog(WARNING, "did not find subXID %u in MyProc", anxid);
#endif
	...
	for (j = MyProcTopLevel->subxids.nxids - 1; j >= 0; j--)
	{
		if (TransactionIdEquals(MyProcTopLevel->subxids.xids[j], xid))
		{
			XidCacheRemove(j);
			break;
		}
					 ...
					 bool exclusive_lock)
{
		...
		TransactionIdFollowsOrEquals(KnownAssignedXids[head - 1], from_xid))
	{
		...
		ereport(ERROR,
                ...
                errmsg("out-of-order XID insertion in KnownAssignedXids")));
	}
