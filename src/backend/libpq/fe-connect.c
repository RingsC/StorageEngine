...
#define ERRCODE_INVALID_PASSWORD_ "28P01"
/* This too */
#define ERRCODE_CANNOT_CONNECT_NOW_ "57P03"
...
static char *PasswordFromFile(const char *hostname, char *port, char *dbname,
				 char *username);
...
THREAD_LOCAL pgthreadlock_t pg_g_threadlock = default_threadlock;
		...
		case CONNECTION_NEEDED:
			{
				...
				while (conn->addr_cur != NULL)
				{
					...
					memcpy(&conn->raddr.addr, addr_cur->ai_addr,
						   addr_cur->ai_addrlen);
					conn->raddr.salen = (int)addr_cur->ai_addrlen;

					/* Open a socket */
					conn->sock = (int)socket(addr_cur->ai_family, SOCK_STREAM, 0);
					if (conn->sock < 0)
					{
						/*
		...
		case CONNECTION_AWAITING_RESPONSE:
			{
				...
				if (!(beresp == 'R' || beresp == 'E'))
				{
					appendPQExpBuffer(&conn->errorMessage,
									  libpq_gettext(
									  "please check whether the port is correct, expected authentication request from "
...
internal_ping(PGconn *conn)
{
	 ...
	 */
	if (strcmp(conn->last_sqlstate, ERRCODE_CANNOT_CONNECT_NOW_) == 0)
				...
				char *errbuf, int errbufsize)
{
	 ...
	 * only kernel calls.
	 */
	if ((tmpsock = (int)socket(raddr->addr.ss_family, SOCK_STREAM, 0)) < 0)
	{
...
cancel_errReturn:
	 ...
	 */
	maxlen = errbufsize - (int)strlen(errbuf) - 2;
	if (maxlen >= 0)
	{
...
PQrequestCancel(PGconn *conn)
{
	...
	r = internal_cancel(&conn->raddr, conn->be_pid, conn->be_key,
						conn->errorMessage.data, (int)conn->errorMessage.maxlen);
...
{
	if (!conn)
		return libpq_gettext((char*)"connection pointer is NULL\n");
...
static char *
PasswordFromFile(const char *hostname, char *port, char *dbname, char *username)
{
	FILE	   *fp;
...
#define LINELEN NAMEDATALEN*5
	...
	while (!feof(fp) && !ferror(fp))
	{
		...
		if ((t = pwdfMatchesString(t, (char*)hostname)) == NULL ||
