...
xobjects := $(patsubst %.c,%.o,$(wildcard *.c))
...
# Don't rebuild the list if only the OBJS have changed
	$(if $(filter-out $(OBJS),$?),( $(if $(SUBDIROBJS),cat $(SUBDIROBJS); )echo $(addprefix $(subdir)/,$(OBJS)) ) >$@,touch $@)
        #$(if $(filter-out $(OBJS),$?),( $(if $(SUBDIROBJS),cat $(SUBDIROBJS); )ar rcs libpostgres.a $(addprefix $(subdir)/,$(OBJS)) ))
...
$(SUBDIRS:%=%-recursive):
	...
	#ar rcs ./libpostgres.a $(SUBDIROBJS)
	#echo $(SUBDIROBJS)
#xobjects := $(patsubst %.c,%.o,$(wildcard *.c))
