 ...
 */
static THREAD_LOCAL SlruCtlData MultiXactOffsetCtlData;
static THREAD_LOCAL SlruCtlData MultiXactMemberCtlData;
...
static THREAD_LOCAL MultiXactStateData *MultiXactState;
static THREAD_LOCAL MultiXactId *OldestMemberMXactId;
static THREAD_LOCAL MultiXactId *OldestVisibleMXactId;
...
static mXactCacheEnt THREAD_LOCAL *MXactCache = NULL;
#else
#define MXactCache (CurrentTransactionState->mTopLevelState->mXactCache)
#endif

#ifndef FOUNDER_XDB_SE
static MemoryContext THREAD_LOCAL MXactContext = NULL;
#else
#define MXactContext (CurrentTransactionState->mTopLevelState->mXactContext)
#endif
...
MultiXactShmemInit(void)
{
	...
	SimpleLruInit(MultiXactOffsetCtl,
				  ...
				  MultiXactOffsetControlLock, "fxdb_multixact/offsets");
	SimpleLruInit(MultiXactMemberCtl,
				  "MultiXactMember Ctl", NUM_MXACTMEMBER_BUFFERS, 0,
				  MultiXactMemberControlLock, "fxdb_multixact/members");

	/* Initialize our shared state struct */
#ifndef FOUNDER_XDB_SE
	MultiXactState = ShmemInitStruct("Shared MultiXact State",
#else
	MultiXactState = (MultiXactStateData *)ShmemInitStruct("Shared MultiXact State",
...
multixact_redo(XLogRecPtr lsn, XLogRecord *record)
{
	...
	else if (info == XLOG_MULTIXACT_CREATE_ID)
	{
		...
		ereport(PANIC,
                ...
                errmsg("multixact_redo: unknown op code %u", info)));
}
