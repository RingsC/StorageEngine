...
/*
build the btree for cluster index. in this case we have no heap, 
so no preexisting key-value pairs, only initialize the metadata page.
*/
Datum
btbuildcluster(PG_FUNCTION_ARGS)
{
    ...
    Assert(heap == 0);// we are building a cluster index, heap must be 0.
	wstate.index = index;
 ...
 */
BTSpool *
_bt_spoolinit_kb(Relation index, bool isunique, int btKbyte)
{
...
_bt_leafbuild(BTSpool *btspool, BTSpool *btspool2)
{
	...
	ereport(LOG,(errmsg("   index build sort begin on index %d",btspool->index->rd_id)));
	tuplesort_performsort(btspool->sortstate);
	if (btspool2)
		tuplesort_performsort(btspool2->sortstate);
	//ereport(LOG,(errmsg("   index build sort on index %d end",btspool->index->rd_id)));
	...
	wstate.btws_pages_written = 0;
	wstate.btws_zeropage = NULL;	/* until needed */
	ereport(LOG,(errmsg("   NBtree build begin on index %d",btspool->index->rd_id)));
	_bt_load(&wstate, btspool, btspool2);
	//ereport(LOG,(errmsg("   NBtree build on index %d end",btspool->index->rd_id)));
}
...
_bt_blwritepage(BTWriteState *wstate, Page page, BlockNumber blkno)
{
	...
	while (blkno > wstate->btws_pages_written)
	{
		...
		smgrextend(wstate->index->rd_smgr, MAIN_FORKNUM,
				   ...
				   true, BlockNumberThresholdByRel(wstate->index));
	}
	...
	if (blkno == wstate->btws_pages_written)
	{
		...
		smgrextend(wstate->index->rd_smgr, MAIN_FORKNUM, blkno,
				   (char *) page, true, BlockNumberThresholdByRel(wstate->index));
		wstate->btws_pages_written++;
	}
	else
	{
			   ...
			   OffsetNumber itup_off)
{
	...
	if (PageAddItem(page, (Item) itup, itemsize, itup_off,
		...
		ereport(ERROR,
                (errcode(ERRCODE_BUF_PAGE_ADD_ITEM_ERROR),
		...
		for (;;)
		{
			...
			else if (itup != NULL)
			{
				for (i = 1; i <= keysz; i++)
				{
					...
					char *pstr1 = VARDATA_ANY(DatumGetCString(attrDatum1));
				    int len1 = VARSIZE_ANY_EXHDR(DatumGetCString(attrDatum1));
					char *pstr2 = VARDATA_ANY(DatumGetCString(attrDatum2));
				    int len2 = VARSIZE_ANY_EXHDR(DatumGetCString(attrDatum2));
					#endif
					...
					else
					{
						...
						#else
						RangeData range1 = {0};
						range1.userData = btspool->index->user_data;
					    btspool->index->rd_colinfo->split_function(range1, pstr1, entry->sk_attno, len1);
					    RangeData range2 = {0};
						range2.userData = btspool2->index->user_data;
					    btspool2->index->rd_colinfo->split_function(range2, pstr2, entry->sk_attno, len2);
						
						char *pstr3 = &pstr1[range1.start];
						int len1 = (int)range1.len;
				        char *pstr4 = &pstr2[range2.start];
						int len2 = (int)range2.len;
						compare = entry->sk_compfun(pstr3, len1, pstr4, len2);
						#endif
