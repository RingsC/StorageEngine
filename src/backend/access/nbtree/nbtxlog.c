...
static THREAD_LOCAL List *incomplete_actions;
	...
	for (; from < end;)
	{
		...
		if (PageAddItem(page, (Item) from, itemsz, FirstOffsetNumber,
			...
			ereport(PANIC,
                    ...
                    errmsg("_bt_restore_page: cannot add item to page")));
		from += itemsz;
	}
}
				 ...
				 BlockNumber fastroot, uint32 fastlevel)
{
	 ...
	 */
	((PageHeader) metapg)->pd_lower =(LocationIndex)(
		...
		if (BufferIsValid(buffer))
		{
			...
			else
			{
				if (PageAddItem(page, (Item) datapos, datalen,
					...
					ereport(PANIC,
                            (errcode(ERRCODE_NBTREE_ERROR),
                            errmsg("btree_insert_redo: failed to add item")));
				 ...
				 XLogRecPtr lsn, XLogRecord *record)
{
	...
	if (onleft && !(record->xl_info & XLR_BKP_BLOCK_1))
	{
		...
		datapos += (int)newitemsz;
		datalen -= (int)newitemsz;
	}
			...
			if (!XLByteLE(lsn, PageGetLSN(lpage)))
			{
				...
				if (PageAddItem(lpage, left_hikey, left_hikeysz,
					...
					ereport(PANIC,
                            ...
                            errmsg("failed to add high key to left page after split")));
...
btree_xlog_vacuum(XLogRecPtr lsn, XLogRecord *record)
{
	...
	if (record->xl_len > SizeOfBtreeVacuum)
	{
		...
		if ((unend - unused) > 0)
			PageIndexMultiDelete(page, unused, (int)(unend - unused));
	}
...
btree_redo(XLogRecPtr lsn, XLogRecord *record)
{
	...
	switch (info)
	{
		...
		default:
			ereport(PANIC,
                    (errcode(ERRCODE_NBTREE_ERROR),
                    errmsg("btree_redo: unknown op code %u", info)));
	}
}
	...
	foreach(l, incomplete_actions)
	{
		...
		if (action->is_split)
		{
			...
			if (!BufferIsValid(lbuf))
				ereport(PANIC,
                        (errcode(ERRCODE_NBTREE_ERROR),
                        errmsg("btree_xlog_cleanup: left block unfound")));
			...
			if (!BufferIsValid(rbuf))
				ereport(PANIC,
                        (errcode(ERRCODE_NBTREE_ERROR),
                        errmsg("btree_xlog_cleanup: right block unfound")));
			rpage = (Page) BufferGetPage(rbuf);
		...
		else
		{
			...
			if (BufferIsValid(buf))
			{
				...
				if (_bt_pagedel(reln, buf, NULL) == 0)
					ereport(PANIC,
                            (errcode(ERRCODE_NBTREE_ERROR),
                            errmsg("btree_xlog_cleanup: _bt_pagedel failed")));
				FreeFakeRelcacheEntry(reln);
			}
		}
	}
