...
fsm_set_avail(Page page, int slot, uint8 value)
{
	...
	Assert(slot < (int)LeafNodesPerPage);
...
restart:
	...
	if (target < 0 || target >= (int)LeafNodesPerPage)
		target = 0;
	...
	while (nodeno < NonLeafNodesPerPage)
	{
		...
		if (childnodeno < (int)NodesPerPage &&
...
fsm_truncate_avail(Page page, int nslots)
{
	...
	Assert(nslots >= 0 && nslots < (int)LeafNodesPerPage);
