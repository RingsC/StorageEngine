...
static THREAD_LOCAL const FSMAddress FSM_ROOT_ADDRESS = {FSM_ROOT_LEVEL, 0};
...
fsm_space_needed_to_cat(Size needed)
{
	...
	if (needed > MaxFSMRequestSize)
		ereport(ERROR,
                ...
                errmsg( "invalid FSM request size %lu",
			                 (unsigned long) needed)));

	if (needed == 0)
		return 1;

	cat = (int)((needed + FSM_CAT_STEP - 1) / FSM_CAT_STEP);
...
fsm_extend(Relation rel, BlockNumber fsm_nblocks)
{
	...
	while (fsm_nblocks_now < fsm_nblocks)
	{
		smgrextend(rel->rd_smgr, FSM_FORKNUM, fsm_nblocks_now,
				   (char *) pg, false, BlockNumberThresholdByRel(rel));
		fsm_nblocks_now++;
	}
...
fsm_vacuum_page(Relation rel, FSMAddress addr, bool *eof_p)
{
	...
	if (addr.level > FSM_BOTTOM_LEVEL)
	{
		...
		for (slot = 0; slot < (int)SlotsPerFSMPage; slot++)
		{
			int			child_avail;
