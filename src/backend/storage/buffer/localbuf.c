...
static THREAD_LOCAL LocalBufferInfo DefaultLocalBuffer;
THREAD_LOCAL int num_temp_buffers = 1024;

#define nextFreeLocalBuf  (CurrentTopLevelState->pLocalBufInfo->nNextFreeLocalBuf)
#define LocalBufHash  (CurrentTopLevelState->pLocalBufInfo->pLocalBufHash)
#define cur_block  (CurrentTopLevelState->pLocalBufInfo->pCur_block)
#define next_buf_in_block  (CurrentTopLevelState->pLocalBufInfo->n_next_buf_in_block)
#define num_bufs_in_block  (CurrentTopLevelState->pLocalBufInfo->n_num_bufs_in_block)
#define total_bufs_allocated (CurrentTopLevelState->pLocalBufInfo->n_total_bufs_allocated)
#define LocalBufferContext  (CurrentTopLevelState->pLocalBufInfo->mLocalBufferContext)
...
#else
		oreln = smgropen(bufHdr->tag.rnode, CurrentTopLevelState->xid);
#endif //FOUNDER_XDB_SE
	...
	if (bufHdr->flags & BM_TAG_VALID)
	{
		...
		if (!hresult)			/* shouldn't happen */
			ereport(ERROR,
                    ...
                    errmsg("local buffer hash table corrupted")));
			...
			bufHdr->tag.blockNum >= firstDelBlock)
		{
			if (LocalRefCount[i] != 0)
				ereport(ERROR,
                        ...
                        errmsg( "block %u of %s is still referenced (local %u)",
					                         bufHdr->tag.blockNum,
...
InitLocalBuffers(void)
{
	...
	LocalBufferDescriptors = (BufferDesc *) palloc0(nbufs * sizeof(BufferDesc));
	LocalBufferBlockPointers = (Block *) palloc0(nbufs * sizeof(Block));
	LocalRefCount = (int32 *) palloc0(nbufs * sizeof(int32));
#else
...
GetLocalBufferStorage(void)
{
	...
	if (next_buf_in_block >= num_bufs_in_block)
	{
		...
		num_bufs = Min(num_bufs, (int)(MaxAllocSize / BLCKSZ));
...
void AtStart_LocalBuffer( void )
{
	CurrentTopLevelState->pLocalBufInfo = (LocalBufferInfo *)MemoryContextAllocZero(TopTransactionContext
		,sizeof(LocalBufferInfo));
}

LocalBufferInfo* GetDefaultLocalBufferInfo( void )
{
     return &DefaultLocalBuffer;
}
/*
