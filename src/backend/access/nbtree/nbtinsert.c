...
IndexTuple CopyIndexTuple4Promote(IndexTuple source);
			 ...
			 IndexUniqueCheck checkUnique, Relation heapRel)
{
	...
	int			natts = (int)rel->rd_colinfo->keys;
	...
	itup_scankey = _bt_mkscankey(rel, itup,&keynum);
...
top:
	...
	if (checkUnique != UNIQUE_CHECK_NO)
	{
		...
		if (IndexTupleHasData(itup)) {// cluster index doesn't have primary heap table, any key duplication is non-unique.
		    // compare with entry at  [offset] (offnum start from 1) to see if equal, in order to check unqiueness  
		    if (PageGetMaxOffsetNumber(BufferGetPage(buf)) >= offset && !_bt_compare(rel, natts, itup_scankey, BufferGetPage(buf), offset))
                ereport(ERROR, // the key to insert is identical to key at offset.
			    (errcode(ERRCODE_UNIQUE_VIOLATION),
			     errmsg("duplicate key value violates unique constraint \"%u\"",
					    RelationGetRelid(rel)),
			     errdetail("Key to be inserted already exists."))); 
		} else
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		 ...
		 */
		if (!IndexTupleHasData(itup))// cluster index are temp tables, no need for serializability checks.
	...
	/* be tidy */
	_bt_freestack(stack);
	_bt_freeskeys(itup_scankey, keynum);
	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			...
			if (!ItemIdIsDead(curitemid))
			{
				...
				itup_scankey->sk_split = rel->rd_colinfo->split_function;
#endif //FOUNDER_XDB_SE
			   ...
			   bool split_only_page)
{
	...
	if (PageGetFreeSpace(page) < itemsz)
	{
		bool		is_root = P_ISROOT(lpageop) != 0;
		  ...
		  bool newitemonleft)
{
	...
	if (!P_RIGHTMOST(oopaque))
	{
						...
						false, false) == InvalidOffsetNumber)
		{
			...
			ereport(ERROR,
                    ...
                    errmsg( "failed to add hikey to the right sibling"
					...
					false, false) == InvalidOffsetNumber)
	{
		...
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                errmsg( "failed to add hikey to the left sibling"
			                 " while splitting block %u of index \"%s\"",
			                 origpagenumber, RelationGetRelationName(rel))));
	}
	...
	for (i = P_FIRSTDATAKEY(oopaque); i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		if (i == newitemoff)
		{
			if (newitemonleft)
			{
				if (!_bt_pgaddtup(leftpage, newitemsz, newitem, leftoff))
				{
					...
					ereport(ERROR,
                            (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                            errmsg( "failed to add new item to the left sibling"
						                             " while splitting block %u of index \"%s\"",
						                             origpagenumber, RelationGetRelationName(rel))));
				}
				leftoff = OffsetNumberNext(leftoff);
			...
			else
			{
				if (!_bt_pgaddtup(rightpage, newitemsz, newitem, rightoff))
				{
					...
					ereport(ERROR,
                            (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                            errmsg( "failed to add new item to the right sibling"
						                             " while splitting block %u of index \"%s\"",
						                             origpagenumber, RelationGetRelationName(rel))));
				}
				rightoff = OffsetNumberNext(rightoff);
		...
		if (i < firstright)
		{
			if (!_bt_pgaddtup(leftpage, itemsz, item, leftoff))
			{
				...
				ereport(ERROR,
                        (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg( "failed to add old item to the left sibling"
					                         " while splitting block %u of index \"%s\"",
					                         origpagenumber, RelationGetRelationName(rel))));
			}
			leftoff = OffsetNumberNext(leftoff);
		...
		else
		{
			if (!_bt_pgaddtup(rightpage, itemsz, item, rightoff))
			{
				...
				ereport(ERROR,
                        (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg( "failed to add old item to the right sibling"
					                         " while splitting block %u of index \"%s\"",
					                         origpagenumber, RelationGetRelationName(rel))));
			}
			rightoff = OffsetNumberNext(rightoff);
				  ...
				  bool is_only)
{
	...
	else
	{
		...
		new_item = CopyIndexTuple4Promote(ritem);
...
_bt_newroot(Relation rel, Buffer lbuf, Buffer rbuf)
{
	...
	new_item->t_info = (unsigned short)itemsz;
	...
	if (PageAddItem(rootpage, (Item) new_item, itemsz, P_HIKEY,
		...
		ereport(PANIC,
                (errcode(ERRCODE_NBTREE_ERROR),
                 errmsg("failed to add leftkey to new root page"
                 " while splitting block %u of index \"%s\"",
	...
	new_item = CopyIndexTuple4Promote(item);
	...
	if (PageAddItem(rootpage, (Item) new_item, itemsz, P_FIRSTKEY,
		...
		ereport(PANIC,
                (errcode(ERRCODE_NBTREE_ERROR),
                 errmsg("failed to add rightkey to new root page"
                 " while splitting block %u of index \"%s\"",
                 BufferGetBlockNumber(lbuf), RelationGetRelationName(rel))));
	pfree(new_item);
			...
			int keysz, ScanKey scankey)
{
    ...
    RangeData range = {0};
    scankey->sk_split(range, pstr0, scankey->sk_attno, len0);
	char *pstr1 = &pstr0[range.start];
	int len1 = (int)range.len;
	char*pstr2 = (char*)scankey->sk_address;
	int len2 = scankey->sk_arglen;
#endif //FOUNDER_XDB_SE
#ifndef FOUNDER_XDB_SE
	...
	result = scankey->sk_compfun(pstr1, len1, pstr2, len2);
