 ...
 */
THREAD_LOCAL int			max_files_per_process = 2400;
...
static THREAD_LOCAL bool have_pending_fd_cleanup = false;
/*
 * Flag to tell whether it's worth scanning VfdCache looking for temp files to
 * close
 */
static THREAD_LOCAL bool have_xact_temporary_files = false;
 ...
 */
static THREAD_LOCAL Vfd *VfdCache;
static THREAD_LOCAL Size SizeVfdCache = 0;

/*
 * Number of file descriptors known to be in use by VFD entries.
 */
static THREAD_LOCAL int	nfile = 0;
/*
...
static THREAD_LOCAL int	numAllocatedDescs = 0;
static THREAD_LOCAL AllocateDesc allocatedDescs[MAX_ALLOCATED_DESCS];
/*
 * Number of temporary files opened during the current session;
 * this is used in generation of tempfile names.
 */
static THREAD_LOCAL long tempFileCounter = 0;

/*
 * Array of OIDs of temp tablespaces.  When numTempTableSpaces is -1,
 * this has not been set in the current transaction.
 */
static THREAD_LOCAL Oid *tempTableSpaces = NULL;
static THREAD_LOCAL int	numTempTableSpaces = -1;
static THREAD_LOCAL int	nextTempTableSpace = 0;
...
InitFileAccess(void)
{
	...
	VfdCache = (Vfd *) palloc0(sizeof(Vfd));
...
count_usable_fds(int max_to_probe, int *usable_fds, int *already_open)
{
	...
	for (;;)
	{
		 ...
		 * some platforms
		 */
		if (getrlimit_status == 0 && highestfd >= (int)rlim.rlim_cur - 1)
			break;
#endif
...
#ifdef FOUNDER_XDB_SE
void set_thread_max_safe_fds(const uint32 thread_num)
{
	extern int autovacuum_max_workers;

	Assert(thread_num > 0);
	max_safe_fds -= thread_num;/* For xlog */
	max_safe_fds /= (thread_num + autovacuum_max_workers + 1);/* For thread and vacuum*/
}
#endif //FOUNDER_XDB_SE
 ...
 */
int
BasicOpenFile(const FileName fileName, int fileFlags, int fileMode)
{
	int			fd;
...
#else
		newVfdCache = (Vfd *) repalloc(VfdCache, sizeof(Vfd) * newCacheSize);
#endif //FOUNDER_XDB_SE
		if (newVfdCache == NULL)
			ereport(ERROR,
					(errcode(ERRCODE_OUT_OF_MEMORY),
...
PathNameOpenFile(FileName fileName, int fileFlags, int fileMode)
{
	...
	MemoryContext oldcxt = MemoryContextSwitchTo(TopMemoryContext);
	fnamecopy = (char*)pstrdup(fileName);
	MemoryContextSwitchTo(oldcxt);
#endif //FOUNDER_XDB_SE
...
OpenTemporaryFileInTablespace(Oid tblspcOid, bool rejectError)
{
	...
	else
	{
		...
		snprintf(tempdirpath, sizeof(tempdirpath), "fxdb_tblspc/%u/%s/%s",
	...
	snprintf(tempfilepath, sizeof(tempfilepath), "%s/%s%d.%ld",
			 tempdirpath, PG_TEMP_FILE_PREFIX, (int)MyProcPid, tempFileCounter++);
		...
		switch (whence)
		{
			case SEEK_SET:
				if (offset < 0)
					ereport(ERROR,
                            (errcode(ERRCODE_OUT_OF_MEMORY),
                            errmsg( "invalid seek offset: " INT64_FORMAT,
						                             (int64) offset)));
...
AllocateFile(const char *name, const char *mode)
{
	...
	if (numAllocatedDescs >= MAX_ALLOCATED_DESCS ||
		...
		ereport(ERROR,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg( "exceeded MAX_ALLOCATED_DESCS while trying to open file \"%s\"",
			                 name)));
...
TryAgain:
	if ((file = fopen(name, mode)) != NULL)
	{
		...
		desc->desc.file = file;
		desc->create_subid = NULL == MyProc ? InvalidSubTransactionId : GetCurrentSubTransactionId();
		numAllocatedDescs++;
...
FreeDesc(AllocateDesc *desc)
{
	...
	switch (desc->kind)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("AllocateDesc kind not recognized")));
...
AllocateDir(const char *dirname)
{
	...
	if (numAllocatedDescs >= MAX_ALLOCATED_DESCS ||
		...
		ereport(ERROR,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg( "exceeded MAX_ALLOCATED_DESCS while trying to open directory \"%s\"",
			                 dirname)));
				  ...
				  SubTransactionId parentSubid)
{
	...
	for (i = 0; i < (Index)numAllocatedDescs; i++)
	{
...
RemovePgTempFiles(void)
{
	 ...
	 */
	spc_dir = AllocateDir("fxdb_tblspc");

	while ((spc_de = ReadDir(spc_dir, "fxdb_tblspc")) != NULL)
	{
		if (strcmp(spc_de->d_name, ".") == 0 ||
			strcmp(spc_de->d_name, "..") == 0)
			continue;

		snprintf(temp_path, sizeof(temp_path), "fxdb_tblspc/%s/%s/%s",
			spc_de->d_name, TABLESPACE_VERSION_DIRECTORY, PG_TEMP_FILES_DIR);
		RemovePgTempFilesInDir(temp_path);

		snprintf(temp_path, sizeof(temp_path), "fxdb_tblspc/%s/%s",
