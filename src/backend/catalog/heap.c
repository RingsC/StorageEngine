				   ...
				   Datum reloptions)
{
	...
	values[Anum_pg_class_relkind - 1] = CharGetDatum(rel_kind);
...
heap_drop_with_catalog(Oid relid)
{
	...
	if (rel->rel_kind != RELKIND_VIEW &&
		rel->rel_kind != RELKIND_COMPOSITE_TYPE &&
...
void RelationTruncateIndex(Relation heapRelation, Oid indexId)
{
	...
	if(currentIndex->rd_amcache != NULL){
		pfree(currentIndex->rd_amcache);
		currentIndex->rd_amcache = NULL;
	}
...
RelationTruncateIndexes(Relation heapRelation)
{
	...
	foreach(indlist, RelationGetIndexList(heapRelation))
	{
		...
		RelationTruncateIndex(heapRelation, indexId);
	}
}
//FOUNDER_XDB_SE
/*
...
heap_truncate_one_rel(Relation rel)
{
...
#ifdef FOUNDER_XDB_SE
	extern void CheckStandbyUpdateError(const char *msg);

	if(!RelationIsTemp(rel))
		CheckStandbyUpdateError("Can not execute truncate table during recovery.");
#endif //FOUNDER_XDB_SE
	...
	toastrelid = get_toast_table_id(rel->rd_id);
#endif
	if (OidIsValid(toastrelid))
	{
		Relation	toastrel = heap_open(toastrelid, AccessExclusiveLock,rel->rd_node.dbNode);
