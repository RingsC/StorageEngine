...
perform_base_backup(basebackup_options *opt, DIR *tblspcdir)
{
	...
	PG_ENSURE_ERROR_CLEANUP(base_backup_cleanup, (Datum) 0);
	{
		...
		while ((de = ReadDir(tblspcdir, "fxdb_tblspc")) != NULL)
		{
			...
			snprintf(fullpath, sizeof(fullpath), "fxdb_tblspc/%s", de->d_name);
			...
			ti->size = opt->progress ? sendDir(linkpath, (int)strlen(linkpath), true) : -1;
			tablespaces = lappend(tablespaces, ti);
		}

		/* Add a node for the base directory at the end */
		ti = (tablespaceinfo *)palloc0(sizeof(tablespaceinfo));
		ti->size = opt->progress ? sendDir((char*)".", 1, true) : -1;
		tablespaces = lappend(tablespaces, ti);
		...
		foreach(lc, tablespaces)
		{
			...
			if (ti->path == NULL)
				sendFileWithContent(BACKUP_LABEL_FILE, labelfile);
                        char *temp = (char*)".";
			sendDir(ti->path == NULL ? temp : ti->path,
					ti->path == NULL ? 1 : (int)strlen(ti->path),
					false);
		...
		while (true)
		{
			...
			for (i = 0; i < XLogSegSize / TAR_SEND_SIZE; i++)
			{
				...
				if (pq_putmessage('d', buf, TAR_SEND_SIZE))
					ereport(ERROR,
                            (errcode(ERRCODE_PUT_MSG_FAILURE),
...
SendBaseBackup(BaseBackupCmd *cmd)
{
	...
	dir = AllocateDir("fxdb_tblspc");
	if (!dir)
		ereport(ERROR,
                (errcode(errcode_for_file_access()),
                errmsg("could not open directory \"fxdb_tblspc\": %m")));
...
send_int8_string(StringInfoData *buf, int64 intval)
{
	...
	pq_sendint(buf, (int)strlen(is), 4);
	pq_sendbytes(buf, is, (int)strlen(is));
}
...
SendBackupHeader(List *tablespaces)
{
	...
	foreach(lc, tablespaces)
	{
		...
		{
			pq_sendint(&buf, (int)strlen(ti->oid), 4);		/* length */
			pq_sendbytes(&buf, ti->oid, (int)strlen(ti->oid));
			pq_sendint(&buf, (int)strlen(ti->path), 4);		/* length */
			pq_sendbytes(&buf, ti->path, (int)strlen(ti->path));
		}
		if (ti->size >= 0)
...
SendXlogRecPtrResult(XLogRecPtr ptr)
{
	...
	pq_sendint(&buf, (int)strlen(str), 4);	/* length */
	pq_sendbytes(&buf, str, (int)strlen(str));
	pq_endmessage(&buf);
...
#ifndef WIN32
		if (S_ISLNK(statbuf.st_mode) && strcmp(path, "./fxdb_tblspc") == 0)
#else
		if (pgwin32_is_junction(pathbuf) && strcmp(path, "./fxdb_tblspc") == 0)
#endif
		{
...
sendFile(char *readfilename, char *tarfilename, struct stat * statbuf)
{
	...
	while ((cnt = fread(buf, 1, Min(sizeof(buf), (size_t)(statbuf->st_size - len)), fp)) > 0)
	{
	...
	if (len < statbuf->st_size)
	{
		...
		{
			cnt = Min(sizeof(buf), (size_t)(statbuf->st_size - len));
