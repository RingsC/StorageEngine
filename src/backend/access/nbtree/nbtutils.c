...
static inline bool _bt_test(char *str1, int len1, char *str2, int len2, CompareCallback compareFunction, StrategyNumber strategyNum);
...
#else
ScanKey
_bt_mkscankey(Relation rel, IndexTuple itup,int* keynum)
{
	ScanKey		skey;
	TupleDesc	itupdesc;
	int			natts;
	...
	itupdesc = RelationGetDescr(rel);
	*keynum = (int)rel->rd_colinfo->keys;
	natts = (int)rel->rd_colinfo->keys;
	//indoption = rel->rd_indoption;
	Datum		datum;
	bool		null;
	...
	skey = (ScanKey) palloc(natts * sizeof(ScanKeyData));
	char *pstr0 = VARDATA_ANY(DatumGetCString(datum));
	int len0 = VARSIZE_ANY_EXHDR(DatumGetCString(datum));
	...
	for (i = 0; i < natts; i++)
	{
		...
		//int			flags;
        RangeData range = {0};
		range.userData = rel->user_data;
        rel->rd_colinfo->split_function(range, pstr0, i + 1, len0);
		Datum datum2 = fdxdb_string_formdatum(&pstr0[range.start], range.len);
		...
		//flags = (null ? SK_ISNULL : 0) | (indoption[i] << SK_BT_INDOPTION_SHIFT);
		Fdxdb_ScanKeyInitWithCallbackInfo(&skey[i],
			                              i + 1,
										  InvalidStrategy,
										  rel->rd_colinfo->rd_comfunction[i],
										  datum2);
		skey[i].sk_split = rel->rd_colinfo->split_function;
	}
...
void
_bt_freeskeys(ScanKey skey,int keynum)
{
	freeScanKeys(skey,keynum);
}
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (cur->sk_attno < 1)
		ereport(ERROR,
                ...
                errmsg("btree index keys must be ordered by attribute")));

	/* We can short-circuit most of the work if there's just one key */
	if (numberOfKeys == 1)
	{
		/* Apply indoption to scankey (might change sk_strategy!) */
		if (!_bt_fix_scankey_strategy(cur, NULL))
			so->qual_ok = false;
...
_bt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
						 ...
						 bool *result)
{	
	bool		leftnull= NULL == leftarg,
		rightnull = NULL == rightarg;
	...
	if(leftnull || rightnull)
	{
		...
		* logic.  If the index is NULLS FIRST, we need to flip the strategy.
		*/
		StrategyNumber strat = op->sk_strategy;
		if (op->sk_flags & SK_BT_NULLS_FIRST)
			strat = BTCommuteStrategyNumber(strat);
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("unrecognized StrategyNumber: %d", (int) strat)));
			*result = false;	/* keep compiler quiet */
			break;
		}
		return true;
	...
	{
		if ((leftarg->sk_attno == leftarg->sk_attno) && (leftarg->sk_strategy == rightarg->sk_strategy))
		{
			//int compareResult = leftarg->sk_compfun(leftarg->sk_argument,rightarg->sk_argument);
			return true;
		}
		else
		{
	...
	switch (skey->sk_strategy)
	{
		...
		default:
			ereport(ERROR,
                    ...
                    errmsg( "unrecognized StrategyNumber: %d",
			  ...
			  ScanDirection dir, bool *continuescan)
{
	...
	int			ikey;
	ScanKey		key;
    bool        isClusterScan = IS_CLUSTER_INDEX(scan->indexRelation->mt_info.type);
    char        *idxdata = 0;
	...
	*continuescan = true;		/* default assumption */
...
#else
        RangeData range = {0};
		range.userData = scan->indexRelation->user_data;
        scan->indexRelation->rd_colinfo->split_function(range, pstr0, key->sk_attno, len0);
		char *pstr1 = &pstr0[range.start];
        int len1 = (int)range.len;
		char*pstr2 = (char*)key->sk_address;
		int len2 = key->sk_arglen;
		result = _bt_test(pstr1, len1, pstr2, len2, key->sk_compfun, key->sk_strategy);
#endif //FOUNDER_XDB_SE
#ifndef FOUNDER_XDB_SE
    ...
    /*
        we have to do index key filtering after above qualification tests because we need to decide
        whether to continue scanning as above.
    */
    char *idxkeyaddr = (char *)tuple + IndexInfoFindDataOffset(tuple->t_info);
    int keylen = VARSIZE_ANY(idxkeyaddr), keylenpad = tuple->t_tid.ip_blkid.bi_lo;    
    if (scan->filter_function && scan->filter_function(scan->user_data, VARDATA_ANY(idxkeyaddr), VARSIZE_ANY_EXHDR(idxkeyaddr)) == false)
        return false;
        
	/* If we get here, the tuple passes all index quals as well as index key filtering. */
	if (tuple_valid){
	    if (!isClusterScan)
		    scan->xs_ctup.t_self = tuple->t_tid;
		else {
		    /* this is a cluster index scan, put index data into datumBlock and update offsets array. 
		    if (so->currPos.datumBlock == NULL) {
		        so->currPos.datumBlock = (char *)palloc0(BLCKSZ);
		        so->currPos.offsets[MaxIndexTuplesPerPage] = 0;
		    }*/
		    int oldcnt = so->currPos.offsets[MaxIndexTuplesPerPage];
		    int oldlen = (oldcnt == 0 ? 0 : so->currPos.offsets[oldcnt - 1]);
		    
		    char *idxdataaddr = (char *)tuple + tuple->t_tid.ip_blkid.bi_lo;
		    int datalen = VARSIZE_ANY(idxdataaddr);
		    so->currPos.offsets[oldcnt] = oldlen + datalen + keylen;
		    memcpy(so->currPos.datumBlock + oldlen, idxkeyaddr, keylen);
		    memcpy(so->currPos.datumBlock + oldlen + keylen, idxdataaddr, datalen);
		    so->currPos.offsets[MaxIndexTuplesPerPage]++;
		}
    }
	return tuple_valid;
}
					 ...
					 ScanDirection dir, bool *continuescan)
{
	...
	switch (subkey->sk_strategy)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_UNDEFINED_PARAMETER),
                    errmsg( "unrecognized RowCompareType: %d",
...
static THREAD_LOCAL BTVacInfo *btvacinfo;
	...
	for (i = 0; i < btvacinfo->num_vacuums; i++)
	{
			...
			vac->relid.dbId == rel->rd_lockInfo.lockRelId.dbId)
		{
			...
			ereport(ERROR,
                    ...
                    errmsg( "multiple active vacuums for index \"%s\"",
...
inline bool _bt_test(char *str1, int len1, char *str2, int len2, CompareCallback compareFunction, StrategyNumber strategyNum)
{
