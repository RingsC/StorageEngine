 ...
 */
THREAD_LOCAL bool		Logging_collector = false;
THREAD_LOCAL int			Log_RotationAge = HOURS_PER_DAY * MINS_PER_HOUR;
THREAD_LOCAL int			Log_RotationSize = 10 * 1024;
THREAD_LOCAL char	   *Log_directory = NULL;
THREAD_LOCAL char	   *Log_filename = NULL;
THREAD_LOCAL bool		Log_truncate_on_rotation = false;
THREAD_LOCAL int			Log_file_mode = S_IRUSR | S_IWUSR;

/*
 * Globally visible state (used by elog.c)
 */
THREAD_LOCAL bool		am_syslogger = false;
 ...
 */
static volatile THREAD_LOCAL sig_atomic_t got_SIGHUP = false;
static volatile THREAD_LOCAL sig_atomic_t rotation_requested = false;
...
SysLoggerMain(int argc, char *argv[])
{
	...
	pg_signal(SIGHUP, sigHupHandler);	/* set flag to read config file */
	...
	pg_signal(SIGUSR1, sigUsr1Handler);	/* request log rotation */
...
//	PG_SETMASK(&UnBlockSig);
	...
	if (threadHandle == 0)
		ereport(FATAL,
                ...
                errmsg("could not create syslogger data transfer thread: %m")));
#endif   /* WIN32 */
...
write_syslogger_file(const char *buffer, int count, int destination)
{
	...
	rc = (int)fwrite(buffer, 1, count, logfile);
