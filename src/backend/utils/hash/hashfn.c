...
uint32
uint32_hash_notaddress(const void *key, Size keysize)
{
	Assert(keysize == sizeof(uint32));
	return DatumGetUInt32(hash_uint32((uint32)(uint64)key));
}

uint32
uint64_hash(const void *key, Size keysize)
{
	Assert(keysize == sizeof(uint64));
	//verify modify is ok --sunwf
	int64 val = *(int64*)key;
	uint32		lohalf = (uint32) val;
	uint32		hihalf = (uint32) (val >> 32);
	lohalf ^= (val >= 0) ? hihalf : ~hihalf;
	return (uint32)hash_uint32(lohalf);	
}
/*
