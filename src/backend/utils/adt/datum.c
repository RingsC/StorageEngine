...
datumGetSize(Datum value, bool typByVal, int typLen)
{
	...
	if (typByVal)
	{
		...
		Assert(typLen > 0 && typLen <= (int)sizeof(Datum));
		size = (Size) typLen;
	}
	else
	{
		...
		else
		{
			ereport(ERROR,
                    ...
                    errmsg("invalid typLen: %d", typLen)));
...
datumIsEqual(Datum value1, Datum value2, bool typByVal, int typLen)
{
	 ...
	 */
	size1 = VARSIZE_ANY_EXHDR(DatumGetPointer(value1));
	size2 = VARSIZE_ANY_EXHDR(DatumGetPointer(value2));
	if (size1 != size2)
		return false;
	s1 = VARDATA_ANY(DatumGetPointer(value1));
	s2 = VARDATA_ANY(DatumGetPointer(value2));
