 ...
 */
#define CurrentSnapshotData (CurrentTransactionState->mTopLevelState->mCurrentSnapshotData)
#define SecondarySnapshotData (CurrentTransactionState->mTopLevelState->mSecondarySnapshotData)

/* Pointers to valid snapshots */
#define CurrentSnapshot (CurrentTransactionState->mTopLevelState->mCurrentSnapshot)
#define SecondarySnapshot (CurrentTransactionState->mTopLevelState->mSecondarySnapshot)
...
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL ActiveSnapshotElt *ActiveSnapshot = NULL;
#else
#define ActiveSnapshot (CurrentTransactionState->mTopLevelState->mActiveSnapshot)
#endif
/*
 ...
 */
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL int	RegisteredSnapshots = 0;
#else
#define RegisteredSnapshots (CurrentTransactionState->mTopLevelState->nRegisteredSnapshots)
#endif
...
#else
#define FirstSnapshotSet (CurrentTransactionState->mTopLevelState->bFirstSnapshotSet)
#endif
/*
 ...
 */
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL bool registered_xact_snapshot = false;
#else
#define registered_xact_snapshot (CurrentTransactionState->mTopLevelState->bRegistered_serializable)
#endif
...
GetLatestSnapshot(void)
{
	...
	if (!FirstSnapshotSet)
		ereport(ERROR,
                ...
                errmsg("no snapshot has been set")));
...
#else
	newactive = (ActiveSnapshotElt *)MemoryContextAlloc(TopTransactionContext, sizeof(ActiveSnapshotElt));
...
#else
		MyProcTopLevel->xmin = InvalidTransactionId;
#endif
}
