...
static THREAD_LOCAL int	elevel = -1;

static THREAD_LOCAL TransactionId OldestXmin;
static THREAD_LOCAL TransactionId FreezeLimit;
...
extern HeapTuple fxdb_search_meta_table_copy(Oid relid, Snapshot snapshot);
				...
				BufferAccessStrategy bstrategy)
{
    ...
    scan_all = TransactionIdPrecedesOrEquals(onerel->rd_rel->relfrozenxid,
        freezeTableLimit);
#else
	HeapTuple tup = fxdb_search_meta_table_copy(RelationGetRelid(onerel), SnapshotNow);
	Form_meta_table formClass = fxdb_get_meta_table_struct_without_copy(tup);
	...
	pgstat_report_vacuum(RelationGetRelid(onerel),
						 ...
						 new_rel_tuples);
#else
	extern void pgstat_report_vacuum(Oid tableoid, Oid dbid, bool shared, PgStat_Counter tuples);

	pgstat_report_vacuum(RelationGetRelid(onerel), onerel->mt_info.database_id,
		false,
		(PgStat_Counter)new_rel_tuples);
#endif //FOUNDER_XDB_SE
	...
	if (IsAutoVacuumWorkerProcess() && Log_autovacuum_min_duration >= 0)
	{
		if (Log_autovacuum_min_duration == 0 ||
			TimestampDifferenceExceeds(starttime, GetCurrentTimestamp(),
...
#else
                                       ereport(LOG,
                                       (errmsg("automatic vacuum of table \"%s.%s\": index scans: %d\n"
                                       "pages: %d removed, %d remain\n"
                                       "tuples: %.0f removed, %.0f remain\n"
                                       "system usage: %s",
                                       get_database_name(MyDatabaseId),                                      
                                       ...
                                       vacrelstats->rel_pages,
                                       vacrelstats->tuples_deleted,
                                       vacrelstats->new_rel_tuples,"use")));
#endif
			   ...
			   Relation *Irel, int nindexes, bool scan_all)
{
    ...
    ereport(elevel,
        ...
        relname)));
#ifdef _DEBUG
    INFO_PRINTF(stdout, "vacuuming \"%s\"\n", relname);
#endif
#endif //FOUNDER_XDB_SE
	...
	*stats = index_bulk_delete(&ivinfo, *stats,
	...
	ereport(elevel,
			(errmsg("scanned index \"%s\" to remove %d row versions",
			 ...
			 errdetail("%s.", " ")));
#ifdef _DEBUG
	INFO_PRINTF(stdout, "scanned index \"%s\" to remove %d row versions. \n",
		RelationGetRelationName(indrel),
		vacrelstats->num_dead_tuples);
#endif
}
...
lazy_space_alloc(LVRelStats *vacrelstats, BlockNumber relblocks)
{
	...
	if (vacrelstats->hasindex)
	{
		...
		maxtuples = Min(maxtuples, (long)(MaxAllocSize / sizeof(ItemPointerData)));

		/* curious coding here to ensure the multiplication can't overflow */
		if ((BlockNumber) (maxtuples / LAZY_ALLOC_TUPLES) > (BlockNumber) relblocks)
