...
static
THREAD_LOCAL bool cluster_use_sort = false;
...
#else
		if (OidIsValid(indexOid) && recheck)
		{
			tuple = fxdb_search_meta_table_copy(indexOid);
			if (!tuple)
			{
				relation_close(OldHeap, AccessExclusiveLock);
				return;
...
#else
	if (OidIsValid(indexOid) && IsSystemTable(OldHeap->rd_node.relNode))
		ereport(ERROR,
		(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		errmsg("cannot cluster a systable")));
#endif //FOUNDER_XDB_SE
...
check_index_is_clusterable(Relation OldHeap, Oid indexOid, bool recheck, LOCKMODE lockmode)
{
	...
	for (unsigned int i = 0; i < MtInfo_GetIndexCount(OldHeap->mt_info); ++i)
	{
		if (MtInfo_GetIndexOid(OldHeap->mt_info)[i] == indexOid)
			isValidIndex = true;
	}
	...
	if (!isValidIndex)
	{
		ereport(ERROR, 
			(errcode(ERRCODE_WRONG_OBJECT_TYPE),
			errmsg("\"%u\" is not an index for table \"%u\"",
			indexOid,
			OldHeap->rd_node.relNode)));
	}
...
#else
	is_system_catalog = IsSystemTable(RelationGetRelid(OldHeap));
#endif //FOUNDER_XDB_SE
...
make_new_heap(Oid OIDOldHeap, Oid NewTableSpace)
{
	...
	heap_close(OldHeap, NoLock);
#else
	Relation OldHeap = heap_open(OIDOldHeap, AccessExclusiveLock);
	HeapTuple tuple = fxdb_search_meta_table_copy(OIDOldHeap, SnapshotNow);
	Form_meta_table formClass = (Form_meta_table)(VARDATA_ANY(GETSTRUCT(tuple)));
	OnCommitAction action = OldHeap->rd_rel->relpersistence == RELPERSISTENCE_TEMP ? ONCOMMIT_DROP : ONCOMMIT_NOOP;

	/*
	 * Create the new heap.
	 */
	Oid OIDNewHeap = fxdb_heap_create(InvalidOid, 
		formClass->colinfo_id, 
		formClass->tablespace_id, 
		formClass->database_id,
		OldHeap->rel_kind,
		OldHeap->rd_rel->relpersistence,
		action);
			   ...
			   bool *pSwapToastByContent, TransactionId *pFreezeXid)
{
	...
	oldTupDesc = newTupDesc = single_attibute_tupDesc;
#endif //FOUNDER_XDB_SE
...
#else
	LockRelationOid(get_toast_table_id(RelationGetRelid(OldHeap)), AccessExclusiveLock);
#endif //FOUNDER_XDB_SE
	...
	*pFreezeXid = FreezeXid;
	...
	is_system_catalog = IsSystemRelation(OldHeap);
#else
	HeapTuple tup = fxdb_search_meta_table_copy(RelationGetRelid(OldHeap));
	Form_meta_table formClass = (Form_meta_table)(VARDATA_ANY(GETSTRUCT(tup)));
	TransactionId relfrozenxid = formClass->relfrozenxid;
	pfree(tup);
...
#else
		indexScan = index_beginscan(OldHeap, OldIndex, SnapshotAny, 0, 0, NULL, NULL);
#endif //FOUNDER_XDB_SE
...
#else
		heapScan = heap_beginscan(OldHeap, SnapshotAny, 0, NULL, NULL, NULL);
#endif //FOUNDER_XDB_SE
		indexScan = NULL;
	}
...
#else
		ereport(elevel,
				(errmsg("vacuuming \"%u\"", RelationGetRelid(OldHeap))));
#endif //FOUNDER_XDB_SE
		...
		switch (HeapTupleSatisfiesVacuum(tuple->t_data, OldestXmin, buf))
		{
			...
			case HEAPTUPLE_INSERT_IN_PROGRESS:
				...
				if (!is_system_catalog &&
					...
					elog(WARNING, "concurrent insert in progress within table \"%u\"",
			...
			case HEAPTUPLE_DELETE_IN_PROGRESS:
				...
				if (!is_system_catalog &&
					...
					elog(WARNING, "concurrent delete in progress within table \"%u\"",
						 RelationGetRelid(OldHeap));
					...
					Oid *mapped_tables)
{
...
#else
	reltup1 = fxdb_search_meta_table_copy(r1);
	if (!HeapTupleIsValid(reltup1))
		elog(ERROR, "cache lookup failed for relation %u", r1);
	relform1 = (Form_meta_table) VARDATA_ANY(GETSTRUCT(reltup1));

	reltup2 = fxdb_search_meta_table_copy(r2);
	if (!HeapTupleIsValid(reltup2))
		elog(ERROR, "cache lookup failed for relation %u", r2);
	relform2 = (Form_meta_table) VARDATA_ANY(GETSTRUCT(reltup2));

	relfilenode1 = relform1->rel_id;
	relfilenode2 = relform2->rel_id;
#endif //FOUNDER_XDB_SE
...
#ifndef FOUNDER_XDB_SE
		swaptemp = relform1->rel_id;
		relform1->rel_id = relform2->rel_id;
		relform2->rel_id = swaptemp;

		swaptemp = relform1->tablespace_id;
		relform1->tablespace_id = relform2->tablespace_id;
		relform2->tablespace_id = swaptemp;
...
#else
		swaptemp = relform1->rel_filenode;
		relform1->rel_filenode = relform2->rel_filenode;
		relform2->rel_filenode = swaptemp;
	...
	else
	{
		 ...
		 */
		if (OidIsValid(relfilenode1) || OidIsValid(relfilenode2))
			elog(ERROR, "cannot swap mapped relation \"%u\" with non-mapped relation",
				 NameStr(relform1->rel_id));
...
#else
	if (relform1->rel_pid == InvalidOid && !is_toast_id(relform1->rel_id))
	{
		Assert(TransactionIdIsNormal(frozenXid));
		relform1->relfrozenxid = frozenXid;
	}
	...
	/* swap size statistics too, since new rel has freshly-updated stats */
	{
		...
		swap_tuples = (float4)(relform1->reltuples);
...
#else
	if (!IsSystemTable(r1) && !is_toast_id(r1))
#endif //FOUNDER_XDB_SE
	{
		if (swap_toast_by_content)
		{
...
#else
			/* Recursively swap the contents of the toast tables */
			swap_relation_files(get_toast_table_id(relform1->rel_id),
				get_toast_table_id(relform2->rel_id),
				target_is_pg_class,
				swap_toast_by_content,
				frozenXid,
				mapped_tables);
...
#else
			if (IsSystemTable(relform1->relfilenode))
				elog(ERROR, "cannot swap toast files by links for system catalogs");
#endif //FOUNDER_XDB_SE
...
#else
	if (swap_toast_by_content &&
		!IsSystemTable(r1) && !is_toast_id(r1))
		swap_relation_files(get_toast_index_id(relform1->rel_id),
							get_toast_index_id(relform2->rel_id),
							target_is_pg_class,
							swap_toast_by_content,
							InvalidTransactionId,
							mapped_tables);
...
#ifdef FOUNDER_XDB_SE
	HeapTuple tuple = fxdb_search_meta_table_copy(OIDOldHeap);
	Form_meta_table formClass = (Form_meta_table) VARDATA_ANY (GETSTRUCT(tuple));
	Oid dbid = formClass->database_id;
	heap_freetuple(tuple);
#endif //FOUNDER_XDB_SE
...
#else
	reindex_relation(OIDOldHeap, dbid, reindex_flags);
#endif //FOUNDER_XDB_SE
...
#ifdef FOUNDER_XDB_SE
void set_cluster_use_sort(bool use_sort)
{
	cluster_use_sort = use_sort;
}
#endif //FOUNDER_XDB_SE
