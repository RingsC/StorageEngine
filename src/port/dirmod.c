...
pgunlink(const char *path)
{
	...
	while (unlink(path))
	{
		if (errno != EACCES)
			return -1;
		if (++loops > 10)		/* time out after 10 sec */
			return -1;
		pg_sleep(100);		/* us */
	}
	return 0;
...
pgsymlink(const char *oldpath, const char *newpath)
{
	...
	len = (int)strlen(nativeTarget) * sizeof(WCHAR);
	reparseBuf->ReparseTag = IO_REPARSE_TAG_MOUNT_POINT;
	reparseBuf->ReparseDataLength = (short)len + 12;
	reparseBuf->Reserved = 0;
	reparseBuf->SubstituteNameOffset = 0;
	reparseBuf->SubstituteNameLength = (short)len;
	reparseBuf->PrintNameOffset = (short)len + sizeof(WCHAR);
