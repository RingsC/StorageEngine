 ...
 */
THREAD_LOCAL int			pgwin32_noblock = 0;
...
{
	static THREAD_LOCAL HANDLE waitevent = INVALID_HANDLE_VALUE;
	static THREAD_LOCAL SOCKET current_socket = -1;
	static THREAD_LOCAL int	isUDP = 0;
	HANDLE		events[2];
	int			r;
...
//	events[0] = pgwin32_signal_event;
	...
	if ((what & FD_WRITE) && isUDP)
	{
		for (;;)
		{
			r = WaitForMultipleObjectsEx(1, events, FALSE, 100, TRUE);
		...
		r = WaitForMultipleObjectsEx(1, events, FALSE, timeout, TRUE);
	...
	ereport(ERROR,
            ...
            errmsg_internal("unrecognized return value from WaitForMultipleObjects: %i (%i)", 
            r, (int) GetLastError())));
	return 0;
}
...
{
	static THREAD_LOCAL HANDLE handleDLL = INVALID_HANDLE_VALUE;
