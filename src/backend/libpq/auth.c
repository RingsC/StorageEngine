...
sendAuthRequest(Port *port, AuthRequest areq)
{
	...
	else if (areq == AUTH_REQ_GSS_CONT)
	{
		if (port->gss->outbuf.length > 0)
		{
			...
			pq_sendbytes(&buf, (const char *)port->gss->outbuf.value, port->gss->outbuf.length);
		}
	}
#endif
...
//		if (sspictx == NULL)
//		{
//			sspictx = (CtxtHandle *)malloc(sizeof(CtxtHandle));
//			if (sspictx == NULL)
//				ereport(ERROR,
//						(errmsg("out of memory")));
//		}
...
//							  (int) GetLastError())));
//
//	tokenuser = (TOKEN_USER *)malloc(retlen);
//	if (tokenuser == NULL)
//		ereport(ERROR,
//				(errmsg("out of memory")));
//
...
//		int			retval;
//
//		namebuf = (char *)palloc(strlen(accountname) + strlen(domainname) + 2);
...
//	 * MD5(secret + vector)
//	 */
//	cryptvector = (uint8 *)palloc(RADIUS_VECTOR_LENGTH + strlen(port->hba->radiussecret));
...
//		 */
//		cryptvector = (uint8 *)palloc(packetlength + strlen(port->hba->radiussecret));
//
