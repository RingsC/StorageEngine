...
#define RDF_ERRORDATA_SIZE 5

static RDF_THREAD_LOCAL librdf_log_message RDFErrorData[RDF_ERRORDATA_SIZE];
static RDF_THREAD_LOCAL int	RDF_ERRORDATA_DEPTH = -1; 


void SetRDFErrorData(int code, librdf_log_level level, librdf_log_facility facility,
                  		 void *locator, const char *message)
{
	librdf_log_message* pErrorData = NULL;
	if (++RDF_ERRORDATA_DEPTH >= RDF_ERRORDATA_SIZE)
	{
		RDF_ERRORDATA_DEPTH = -1;
		SetRDFErrorData(0,LIBRDF_LOG_FATAL,LIBRDF_FROM_NONE,NULL,"RDF_ERRORDATA_SIZE exceeded");
	}
	
	pErrorData = &RDFErrorData[RDF_ERRORDATA_DEPTH];
	pErrorData->code=code;
	pErrorData->level=level;
	pErrorData->facility=facility;
	char* pMessage = NULL;
	if (NULL != message)
	{
		size_t len = strlen(message);
		pMessage = (char*)LIBRDF_MALLOC(char*,len+1);
		strncpy(pMessage,message,len);
		pMessage[len] = 0;
		
	}
	pErrorData->message = pMessage;		
	pErrorData->locator=(raptor_locator*)locator;	
}

extern char* GetRDFErrorData(int& err_no )
{
	if (RDF_ERRORDATA_DEPTH < 0)
	{
		RDF_ERRORDATA_DEPTH = -1;
		return NULL;
	}
	
    librdf_log_message* edata = &RDFErrorData[RDF_ERRORDATA_DEPTH];
	char *errmsg = NULL;
    err_no = edata->code;
    ...
    size_t len  = strlen(edata->message) + sizeof(fmt) + 256;
   	errmsg = (char*)LIBRDF_MALLOC(char*,len+1);
	errmsg[len] = 0;
    sprintf(errmsg,fmt ,edata->message);
	
	if (NULL != edata->message)
		LIBRDF_FREE(char*,edata->message);
    --RDF_ERRORDATA_DEPTH;
	return errmsg;
}

bool librdf_has_exception()
{
	if (-1 == RDF_ERRORDATA_DEPTH)
		return false;
	else
		return true;
}
                  ...
                  void *locator, const char *message)
{
  ...
  fputc('\n', stderr);
#else
	if (LIBRDF_LOG_ERROR == level || LIBRDF_LOG_FATAL == level)
		SetRDFErrorData(code,level,facility,locator,message);
#endif
}
