...
#if HASH_STATISTICS
static THREAD_LOCAL long hash_accesses,
			hash_collisions,
			hash_expansions;
#endif
 ...
 */
static THREAD_LOCAL MemoryContext CurrentDynaHashCxt = NULL;
...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
	...
	if (!init_htab(hashp, nelem))
		ereport(ERROR,
                (errcode(ERRCODE_HASH_TABLE_INIT_FAILURE),
...
choose_nelem_alloc(Size entrysize)
{
	...
	{
		allocSize <<= 1;
		nelem_alloc = (int)(allocSize / elementSize);
...
void fxdb_free_hashtab()
{
	extern void fxdb_free_smgrhash();
	extern void fxdb_free_lockhash();

	fxdb_free_smgrhash();
	fxdb_free_relid_cache();
	fxdb_free_lockhash();
}
							...
							bool *foundPtr)
{
		...
		case HASH_ENTER:
			...
			if (hashp->frozen)
				ereport(ERROR,
                        ...
                        errmsg( "cannot insert into frozen hashtable \"%s\"",
					                         hashp->tabname)));
	...
	ereport(ERROR,
            (errcode(ERRCODE_HASH_UNKNOWN_ACTION_CODE),
            errmsg("unrecognized hash action code: %d", (int) action)));
...
hash_freeze(HTAB *hashp)
{
	if (hashp->isshared)
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                errmsg("cannot freeze shared hashtable \"%s\"", hashp->tabname)));
	if (!hashp->frozen && has_seq_scans(hashp))
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                errmsg( "cannot freeze hashtable \"%s\" because it has active scans",
			                 hashp->tabname)));
	hashp->frozen = true;
}
...
#else
        if ((hashp->dir[new_segnum] = seg_alloc(hashp)) == NULL)
#endif //FOUNDER_XDB_SE		
			return false;
		hctl->nsegs++;
	}
...
hash_corrupted(HTAB *hashp)
{
	...
	if (hashp->isshared)
		ereport(PANIC,
                ...
                errmsg("hash table \"%s\" corrupted", hashp->tabname)));
	else
		ereport(FATAL,
                (errcode(ERRCODE_DATA_CORRUPTED),
                errmsg("hash table \"%s\" corrupted", hashp->tabname)));
...
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL HTAB *seq_scan_tables[MAX_SEQ_SCANS];	/* tables being scanned */
static THREAD_LOCAL int	seq_scan_level[MAX_SEQ_SCANS];		/* subtransaction nest level */
static THREAD_LOCAL int	num_seq_scans = 0;

#else
#define seq_scan_tables ((NULL == CurrentTransactionState ? GetDefaultXact() : CurrentTransactionState)->mTopLevelState->mSeq_scan_tables)
#define seq_scan_level ((NULL == CurrentTransactionState ? GetDefaultXact() : CurrentTransactionState)->mTopLevelState->mSeq_scan_level)
#define num_seq_scans ((NULL == CurrentTransactionState ? GetDefaultXact() : CurrentTransactionState)->mTopLevelState->mNum_seq_scans)
...
register_seq_scan(HTAB *hashp)
{
	if (num_seq_scans >= MAX_SEQ_SCANS)
		ereport(ERROR,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg( "too many active hash_seq_search scans, cannot start one on \"%s\"",
			                 hashp->tabname)));
...
deregister_seq_scan(HTAB *hashp)
{
	...
	ereport(ERROR,
            ...
            errmsg( "no hash_seq_search scan for hash table \"%s\"",
		             hashp->tabname)));
}
