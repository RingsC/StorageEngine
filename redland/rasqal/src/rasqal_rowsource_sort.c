...
static raptor_sequence*
rasqal_sort_rowsource_sortentry_to_sequence(FS::SortEntrySet *entryset, raptor_sequence* seq)
{
	FS::StorageEngine *storageEngine = FS::StorageEngine::getStorageEngine();
	FS::Transaction *trans = storageEngine->GetCurrentTransaction();

	FS::EntrySetScan *scan = entryset->startEntrySetScan(trans);
	FS::DataItem di;
	FS::EntryID ei;

	while (scan->getNext(FS::EntrySetScan::NEXT_FLAG, ei, di) != NO_DATA_FOUND)
	{
		rasqal_row* row = rasqal_sort_cmp_row_get_row((rasqal_sort_compare_row *)di.getData());
		row = rasqal_new_row_from_row(row);
		raptor_sequence_push(seq, row);
		FS::MemoryContext::deAlloc(di.getData());
	}
...
rasqal_sort_rowsource_init(rasqal_rowsource* rowsource, void *user_data)
{
  ...
  if(con->order_size > 0 ) {
    ...
    con->map = rasqal_engine_new_rowsort_map(con->distinct,
		...
		FS::StorageEngine *storageEngine = FS::StorageEngine::getStorageEngine();
		FS::Transaction *trans = storageEngine->GetCurrentTransaction();
		con->entryset = storageEngine->createSortEntrySet(trans, 
			FS::EntrySet::SST_GenericDataTuples, 
			SortEntrySetWorkMem, 
			true, 
			sort_compare_row_func);

    if(!con->map || !con->entryset)
      return 1;
  }
                              ...
                              rasqal_sort_rowsource_context* con)
{
  ...
  while(1) {
    ...
    row->offset = offset;
		...
		rasqal_sort_compare_row *cmp_row = rasqal_new_sort_cmp_row();
		rasqal_sort_cmp_row_set_map(cmp_row, con->map);
		rasqal_sort_cmp_row_set_row(cmp_row, row);

		FS::DataItem di(cmp_row, rasqal_sort_cmp_row_struct_size());
		FS::EntryID ei;
		con->entryset->insertEntry(trans, ei, di);

		RASQAL_FREE(rasqal_sort_compare_row *, cmp_row);

    offset++;
  }

	//Ö´ÐÐÅÅÐò
	con->entryset->performSort();
  ...
  /* do sort/distinct: walk map in order, adding rows to sequence */
	rasqal_sort_rowsource_sortentry_to_sequence(con->entryset, con->seq);
  rasqal_free_map(con->map); con->map = NULL;
	delete con->entryset; con->entryset = NULL;
