...
THREAD_LOCAL pg_tz	   *session_timezone = NULL;

/* Current log timezone (controlled by log_timezone GUC) */
THREAD_LOCAL pg_tz	   *log_timezone = NULL;

/* Fallback GMT timezone for last-ditch error message formatting */
THREAD_LOCAL pg_tz	   *gmt_timezone = NULL;
static THREAD_LOCAL pg_tz gmt_timezone_data;
...
pg_TZDIR(void)
{
	...
	static THREAD_LOCAL bool done_tzdir = false;
	static THREAD_LOCAL char tzdir[MAXPGPATH];
...
#else 
    strlcpy(tzdir, DataDir, MAXPGPATH - strlen(tzdir));
    strlcpy(tzdir+strlen(tzdir), "/../share", MAXPGPATH - strlen(tzdir));
...
pg_open_tzfile(const char *name, char *canonname)
{
	...
	orignamelen = fullnamelen = (int)strlen(fullname);
	fname = name;
	for (;;)
	{
		const char *slashptr;
		...
		fullnamelen += (int)strlen(fullname + fullnamelen);
		if (slashptr)
			fname = slashptr + 1;
		else
			break;
...
#else
        if (strlen(direntry->d_name) == (uint32)fnamelen &&
#endif //FOUNDER_XDB_SE		
...
{
	static THREAD_LOCAL char resultbuf[TZ_STRLEN_MAX + 1];
	time_t		tnow;
	time_t		t;
	struct tztry tt;
	struct tm  *tm;
...
static THREAD_LOCAL HTAB *timezone_cache = NULL;
...
pg_timezone_pre_initialize(void)
{
	...
	if (tzparse("GMT", &gmt_timezone_data.state, TRUE) != 0)
		ereport(FATAL,
                ...
                errmsg("could not initialize GMT time zone")));
...
pg_tzenumerate_start(void)
{
	...
	ret->baselen = (int)strlen(startdir) + 1;
	ret->depth = 0;
