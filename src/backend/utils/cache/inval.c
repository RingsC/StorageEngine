...
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL TransInvalidationInfo *transInvalInfo = NULL;
static THREAD_LOCAL SharedInvalidationMessage *SharedInvalidMessagesArray;
static THREAD_LOCAL int	numSharedInvalidMessagesArray;
static THREAD_LOCAL int	maxSharedInvalidMessagesArray;
#else
#define transInvalInfo (CurrentTransactionState->mTopLevelState->mTransInvalInfo)
#define SharedInvalidMessagesArray (CurrentTransactionState->mTopLevelState->mSharedInvalidMessagesArray)
#define numSharedInvalidMessagesArray (CurrentTransactionState->mTopLevelState->mNumSharedInvalidMessagesArray)
#define maxSharedInvalidMessagesArray (CurrentTransactionState->mTopLevelState->mMaxSharedInvalidMessagesArray)
#endif
...
static THREAD_LOCAL struct SYSCACHECALLBACK
{
...
static THREAD_LOCAL int	syscache_callback_count = 0;

static THREAD_LOCAL struct RELCACHECALLBACK
{
	RelcacheCallbackFunction function;
	Datum		arg;
}	relcache_callback_list[MAX_RELCACHE_CALLBACKS];

static THREAD_LOCAL int	relcache_callback_count = 0;
...
LocalExecuteInvalidationMessage(SharedInvalidationMessage *msg)
{
	...
	else if (msg->id == SHAREDINVALRELMAP_ID)
	{
		...
		ereport(FATAL,
                ...
                errmsg("unrecognized SI message id: %d", msg->id)));
}
...
PrepareForTupleInvalidation(Relation relation, HeapTuple tuple)
{
	...
	PrepareToInvalidateCacheTuple(relation, tuple,
								  RegisterCatcacheInvalidation);
#else
	if (!isMetaTable(RelationGetRelid(relation)))
		return;
#endif
	/*
	...
	{
		Form_meta_table classtup = (Form_meta_table) VARDATA_ANY (GETSTRUCT(tuple));

		relationId = classtup->rel_id;
		databaseId = classtup->database_id;
	} else
		return;
#endif //FOUNDER_XDB_SE
...
//		return;
	 ...
	 */
	//Assert(false);// it's wrong to get here, because the databaseId and relationId below isn't assigned yet.
...
AcceptInvalidationMessages(void)
{
	...
	{
		static THREAD_LOCAL bool in_recursion = false;
...
MakeSharedInvalidMessagesArray(const SharedInvalidationMessage *msgs, int n)
{
...
#else
		SharedInvalidMessagesArray = (SharedInvalidationMessage *)palloc(maxSharedInvalidMessagesArray
		                                * sizeof(SharedInvalidationMessage));
#endif //FOUNDER_XDB_SE
	...
	if ((numSharedInvalidMessagesArray + n) > maxSharedInvalidMessagesArray)
	{
		...
		SharedInvalidMessagesArray = repalloc(SharedInvalidMessagesArray,
										...
										* sizeof(SharedInvalidationMessage));
#else
		SharedInvalidMessagesArray = (SharedInvalidationMessage *)repalloc(SharedInvalidMessagesArray,
											  maxSharedInvalidMessagesArray
										* sizeof(SharedInvalidationMessage));
#endif //FOUNDER_XDB_SE
	}
	 ...
	 */
#ifndef FOUNDER_XDB_SE
	memcpy(SharedInvalidMessagesArray + (numSharedInvalidMessagesArray * sizeof(SharedInvalidationMessage)),
...
#else
CommandEndInvalidationMessages(bool append)
#endif //FOUNDER_XDB_SE
{
	/*
...
CacheInvalidateCatalog(Oid catalogId)
{
	...
	if (IsSharedRelation(catalogId))
		...
		databaseId = MyDatabaseId;
#else
	HeapTuple tup = fxdb_search_meta_table_copy(catalogId);
	Form_meta_table formClass = (Form_meta_table) VARDATA_ANY(GETSTRUCT(tup));
	databaseId = formClass->database_id;
	pfree(tup);
#endif //FOUNDER_XDB_SE
...
CacheInvalidateRelcache(Relation relation)
{
	...
	if(IsTmpOid(RelationGetRelid(relation)))
	{
		return ;
	}
#endif //FOUNDER_XDB_SE
	...
	if (relation->rd_rel->relisshared)
		...
		databaseId = relation->rd_node.dbNode;
...
CacheInvalidateRelcacheByTuple(HeapTuple classTuple)
{
	...
	RegisterRelcacheInvalidation(databaseId, relationId);
#else
	Form_meta_table classtup = (Form_meta_table) VARDATA_ANY(GETSTRUCT(classTuple));
	Oid			databaseId;
	Oid			relationId;
...
#else
	tup = fxdb_search_meta_table_copy(relid);
#endif //FOUNDER_XDB_SE
							  ...
							  Datum arg)
{
	if (syscache_callback_count >= MAX_SYSCACHE_CALLBACKS)
		ereport(FATAL,
                ...
                errmsg("out of syscache_callback_list slots")));
							  ...
							  Datum arg)
{
	if (relcache_callback_count >= MAX_RELCACHE_CALLBACKS)
		ereport(FATAL,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg("out of relcache_callback_list slots")));
