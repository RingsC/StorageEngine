...
#ifdef FOUNDER_XDB_SE
static THREAD_LOCAL char socketLockFile[MAXPGPATH];
#endif //FOUNDER_XDB_SE
...
#ifndef FOUNDER_XDB_SE
        newdir = malloc(strlen(buf) + strlen(path) + 2);
#else 
				MemoryContext oldcxt = MemoryContextSwitchTo(TopMemoryContext);
        newdir = (char *)palloc0(strlen(buf) + strlen(path) + 2);
...
#ifdef FOUNDER_XDB_SE
static THREAD_LOCAL Oid	AuthenticatedUserId = InvalidOid;
static THREAD_LOCAL Oid	SessionUserId = InvalidOid;
static Oid	OuterUserId = InvalidOid;
static THREAD_LOCAL Oid	CurrentUserId = InvalidOid;
#endif //FOUNDER_XDB_SE
			   ...
			   bool isDDLock, const char *refName)
{
		...
		if (other_pid <= 0)
			ereport(FATAL,
                    (errcode(ERRCODE_DATA_CORRUPTED),
                     errmsg("bogus data in lock file \"%s\": \"%s\"",
                     filename, buffer)));
	...
	errno = 0;
	if (write(fd, buffer, (unsigned int)strlen(buffer)) != (unsigned int)strlen(buffer))
	{
...
#ifdef FOUNDER_XDB_SE
THREAD_LOCAL char	   *shared_preload_libraries_string = NULL;
THREAD_LOCAL char	   *local_preload_libraries_string = NULL;
#endif //FOUNDER_XDB_SE
