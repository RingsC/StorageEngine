...
{
#ifdef HAVE_INT64_TIMESTAMP
	static THREAD_LOCAL const int64 TimestampScales[MAX_TIMESTAMP_PRECISION + 1] = {
		INT64CONST(1000000),
		INT64CONST(100000),
		INT64CONST(10000),
		INT64CONST(1000),
	...
	static THREAD_LOCAL const int64 TimestampOffsets[MAX_TIMESTAMP_PRECISION + 1] = {
		INT64CONST(500000),
		INT64CONST(50000),
		INT64CONST(5000),
		INT64CONST(500),
...
#else
	static THREAD_LOCAL const double TimestampScales[MAX_TIMESTAMP_PRECISION + 1] = {
		1,
		10,
		100,
		1000,
	...
	switch (dtype)
	{
		...
		default:
			ereport(ERROR,
                    ...
                    errmsg( "unexpected dtype %d while parsing interval \"%s\"",
				                     dtype, str)));
	}
...
interval_out(PG_FUNCTION_ARGS)
{
	...
	if (interval2tm(*span, tm, &fsec) != 0)
		ereport(ERROR,
                (errcode(ERRCODE_TIME_STAMP_INTERVAL_2_TIME_FAILURE),
...
intervaltypmodout(PG_FUNCTION_ARGS)
{
	...
	switch (fields)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_UNDEFINED_PARAMETER),
                    errmsg("invalid INTERVAL typmod: 0x%x", typmod)));
			fieldstr = "";
			break;
	}
...
AdjustIntervalForTypmod(Interval *interval, int32 typmod)
{
	...
	if (typmod >= 0)
	{
						   ...
						   INTERVAL_MASK(SECOND)))
		{
			...
			ereport(ERROR,
                    (errcode(ERRCODE_TIME_STAMP_UNKNOWN_INTERVAL_TYPMOD),
...
EncodeSpecialTimestamp(Timestamp dt, char *str)
{
	if (TIMESTAMP_IS_NOBEGIN(dt))
		...
		ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("invalid argument for EncodeSpecialTimestamp")));
}
#ifndef FOUNDER_XDB_SE
Datum
...
{
	static THREAD_LOCAL char buf[MAXDATELEN + 1];
	strlcpy(buf, (char *)&t, sizeof(buf));
	return buf;
}
...
dt2time(Timestamp jd, int *hour, int *min, int *sec, fsec_t *fsec)
{
	...
	*fsec = (fsec_t)(time - (*sec * USECS_PER_SEC));
#else
...
interval2tm(Interval span, struct pg_tm * tm, fsec_t *fsec)
{
	...
	tm->tm_hour = (int)tfrac;		/* could overflow ... */
	tfrac = time / USECS_PER_MINUTE;
	time -= tfrac * USECS_PER_MINUTE;
	tm->tm_min = (int)tfrac;
	tfrac = time / USECS_PER_SEC;
	*fsec = (int)(time - (tfrac * USECS_PER_SEC));
	tm->tm_sec = (int)tfrac;
#else
recalc:
...
#ifdef HAVE_INT64_TIMESTAMP
	result->time = (TimeOffset)rint(span->time * factor + sec_remainder * USECS_PER_SEC);
#else
...
interval_accum(PG_FUNCTION_ARGS)
{
	...
	if (ndatums != 2)
		ereport(ERROR,
                ...
                errmsg("expected 2-element interval array")));
