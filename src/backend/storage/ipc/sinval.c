 ...
 */
static THREAD_LOCAL volatile int catchupInterruptEnabled = 0;
static THREAD_LOCAL volatile int catchupInterruptOccurred = 0;
...
{
    SinvalTlsState* pInvalState = CurrentTransactionState->mTopLevelState->pSinvalState;
	SharedInvalidationMessage *messages = pInvalState->messages;
	...
	while (pInvalState->nextmsg < pInvalState->nummsgs)
	{
		SharedInvalidationMessage *msg = &messages[pInvalState->nextmsg++];
	...
	do
	{
		...
		pInvalState->nextmsg = pInvalState->nummsgs = 0;
		...
		pInvalState->nextmsg = 0;
		pInvalState->nummsgs = getResult;
		 ...
		 */
	} while (pInvalState->nummsgs == MAXINVALMSGS);
