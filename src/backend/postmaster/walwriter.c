 ...
 * GUC parameters
 */
THREAD_LOCAL int			WalWriterDelay = 200;
...
WalWriterMain(void)
{
	 ...
	 */
	pg_signal(SIGHUP, WalSigHupHandler); /* set flag to read config file */
	pg_signal(SIGINT, WalShutdownHandler);		/* request shutdown */
	pg_signal(SIGTERM, WalShutdownHandler);		/* request shutdown */
	pg_signal(SIGQUIT, wal_quickdie);	/* hard crash time */
	pg_signal(SIGALRM, SIG_IGN);
	pg_signal(SIGPIPE, SIG_IGN);
	pg_signal(SIGUSR1, SIG_IGN); /* reserve for ProcSignal */
	pg_signal(SIGUSR2, SIG_IGN); /* not used */
