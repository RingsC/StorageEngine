...
#define on_commits (CurrentTransactionState->mTopLevelState->mOnCommits)
#ifndef FOUNDER_XDB_SE
/*
...
//THREAD_LOCAL struct dropmsgstrings
{
	char		kind;
				   ...
				   DropBehavior behavior)
{
	...
	if (targetrelation->rel_kind == RELKIND_COMPOSITE_TYPE)
	{
		List	   *child_oids;
		ListCell   *lo;
...
RenameRelation(Oid myrelid, const char *newrelname, ObjectType reltype)
{
	...
	relkind = targetrelation->rel_kind;
...
RenameRelationInternal(Oid myrelid, const char *newrelname, Oid namespaceId)
{
	 ...
	 */
	if (targetrelation->rel_kind == RELKIND_INDEX)
	{
...
AlterTable(AlterTableStmt *stmt)
{
	...
	switch (stmt->relkind)
	{
		case OBJECT_TABLE:
			 ...
			 */
			if (rel->rel_kind == RELKIND_COMPOSITE_TYPE
				|| rel->rel_kind == RELKIND_FOREIGN_TABLE)
				ereport(ERROR,
						(errcode(ERRCODE_WRONG_OBJECT_TYPE),
						 errmsg("\"%s\" is not a table",
								RelationGetRelationName(rel))));
		...
		case OBJECT_SEQUENCE:
			if (rel->rel_kind != RELKIND_SEQUENCE)
				ereport(ERROR,
						(errcode(ERRCODE_WRONG_OBJECT_TYPE),
						 errmsg("\"%s\" is not a sequence",
								RelationGetRelationName(rel))));
		...
		case OBJECT_FOREIGN_TABLE:
			if (rel->rel_kind != RELKIND_FOREIGN_TABLE)
				ereport(ERROR,
						(errcode(ERRCODE_WRONG_OBJECT_TYPE),
						 errmsg("\"%s\" is not a foreign table",
								RelationGetRelationName(rel))));
				  ...
				  AlterTableCmd *cmd, bool recurse, LOCKMODE lockmode)
{
	 ...
	 */
	if (recurse && rel->rel_kind == RELKIND_RELATION)
	{
		Oid			relid = RelationGetRelid(rel);
		ListCell   *child;
		List	   *children;
					  ...
					  LOCKMODE lockmode)
{
	...
	Assert(rel->rel_kind == RELKIND_COMPOSITE_TYPE);
				...
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot alter type \"%s\" because column \"%s\".\"%s\" uses it",
								...
								RelationGetRelationName(rel),
								NameStr(att->attname))));
			else if (origRelation->rel_kind == RELKIND_COMPOSITE_TYPE)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot alter type \"%s\" because column \"%s\".\"%s\" uses it",
								...
								RelationGetRelationName(rel),
								NameStr(att->attname))));
			else if (origRelation->rel_kind == RELKIND_FOREIGN_TABLE)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
check_of_type(HeapTuple typetuple)
{
	...
	if (typ->typtype == TYPTYPE_COMPOSITE)
	{
		...
		typeOk = (typeRelation->rel_kind == RELKIND_COMPOSITE_TYPE);
						  ...
						  Constraint *fkconstraint, LOCKMODE lockmode)
{
	 ...
	 * numbers)
	 */
	if (pkrel->rel_kind != RELKIND_RELATION)
		ereport(ERROR,
				(errcode(ERRCODE_WRONG_OBJECT_TYPE),
...
ATExecSetRelOptions(Relation rel, List *defList, bool isReset, LOCKMODE lockmode)
{
	...
	switch (rel->rel_kind)
	{
		case RELKIND_RELATION:
		case RELKIND_TOASTVALUE:
			(void) heap_reloptions(rel->rel_kind, newOptions, true);
			break;
		case RELKIND_INDEX:
				   ...
				   ForkNumber forkNum, char relpersistence)
{
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		 ...
		 */
		smgrextend(dst, forkNum, blkno, buf, true, InvalidBlockNumber);
	}
...
PreCommit_on_commit_actions(void)
{
	...
	ResourceOwner owner = TopTransactionResourceOwner;
...
int Fxdb_NumOfTmpRel()
{
	return list_length(on_commits);
}
#endif //FOUNDER_XDB_SE
