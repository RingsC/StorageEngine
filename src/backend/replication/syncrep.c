...
static THREAD_LOCAL bool announce_next_takeover = true;
#ifdef FOUNDER_XDB_SE
/* 
 * true: we can cancel the transaction that are waiting sync replaction.
 * Protected by SyncRepLock.
 */
static bool canCancelSyncRepWait = false;
#endif
...
#ifdef FOUNDER_XDB_SE
static bool SyncRepNeedWaitLSN(void);
static bool SyncRepCanCancelWait(void);
static bool SyncRepNeedCancelWait(void);
#endif
...
#ifdef FOUNDER_XDB_SE
	/* Check if we want to wait this lsn */
	if (!SyncRepNeedWaitLSN())
	{
		LWLockRelease(SyncRepLock);
		return;
	}
...
#ifdef FOUNDER_XDB_SE
		/*
		 * BUG #3306: if we are in shut down database and there are no
		 * active wal senders, cancel the wait.
		 */
		if (ProcDiePending || SyncRepNeedCancelWait())
#else
		if (ProcDiePending)
#endif
		{
...
SyncRepReleaseWaiters(void)
{
	...
	if (XLByteLT(walsndctl->lsn, MyWalSnd->flush))
	{
		 ...
		 * up to this location.
		 */
		copyXLogRecPtr(walsndctl->lsn, MyWalSnd->flush);
...
SyncRepGetStandbyPriority(void)
{
	...
	if(SyncRepStandbyNames == 0)
	    SyncRepStandbyNames=(char*)"";
...
#ifdef FOUNDER_XDB_SE
/*
 * Decide transaction need wait, we don't wait for lsn in following case:
 *     1. all wal sender in the base backup status or startup status
 *     2. none standby has connected to primary
 * The reason for doing this check is that we want to reduce the wait time
 * of the committing transaction.
 * 
 * Slave has higher ratio to report the "request WAL segment has already been removed"
 * error, if we do this check. We can set 'wal_keep_segments' to bigger enough or used
 * xlog archive to reduce this ratio.
 */
static bool
SyncRepNeedWaitLSN(void)
{
	volatile WalSndCtlData *walsndctl = WalSndCtl;
	int i = 0;
	bool needWait = false;
	...
	for (i = 0; i < max_wal_senders; i++)
	{
		...
		/*
		 * Wal sender may switch to WALSNDSTATE_STARTUP from WALSNDSTATE_CATCHUP
		 * or WALSNDSTATE_STREAMING, walsnd->sentPtr must be invalid if wal sender switch
		 * to startup status from catchup or streaming status. In this case transaction wait lsn.
		 */
		if (!(walsnd->state == WALSNDSTATE_STARTUP && 
			XLogRecPtrIsInvalid(walsnd->sentPtr)) &&
			walsnd->state != WALSNDSTATE_BACKUP)
		{
			SpinLockRelease(&walsnd->mutex);
			needWait = true;
			break;
...
void 
SyncRepSetCancelWait(void)
{
	LWLockAcquire(SyncRepLock, LW_EXCLUSIVE);
	canCancelSyncRepWait = true;
	LWLockRelease(SyncRepLock);
}

static bool
SyncRepCanCancelWait(void)
{
	bool shouldCancel = false;
	
	LWLockAcquire(SyncRepLock, LW_EXCLUSIVE);
	shouldCancel = canCancelSyncRepWait;
	LWLockRelease(SyncRepLock);
...
/*
 * BUG #3306:
 * 	Check we can cancel the waiting transactions.
 */
static bool
SyncRepNeedCancelWait(void)
{
	if (!SyncRepCanCancelWait())
		return false;

	return !SyncRepNeedWaitLSN();
}

void
CheckForSyncParam(void)
{
	...
	if(SyncRepStandbyNames)
	{
		...
		if (!SplitIdentifierString(rawstring, ';', &elemlist))
		{
			...
			list_free(elemlist);
			ereport(ERROR,
				(errmsg("syntax error in SyncRepStandbyNames list")));
		}
	}

	if(app_name == NULL)
		return;
	else if(app_name != NULL && SyncRepStandbyNames == NULL)
		ereport(ERROR,
		(errmsg("application_name not in SyncRepStandbyNames list")));

	foreach(l, elemlist)
	{
		char	   *standby_name = (char *) lfirst(l);

		if (pg_strcasecmp(standby_name, app_name) == 0 ||
			pg_strcasecmp(standby_name, "*") == 0)
		{
			found = true;
			break;
