	...
	switch (a->type)
	{
		...
		default:
			ereport(ERROR,
                    ...
                    errmsg( "unrecognized list node type: %d",
				                     (int) a->type)));
...
_equalValue(Value *a, Value *b)
{
	...
	switch (a->type)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_UNDEFINED_PARAMETER),
                    errmsg("unrecognized node type: %d", (int) a->type)));
			break;
	}
 ...
 * only nodetag types defined in nodes.h's NodeTag are enabled in this functions. if you enable a new tag in NodeTag, eanble the 
 * corresponding case branch in this function.
 */
bool
equal(void *a, void *b)
{
...
#else
			retval = _equalRemoveFuncStmt((RemoveFuncStmt*)a, (RemoveFuncStmt*)b);
#endif //FOUNDER_XDB_SE
			break;
#ifndef FOUNDER_XDB_SE
		case T_DoStmt:
...
#else
			retval = _equalRemoveOpClassStmt((RemoveOpClassStmt*)a, (RemoveOpClassStmt*)b);
#endif //FOUNDER_XDB_SE
			break;
...
#else
			retval = _equalRemoveOpFamilyStmt((RemoveOpFamilyStmt*)a, (RemoveOpFamilyStmt*)b);
#endif //FOUNDER_XDB_SE
			break;
		case T_RenameStmt:
#ifndef FOUNDER_XDB_SE
