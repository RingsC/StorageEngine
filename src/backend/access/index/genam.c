 ...
 */
void
IndexScanEnd(IndexScanDesc scan, bool freeScankey, bool freeOrderByData)
{
	if (freeScankey && (scan->keyData != NULL))
	{
		freeScanKeys(scan->keyData,scan->numberOfKeys);
	}
	if (freeOrderByData && (scan->orderByData != NULL))
...
#endif //FOUNDER_XDB_SE	
{
	...
	{
		sysscan->scan = heap_beginscan(heapRelation, snapshot, nkeys, key, 0, 0);
		sysscan->iscan = NULL;
	}
	return sysscan;
}
