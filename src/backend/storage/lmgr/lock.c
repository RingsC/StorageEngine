...
THREAD_LOCAL int			max_locks_per_xact = 128; /* set by guc.c */
...
#ifndef LOCK_DEBUG
static THREAD_LOCAL bool Dummy_trace = false;
#endif
 ...
 */
static THREAD_LOCAL HTAB *LockMethodLockHash;
static THREAD_LOCAL HTAB *LockMethodProcLockHash;
#ifndef FOUNDER_XDB_SE
static THREAD_LOCAL HTAB *LockMethodLocalHash;
#else
#define LockMethodLocalHash (CurrentTransactionState->mTopLevelState->mLockMethodLocalHash)
#endif

/* private state for GrantAwaitedLock */
static THREAD_LOCAL LOCALLOCK *awaitedLock;
static THREAD_LOCAL ResourceOwner awaitedOwner;
...
THREAD_LOCAL int			Trace_lock_oidmin = FirstNormalObjectId;
THREAD_LOCAL bool		Trace_locks = false;
THREAD_LOCAL bool		Trace_userlocks = false;
THREAD_LOCAL int			Trace_lock_table = 0;
THREAD_LOCAL bool		Debug_deadlocks = false;
...
slock_t g_lock_mutex;
char* g_lock_buf_ptr = NULL;
unsigned int g_lock_used_buf = 0;
void write_lock_wait_info_start(timeval &start)
{
...
{
	FILE* pFile = NULL;
	char* path = "../lock_wait.log";
	pFile = fopen(path,"a");
	if (pFile != NULL)
	{
		fwrite(g_lock_buf_ptr,1,g_lock_used_buf,pFile);
	}
	fclose(pFile);

	memset(g_lock_buf_ptr,0,g_lock_used_buf);
	g_lock_used_buf = 0;
}
void write_lock_wait_info_end(const LOCKTAG* locktag, LOCKMODE lockmode, const timeval &start)
{
	//end time
	timeval end;
	...
	//alloc
	unsigned int buf_total = 1024 * 8;

	if (NULL == g_lock_buf_ptr)
	{
		g_lock_buf_ptr = (char*)MemoryContextAllocZero(ProcessTopMemoryContext, buf_total);
	}
	
	Assert(NULL != g_lock_buf_ptr);

	static char* pIter = g_lock_buf_ptr;


	Assert(g_lock_used_buf < buf_total);

	//identify enough space
	unsigned int stack_depth = 15;
	unsigned int fmt_size = 40;
	unsigned int space_limit = stack_depth * STRLENGTH + fmt_size;

	if (buf_total - g_lock_used_buf < space_limit)
	{
		lock_buf_flush();
		pIter = g_lock_buf_ptr;
	}

	//elapse time
	unsigned int elapse_time = (end.tv_sec - start.tv_sec)*1000 + (end.tv_usec - start.tv_usec)/1000;//milliseconds

	if(elapse_time >= 100)
	{
		BackTrace(stack_depth,pIter);
		char* pLineEnd = pIter;
		for (unsigned int i = 0; i < stack_depth; i ++)
		{
			if (0 == i)
			{
				pLineEnd = pLineEnd + STRLENGTH - 1;
			}
			else
			{
				pLineEnd = pLineEnd + STRLENGTH;
			}			
			
			pLineEnd[0] = '\n';
		}
		
		pIter += stack_depth*STRLENGTH;
		g_lock_used_buf += stack_depth*STRLENGTH;
	}

	//calculate str len
	sprintf_s(pIter,fmt_size,"%d %d %d %d %d %d %d %d\n\n",
	...
	unsigned int len = strlen(pIter);
	pIter += len;
	g_lock_used_buf += len;
}
#endif //LOCK_WAIT_INFO_LOG
					...
					bool reportMemoryError)
{
	...
	if (lockmethodid <= 0 || lockmethodid >= lengthof(LockMethods))
		ereport(ERROR,
                (errcode(ERRCODE_LOCK_UNKNOWN_METHOD),
                errmsg("unrecognized lock method: %d", lockmethodid)));
	lockMethodTable = LockMethods[lockmethodid];
	if (lockmode <= 0 || lockmode > lockMethodTable->numLockModes)
		ereport(ERROR,
                (errcode(ERRCODE_LOCK_UNKNOWN_MODE),
	...
	proclocktag.myProc = MyProc->mCurrentTransState->mTopLevelState->lockProc;
	...
	if (!found)
	{
		...
		SHMQueueInsertBefore(&(MyProc->mCurrentTransState->mTopLevelState->lockProc->myProcLocks[partition]),
							 &proclock->procLink);
	...
	if (proclock->holdMask & LOCKBIT_ON(lockmode))
		ereport(ERROR,
                (errcode(ERRCODE_LOCK_ACQUIRE_FAILURE),
                errmsg( "lock %s on object %u/%u/%u is already held",
			                 lockMethodTable->lockModeNames[lockmode],
			                 lock->tag.locktag_field1, lock->tag.locktag_field2,
			                 lock->tag.locktag_field3)));
	...
	if (lockMethodTable->conflictTab[lockmode] & lock->waitMask)
		...
		status = LockCheckConflicts(lockMethodTable, lockmode,
									lock, proclock, MyProc->mCurrentTransState->mTopLevelState->lockProc);
	...
	else
	{
		...
		if (dontWait)
		{
			if (proclock->holdMask == 0)
			{
				...
				if (!hash_search_with_hash_value(LockMethodProcLockHash,
					...
					ereport(PANIC,
                            (errcode(ERRCODE_DATA_CORRUPTED),
                            errmsg("proclock table corrupted")));
			}
			else
		 ...
		 */
		MyProc->mCurrentTransState->mTopLevelState->lockProc->heldLocks = proclock->holdMask;
...
#ifdef LOCK_WAIT_INFO_LOG
		timeval start;
		write_lock_wait_info_start(start);
#endif //LOCK_WAIT_INFO_LOG
...
#ifdef LOCK_WAIT_INFO_LOG
		SpinLockAcquire(&g_lock_mutex);
		write_lock_wait_info_end(locktag,lockmode,start);
		SpinLockRelease(&g_lock_mutex);
#endif //LOCK_WAIT_INFO_LOG
...
LockReleaseAll(LOCKMETHODID lockmethodid, bool allLocks)
{
	...
	for (partition = 0; partition < NUM_LOCK_PARTITIONS; partition++)
	{
		...
		LWLockId	partitionLock = (LWLockId)(FirstLockMgrLock + partition);

#endif // FOUNDER_XDB_SE
		SHM_QUEUE  *procLocks = &(MyProc->mCurrentTransState->mTopLevelState->lockProc->myProcLocks[partition]);
		...
		while (proclock)
		{
			...
			Assert(proclock->tag.myProc == MyProc->mCurrentTransState->mTopLevelState->lockProc);
...
GetLockConflicts(const LOCKTAG *locktag, LOCKMODE lockmode)
{
	...
	while (proclock)
	{
		if (conflictMask & proclock->holdMask)
		{
			...
			if (proc != MyProc->mCurrentTransState->mTopLevelState->lockProc)
			{
	...
	if (count > MaxBackends)	/* should never happen */
		ereport(PANIC,
                (errcode(ERRCODE_LOCK_ERROR),
                errmsg("too many conflicting locks found")));
		...
		for (i = locallock->numLockOwners - 1; i >= 0; i--)
		{
			...
			if (lockOwners[i].owner == NULL)
				ereport(ERROR,
                        ...
                        errmsg("cannot PREPARE when session locks exist")));
		}
	...
	for (partition = 0; partition < NUM_LOCK_PARTITIONS; partition++)
	{
		...
		while (proclock)
		{
			...
			if (proclock->releaseMask != proclock->holdMask)
				ereport(PANIC,
                        (errcode(ERRCODE_LOCK_ERROR),
                        errmsg("we seem to have dropped a bit somewhere")));
...
GetLockStatusData(void)
{
...
#else
	for (i = 0; i < NUM_LOCK_PARTITIONS; i++)
		LWLockAcquire((LWLockId)(FirstLockMgrLock + i), LW_SHARED);
#endif // FOUNDER_XDB_SE
	...
	el = 0;
	while ((proclock = (PROCLOCK *) hash_seq_search(&seqstat)) != NULL)
	{
		PGPROC	   *proc = proclock->tag.myProc;
...
#else
            accessExclusiveLocks[index].xid = ProcTopLevel->xid;
#endif
						 ...
						 void *recdata, uint32 len)
{
	...
	if (!lock)
		ereport(PANIC,
                (errcode(ERRCODE_LOCK_ERROR),
                errmsg("failed to re-find shared lock object")));
	...
	if (!proclock)
		ereport(PANIC,
                (errcode(ERRCODE_LOCK_ACQUIRE_FAILURE),
                errmsg("failed to re-find shared proclock object")));
