 ...
 * GUC parameters
 */
bool		autovacuum_start_daemon = true;
int			autovacuum_max_workers = DEFAULT_AUTOVAC_WORKER_NUM;
int			autovacuum_naptime = DEFAULT_AUTOVAC_NAPTIME;
int			autovacuum_vac_thresh = DEFAULT_AUTOVAC_THRESH;
double		autovacuum_vac_scale = DEFAULT_AUTOVAC_SCALE;
int			autovacuum_anl_thresh = 50;
double		autovacuum_anl_scale = 0.10000000000000001;
int			autovacuum_freeze_max_age = DEFAULT_AUTOVAC_FREEZE_MAX_AGE;
...
static THREAD_LOCAL bool am_autovacuum_launcher = false;
static THREAD_LOCAL bool am_autovacuum_worker = false;

/* Flags set by signal handlers */
static THREAD_LOCAL volatile sig_atomic_t got_SIGHUP = false;
static THREAD_LOCAL volatile sig_atomic_t got_SIGUSR2 = false;
static THREAD_LOCAL volatile sig_atomic_t got_SIGTERM = false;

/* Comparison point for determining whether freeze_max_age is exceeded */
static THREAD_LOCAL TransactionId recentXid;

/* Default freeze ages to use for autovacuum (varies by database) */
static THREAD_LOCAL int	default_freeze_min_age;
static THREAD_LOCAL int	default_freeze_table_age;

/* Memory context for long-lived data */
static THREAD_LOCAL MemoryContext AutovacMemCxt;
...
static THREAD_LOCAL WorkerInfo MyWorkerInfo = NULL;

/* PID of launcher, valid only in worker while shutting down */
THREAD_LOCAL int			AutovacuumLauncherPid = 0;
...
AutoVacLauncherMain(int argc, char *argv[])
{
...
#endif
	ereport(LOG,
			(errmsg("vc launcher started")));

	if (PostAuthDelay)
		pg_sleep(PostAuthDelay * 1000000L);
	 ...
	 * tcop/postgres.c.
	 */
	pg_signal(SIGHUP, avl_sighup_handler);
	pg_signal(SIGINT, StatementCancelHandler);
	pg_signal(SIGTERM, avl_sigterm_handler);

	pg_signal(SIGQUIT, quickdie);
	pg_signal(SIGALRM, handle_sig_alarm);

	pg_signal(SIGPIPE, SIG_IGN);
	pg_signal(SIGUSR1, procsignal_sigusr1_handler);
	pg_signal(SIGUSR2, avl_sigusr2_handler);
	pg_signal(SIGFPE, FloatExceptionHandler);
...
//			ProcessConfigFile(PGC_SIGHUP);
		...
		if (got_SIGUSR2)
		{
			...
			if (AutoVacuumShmem->av_signal[AutoVacForkFailed])
			{
				...
				pg_sleep(1000000L);	/* 1s */
#ifndef FOUNDER_XDB_SE
				SendPostmasterSignal(PMSIGNAL_START_AUTOVAC_WORKER);
#else
				extern void fxdb_vacuum_worker_forkexec();
				fxdb_vacuum_worker_forkexec();
#endif //FOUNDER_XDB_SE
				continue;
			}
		}
	...
	ereport(LOG,
			(errmsg("vc launcher shutting down")));
...
launcher_determine_sleep(bool canlaunch, bool recursing, struct timeval * nap)
{
	...
	{
		nap->tv_sec = 0;
		nap->tv_usec = (long)(MIN_AUTOVAC_SLEEPTIME * 1000);
	}
}
...
//#endif //FOUNDER_XDB_SE
	...
	if (nelems > 0)
	{
		 ...
		 * the GUC variable).
		 */
		millis_increment = (int)(1000.0 * autovacuum_naptime / nelems);
...
#else
	ereport(LOG,
		(errmsg("vaccuum worker started:%d\n", (int)MyProcPid)));
#endif //FOUNDER_XDB_SE
...
//	PG_SETMASK(&UnBlockSig);
	...
	if (AutoVacuumShmem->av_startingWorker != NULL)
	{
		...
		if (AutoVacuumShmem->av_launcherpid != 0)
			pgkill(AutoVacuumShmem->av_launcherpid, SIGUSR2);
	}
	else
	{
	...
	while (worker)
	{
			...
			worker->wi_cost_limit_base > 0 && worker->wi_cost_delay > 0)
		{
			...
			elog(DEBUG2, "autovac_balance_cost(pid=%u db=%u, rel=%u, cost_limit=%d, cost_limit_base=%d, cost_delay=%d)",
				 (unsigned int)worker->wi_proc->pid, worker->wi_dboid, worker->wi_tableoid,
...
get_database_list(void)
{
	...
	while (HeapTupleIsValid(tup = heap_getnext(scan, ForwardScanDirection)))
	{
		...
		Form_meta_database formDatabase = Tuple2FormDatabaseData(tup);
		//Form_meta_database formDatabase = (Form_meta_database)fxdb_tuple_to_chars(tup);
		//formDatabase = (Form_meta_database)data;
		avdb->adw_datid = formDatabase->db_id;
		avdb->adw_name = pstrdup(NameStr(formDatabase->db_name));
		avdb->adw_frozenxid = formDatabase->xid;
		//pfree(formDatabase);
#endif //FOUNDER_XDB_SE
		/* this gets set later: */
		avdb->adw_entry = NULL;
...
do_autovacuum(Oid dbId)
{
	...
	SetCurrentDatabase(NULL, dbId, InvalidOid);
...
#else
	{
		...
		char tmp_oid[4];
		memset(tmp_oid, 0, sizeof(tmp_oid));
		Datum values = fdxdb_string_formdatum(tmp_oid, sizeof(tmp_oid));
	...
	relScan = heap_beginscan(classRel, SnapshotNow, 1, &key, 0, 0);
	...
	while ((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
	{
...
#else 
		Form_meta_table classForm = fxdb_get_meta_table_struct(tuple);

		// Ignore other database table
		if (classForm->database_id != MyDatabaseId)
		{
			continue;
		}

		// Ignore toast table.
		extern bool is_toast_id(Oid rel_id);
		if (is_toast_id(classForm->rel_id))
		{
			pfree(classForm);
			continue;
		}
...
#else
		extern Oid get_toast_table_id(Oid rel_id);

		//char *data = fxdb_tuple_to_chars(tuple);

		relid = classForm->rel_id;
		Oid reltoastrelid = get_toast_table_id(relid);

		tabentry = get_pgstat_tabentry_relid(relid, false, shared, dbentry);

		//pfree(data);

		/* Check if it needs vacuum or analyze */
		relation_needs_vacanalyze(relid, NULL, classForm, tabentry,
			&dovacuum, &doanalyze, &wraparound);
#endif //FOUNDER_XDB_SE
		...
		if (classForm->relpersistence == RELPERSISTENCE_TEMP)
		{
			...
			ereport(LOG,
				(errmsg("autovacuum: found orphan temp table \"%d\" in database \"%d\"",
				classForm->rel_id, classForm->database_id)));
		}
		else
		{
#endif //FOUNDER_XDB_SE
			...
			if (OidIsValid(reltoastrelid))
			{
				av_relation *hentry;
				bool		found;
				hentry = (av_relation*)hash_search(table_toast_map,
									 &reltoastrelid,
									 HASH_ENTER, &found);
#endif //FOUNDER_XDB_SE
...
//#ifndef FOUNDER_XDB_SE
	...
	while ((tuple = heap_getnext(relScan, ForwardScanDirection)) != NULL)
	{
		...
		relid = HeapTupleGetOid(tuple);
#else
		extern bool is_toast_id(Oid relid);

		relid = classForm->rel_id;
		/* Check it was a toast table but a toast index table. */
		if(!is_toast_id(relid) || is_toast_id(classForm->rel_pid))
		{
			continue;
		}
#endif //FOUNDER_XDB_SE
...
#else
			hentry = (av_relation*)hash_search(table_toast_map, &relid, HASH_FIND, &found);
#endif //FOUNDER_XDB_SE
		...
		relation_needs_vacanalyze(relid, relopts, classForm, tabentry,
			&dovacuum, &doanalyze, &wraparound);
#else
		tabentry = get_pgstat_tabentry_relid(relid, false,
			shared, dbentry);
						  ...
						  PgStat_StatDBEntry *dbentry)
{
	...
	if (isshared)
	{
		...
		tabentry = hash_search(shared->tables, &relid,
								   HASH_FIND, NULL);
#else
		tabentry = (PgStat_StatTabEntry *)hash_search(shared->tables, &relid,
								   HASH_FIND, NULL);
#endif //FOUNDER_XDB_SE
		...
		tabentry = hash_search(dbentry->tables, &relid,
							   HASH_FIND, NULL);
#else
		tabentry = (PgStat_StatTabEntry *)hash_search(dbentry->tables, &relid,
							   HASH_FIND, NULL);
#endif //FOUNDER_XDB_SE
...
#else
	/* fetch the relation's relcache entry */
	classTup = fxdb_search_meta_table_copy(relid);
	if (!HeapTupleIsValid(classTup))
		return NULL;
	classForm = fxdb_get_meta_table_struct(classTup);
#endif //FOUNDER_XDB_SE
...
hentry = (av_relation*)hash_search(table_toast_map, &relid, HASH_FIND, &found);
	...
	/* ignore ANALYZE for toast tables */
	if (/*classForm->relkind == RELKIND_TOASTVALUE*/false)
		doanalyze = false;
...
#endif //FOUNDER_XDB_SE
	{
		...
		tab->at_datname = NULL;
#else
		tab->at_relname = (char*)"test_name";
		tab->at_nspname = (char*)"test_nspname";
		tab->at_datname = (char*)"test_datname";
#endif //FOUNDER_XDB_SE
	}
...
#endif //FOUNDER_XDB_SE
{
	...
	*wraparound = force_vacuum;
	...
	{
		reltuples = (float4)classForm->reltuples;
		vactuples = (float4)tabentry->n_dead_tuples;
		anltuples = (float4)tabentry->changes_since_analyze;
