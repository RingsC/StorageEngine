...
PageInit(Page page, Size pageSize, Size specialSize)
{
	...
	p->pd_upper = (LocationIndex)(pageSize - specialSize);
	p->pd_special = (LocationIndex)(pageSize - specialSize);
...
PageRepairFragmentation(Page page)
{
	 ...
	 */
	if (pd_lower < (Offset)SizeOfPageHeaderData ||
		pd_lower > pd_upper ||
		pd_upper > pd_special ||
		pd_special > (Offset)BLCKSZ ||
	...
	{							/* nstorage != 0 */
		...
		for (i = 0, itemidptr = itemidbase; i < nstorage; i++, itemidptr++)
		{
			...
			upper -= (Offset)itemidptr->alignedlen;
...
PageIndexTupleDelete(Page page, OffsetNumber offnum)
{
...
#else
	if (offset < phdr->pd_upper || (offset + size) > phdr->pd_special ||
		offset != (unsigned int)MAXALIGN(offset) || size != (unsigned int)MAXALIGN(size))
		ereport(ERROR,
				(errcode(ERRCODE_DATA_CORRUPTED),
				 errmsg("corrupted item pointer: offset = %u, size = %u",
						offset, (unsigned int) size)));
	...
	phdr->pd_upper += (LocationIndex)size;
...
PageIndexMultiDelete(Page page, OffsetNumber *itemnos, int nitems)
{
...
#else
		if ((Offset)offset < pd_upper ||
			(Offset)(offset + size) > pd_special ||
			offset != (unsigned int)MAXALIGN(offset))
			ereport(ERROR,
					(errcode(ERRCODE_DATA_CORRUPTED),
					 errmsg("corrupted item pointer: offset = %u, size = %u",
							offset, (unsigned int) size)));
	...
	if (nextitm != nitems)
		ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("incorrect index offsets supplied")));
	...
	phdr->pd_lower = (LocationIndex)(SizeOfPageHeaderData + nused * sizeof(ItemIdData));
	phdr->pd_upper = upper;
