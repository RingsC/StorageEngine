...
/* in buf_init.c */
extern THREAD_LOCAL BufferDesc *BufferDescriptors;
...
struct LocalBufferInfo
{
    ...
    BufferDesc *pLocalBufferDescriptors;
    Block	   *pLocalBufferBlockPointers;
    int32	   *pLocalRefCount;

    int	nNextFreeLocalBuf;

    HTAB *pLocalBufHash;

    char *pCur_block;
    int	n_next_buf_in_block;
    int	n_num_bufs_in_block;
    int	n_total_bufs_allocated;
    MemoryContext mLocalBufferContext;
};
LocalBufferInfo* GetDefaultLocalBufferInfo( void );
extern void AtStart_LocalBuffer( void );
