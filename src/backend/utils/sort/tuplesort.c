...
/* GUC variables */
#ifdef TRACE_SORT
THREAD_LOCAL bool		trace_sort = false;
#endif

#ifdef DEBUG_BOUNDED_SORT
THREAD_LOCAL bool		optimize_bounded_sort = true;
#endif
#endif //FOUNDER_XDB_SE
...
struct Tuplesortstate
{
    ...
    bool        sorted;         /* has the tuples been sorted? */
	/*
...
#define USEMEM(state,amt)	((state)->availMem -= (long)(amt))
#define FREEMEM(state,amt)	((state)->availMem += (long)(amt))
...
#define LogicalTapeReadExact(tapeset, tapenum, ptr, len) \
	do { \
		if (LogicalTapeRead(tapeset, tapenum, ptr, len) != (size_t) (len)) \
			ereport(ERROR,\
                    (errcode(ERRCODE_TUPLE_UNEXPECT_DATA_END),\
...
static int comparetup_datum(const SortTuple *a, const SortTuple *b,
				 Tuplesortstate *state);
static int comparetup_index_datum(const SortTuple *a, const SortTuple *b,
							Tuplesortstate *state);
...
tuplesort_begin_common(int workMem, bool randomAccess)
{
	...
	if (LACKMEM(state))
		ereport(ERROR,
                ...
                errmsg("insufficient memory allowed for sort")));
...
#endif //FOUNDER_XDB_SE
{
...
#else
	AssertArg(compare_func != NULL);

#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG,
		"begin tuple sort: nkeys = %d, workMem = %d, randomAccess = %c",
		1, workMem, randomAccess ? 't' : 'f');
#endif
	...
	state->tupDesc = tupDesc;	/* assume we need not copy tupDesc */
	state->scanKeys = (ScanKey) palloc0(sizeof(ScanKeyData));
	state->scanKeys->sk_compfun = (CompareCallback) compare_func;
#ifndef FOUNDER_XDB_SE
	Oid			sortFunction;
	bool		reverse;
#endif //FOUNDER_XDB_SE
	...
	* will never be passed to an index.
	*/
	ScanKeyEntryInitialize(state->scanKeys,
		flags,
		1,
		InvalidStrategy,
		InvalidOid,
							...
							int workMem, bool randomAccess)
{
	...
	state->nKeys = RelationGetNumberOfAttributes(indexRel);
#else
	state->nKeys = (int)indexRel->rd_colinfo->keys;
#endif //FOUNDER_XDB_SE
...
tuplesort_begin_datum(Oid datumType, Oid sortOperator, Oid sortCollation,
					  ...
					  int workMem, bool randomAccess)
#else
tuplesort_begin_data(int workMem, bool randomAccess, void *compare_func)
#endif //FOUNDER_XDB_SE
{
	Tuplesortstate *state = tuplesort_begin_common(workMem, randomAccess);
	MemoryContext oldcontext;
	...
	typlen = 0;
	typbyval = false;
	if(state->indexScanKey == NULL)
	{
		state->indexScanKey = (ScanKey)palloc0(sizeof(ScanKeyData));
		state->indexScanKey->sk_compfun = (CompareCallback)compare_func;
	}
#endif //FOUNDER_XDB_SE
...
Tuplesortstate* tuplesort_begin_index_datum(int workMem, bool randomAccess, Relation indexRel)
{
	Tuplesortstate *state = tuplesort_begin_common(workMem, randomAccess);
	MemoryContext oldcontext;
	bool		reverse;
	...
	state->nKeys = (int)indexRel->rd_colinfo->keys;			/* always a one-column sort */
	...
	state->comparetup = comparetup_index_datum;
	state->copytup = copytup_datum;
	state->writetup = writetup_datum;
	state->readtup = readtup_datum;
	state->reversedirection = reversedirection_datum;
	...
	typlen = 0;
	typbyval = false;
	if(state->scanKeys == NULL)
	{
		state->scanKeys = (ScanKey)palloc0(indexRel->rd_colinfo->keys * sizeof(ScanKeyData));
	}
	for (size_t i = 0; i < indexRel->rd_colinfo->keys; ++i)
	{
		state->scanKeys[i].sk_compfun = (CompareCallback)indexRel->rd_colinfo->rd_comfunction[i];
		state->scanKeys[i].sk_split = indexRel->rd_colinfo->split_function;
		state->scanKeys[i].sk_attno = (AttrNumber)i+1;//index col is based on 1
	}
...
grow_memtuples(Tuplesortstate *state)
{
	...
	if (LACKMEM(state))
		ereport(ERROR,
                (errcode(ERRCODE_OUT_OF_MEMORY),
                errmsg("unexpected out-of-memory situation during sort")));
	return true;
}
#ifndef FOUNDER_XDB_SE
/*
...
#else
tuplesort_putdata(Tuplesortstate *state, void *data, size_t len)
#endif //FOUNDER_XDB_SE
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	SortTuple	stup;
...
#else
	stup.datum1 = fdxdb_string_formdatum((char*)data, len);
	stup.isnull1 = false;
	stup.tuple = DatumGetPointer(stup.datum1);
	USEMEM(state, GetMemoryChunkSpace(stup.tuple));
#endif //FOUNDER_XDB_SE
...
void tuplesort_putdatum(Tuplesortstate *state, Datum val, ItemPointer tid)
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
...
puttuple_common(Tuplesortstate *state, SortTuple *tuple)
{
	switch (state->status)
	{
		...
		default:
			ereport(ERROR,
                    (errcode(ERRCODE_TUPLE_SORT_INVALID_STATE),
                    errmsg("invalid tuplesort state")));
			break;
	}
	state->sorted = false;
}
 ...
 */
void
tuplesort_performsort(Tuplesortstate *state, void *compare_func)
{
	...
	switch (state->status)
	{
		case TSS_INITIAL:
			 ...
			 */
			if (state->memtupcount > 1 && !state->sorted)
		...
		case TSS_BUILDRUNS:
			...
			//if the tuple set is sorted and ascending, then status->currentRun must be 1. state->resultTape and state->status
			//will be set correctly in mergeruns() method, so there is no need to do condition check of sorted.
		    mergeruns(state);
...
#ifdef FOUNDER_XDB_SE
			else if (!LogicalTapeBackspace(state->tapeset,
				state->result_tape,
				sizeof(unsigned int)))
			{
				/*
				 ...
				 */
					LogicalTapeSeekToEOF(state->tapeset, state->result_tape);
					if (!LogicalTapeBackspace(state->tapeset,
						state->result_tape,
						2 * sizeof(unsigned int)))
						return false;
			...
			else
			{
										  ...
										  tuplen + 2 * sizeof(unsigned int)))
				{
					...
					if (!LogicalTapeBackspace(state->tapeset,
						...
						ereport(ERROR,
                                (errcode(ERRCODE_TUPLE_BOGUS_TUPLE_LEN),
			...
			if (state->memtupcount > 0)
			{
				...
				*stup = state->memtuples[0];
				...
				if (stup->tuple)
				{
					...
					state->availMem += (long)tuplen;
					state->mergeavailmem[srcTape] += (long)tuplen;
				}
				tuplesort_heap_siftup(state, false);
...
#else
	HeapTuple tup = NULL;
	if(stup.tuple != NULL && state->copytup == copytup_heap)
	{
		tup = heap_tuple_from_minimal_tuple((MinimalTuple)stup.tuple);
		if (*should_free)
			pfree(stup.tuple);
		*should_free = true;
	} else if (state->copytup == copytup_cluster)
	{
...
#else
tuplesort_getdata(Tuplesortstate *state, bool forward, void **pData, int32 *len)
#endif //FOUNDER_XDB_SE
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	SortTuple	stup;
...
#else
	*pData = VARDATA(stup.datum1);
	*len = VARSIZE(stup.datum1);

	void *tData = palloc0(*len);
	memcpy(tData, *pData, *len);
	*pData = tData;
				   ...
				   Datum *val, ItemPointer tid)
{
	...
	*val = stup.datum1;
	memcpy(tid,stup.tuple,sizeof(ItemPointerData));
...
bool tuplesort_getdatum1(Tuplesortstate *state, bool forward,
				   Datum *val, void **privateData)
{
	MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
	SortTuple	stup;
	bool		should_free;
...
inittapes(Tuplesortstate *state)
{
	...
	if ((long)(tapeSpace + GetMemoryChunkSpace(state->memtuples)) < state->allowedMem)
	...
	switch (state->status)
	{
		...
		case TSS_SORTEDONTAPE:
			if (!LogicalTapeSeek(state->tapeset,
								 ...
								 state->markpos_offset))
				ereport(ERROR,
                        (errcode(ERRCODE_TUPLE_SEEK_FAILURE),
                        errmsg("tuplesort_restorepos failed")));
			state->eof_reached = state->markpos_eof;
			break;
		default:
			ereport(ERROR,
...
getlen(Tuplesortstate *state, int tapenum, bool eofOK)
{
	...
	if (LogicalTapeRead(state->tapeset, tapenum,
		...
		ereport(ERROR,
                (errcode(ERRCODE_TUPLE_UNEXPECT_TAPE_END),
                errmsg("unexpected end of tape")));
	if (len == 0 && !eofOK)
		ereport(ERROR,
                (errcode(ERRCODE_TUPLE_UNEXPECT_DATA_END),
                errmsg("unexpected end of data")));
...
#else
static inline int32
inlineApplySortFunction(CompareCallback compareFunction, int sk_flags, Oid collation,
						char* data1,int len1, bool isNull1,
						char* data2,int len2, bool isNull2)
{
	int32		compare;
	...
	{		
		compare = compareFunction(data1, len1, data2, len2);
...
#else
int32
ApplySortFunction(CompareCallback compareFunction, int sortFlags, Oid collation,
				  Datum datum1, bool isNull1,
				  Datum datum2, bool isNull2)
{
	return inlineApplySortFunction(compareFunction, sortFlags, collation,
								   VARDATA_ANY(DatumGetCString(datum1)),VARSIZE_ANY_EXHDR(DatumGetCString(datum1)), isNull1,
								   VARDATA_ANY(DatumGetCString(datum2)),VARSIZE_ANY_EXHDR(DatumGetCString(datum2)), isNull2);
}
...
comparetup_heap(const SortTuple *a, const SortTuple *b, Tuplesortstate *state)
{
	...
	compare = inlineApplySortFunction(&scanKey->sk_func, scanKey->sk_flags,
									  ...
									  b->datum1, b->isnull1);
#else
	compare = inlineApplySortFunction(scanKey->sk_compfun, scanKey->sk_flags,
									  scanKey->sk_collation,
									  VARDATA_ANY(DatumGetCString(a->datum1)),VARSIZE_ANY_EXHDR(DatumGetCString(a->datum1)), a->isnull1,
									  VARDATA_ANY(DatumGetCString(b->datum1)),VARSIZE_ANY_EXHDR(DatumGetCString(b->datum1)), b->isnull1);
#endif //FOUNDER_XDB_SE
	if (compare != 0)
		return compare;
...
{
	/*
	 * We expect the passed "tup" to be a HeapTuple, and form a
				   ...
				   Tuplesortstate *state)
{
	...
	{
		Assert(false);  //should not come here, because we havn't expression index.
#ifndef FOUNDER_XDB_SE
				...
				int tapenum, unsigned int tuplen)
{
	...
	HeapTuple	tuple = (HeapTuple) palloc0(t_len + HEAPTUPLESIZE);
...
//	CHECK_FOR_INTERRUPTS();
	...
	tuple1 = (IndexTuple) a->tuple;
	tuple2 = (IndexTuple) b->tuple;
	char *pstr1 = VARDATA_ANY(DatumGetCString(a->datum1));
   	char *pstr2 = VARDATA_ANY(DatumGetCString(b->datum1));
   
	keysz = state->nKeys;
	for (nkey = 1; nkey <= keysz; nkey++, scanKey++)
	{
		RangeData range1 = {0};
		range1.userData = state->indexRel->user_data;
		state->indexRel->rd_colinfo->split_function(range1, pstr1, scanKey->sk_attno, VARSIZE_ANY_EXHDR(DatumGetCString(a->datum1)));
		RangeData range2 = {0};
		range2.userData = state->indexRel->user_data;
		state->indexRel->rd_colinfo->split_function(range2, pstr2, scanKey->sk_attno, VARSIZE_ANY_EXHDR(DatumGetCString(b->datum1)));

		compare = inlineApplySortFunction(scanKey->sk_compfun, scanKey->sk_flags,
									  scanKey->sk_collation,
									  &pstr1[range1.start],(int)range1.len, false,
									  &pstr2[range2.start],(int)range2.len, false);
		if (compare != 0)
			return compare;		/* done when we find unequal attributes */
	...
	if (state->enforceUnique && !equal_hasnull && tuple1 != tuple2)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_UNIQUE_VIOLATION),
				 errmsg("could not create unique index \"%u\"",
						RelationGetRelid(state->indexRel))));
	}
...
reversedirection_index_hash(Tuplesortstate *state)
{
	...
	ereport(ERROR,
            ...
            errmsg("reversedirection_index_hash is not implemented")));
}
...
//	CHECK_FOR_INTERRUPTS();
	...
	return inlineApplySortFunction(&state->sortOpFn, state->sortFnFlags,
								   ...
								   b->datum1, b->isnull1);
#else
	return inlineApplySortFunction(state->indexScanKey->sk_compfun, state->sortFnFlags,
								   state->sortCollation,
								   VARDATA_ANY(DatumGetCString(a->datum1)),VARSIZE_ANY_EXHDR(DatumGetCString(a->datum1)), a->isnull1,
								   VARDATA_ANY(DatumGetCString(b->datum1)),VARSIZE_ANY_EXHDR(DatumGetCString(b->datum1)), b->isnull1);
#endif //FOUNDER_XDB_SE
...
static int
comparetup_index_datum(const SortTuple *a, const SortTuple *b, Tuplesortstate *state)
{
	/* Allow interrupting long sorts */
//	CHECK_FOR_INTERRUPTS();

	int nKeys = state->nKeys;
	ScanKey keys = state->scanKeys;
	for (int i = 0; i < nKeys; ++i)
	{
		char* pa = VARDATA_ANY(a->datum1);
		int lena = VARSIZE_ANY_EXHDR(a->datum1);
		RangeData rangea;
		keys->sk_split(rangea,pa,keys->sk_attno,lena);
		char* pCola = pa + rangea.start;

		char* pb = VARDATA_ANY(b->datum1);
		int lenb = VARSIZE_ANY_EXHDR(b->datum1);
		RangeData rangeb;
		keys->sk_split(rangeb,pb,keys->sk_attno,lenb);
		char* pColb = pb + rangeb.start;

		int32 compare = keys->sk_compfun(pCola,rangea.len,pColb,rangeb.len);
...
copytup_datum(Tuplesortstate *state, SortTuple *stup, void *tup)
{
	...
	ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_FUNCTION),
            errmsg("copytup_datum() should not be called")));
}
...
#ifdef FOUNDER_XDB_SE
bool tuplesort_support_random_access(Tuplesortstate *state)
{
	return state->randomAccess;
}

void tuplesort_set_sorted(Tuplesortstate *state)
{		
    switch (state->status)
	{
	case TSS_BUILDRUNS:
