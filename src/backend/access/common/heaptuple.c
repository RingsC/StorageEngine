				...
				uint16 *infomask, bits8 *bit)
{
	...
	*infomask &= ~(HEAP_HASNULL | HEAP_HASVARWIDTH | HEAP_HASEXTERNAL);
	...
	Assert((Size)(data - start) == data_size);
}
...
heap_attisnull(HeapTuple tup, int attnum)
{
	...
	switch (attnum)
	{
		...
		default:
			ereport(ERROR,
			(errcode(ERRCODE_HEAP_INVALID_TUPLE_ATTNUM),
			errmsg("invalid attnum: %d", attnum)));
	}
...
heap_copytuple(HeapTuple tuple)
{
	...
	newTuple->t_nIndexes = tuple->t_nIndexes;
	newTuple->t_indexOids = tuple->t_indexOids;
				...
				Oid *pEids)
{
	...
	tuple->t_nIndexes = nEids;
	tuple->t_indexOids = pEids;
				  ...
				  Datum *values, bool *isnull)
{
    ...
    if (natts == 1) {// for better performance. we have toast tables which may have >1 attrs. otherwise, it's always a single column byte string.
        values[0] = PointerGetDatum(tp);
        isnull[0] = false;
        return;
    }
#endif
	...
	for (attnum = 0; attnum < natts; attnum++)
	{
		...
		off = (long)att_addlength_pointer(off, thisatt->attlen, tp + off);
						...
						bool *isnull)
{
	...
	tuple->t_hoff = (uint8)(hoff + MINIMAL_TUPLE_OFFSET);
...
extern char *text_to_cstring_with_len(const text *t, int &len);

void *
minimal_tuple_getdata(MinimalTuple mtup,int &len)
{	
	void * data = 0;	
	char * p = (char*)mtup - MINIMAL_TUPLE_OFFSET + mtup->t_hoff;	
	data = text_to_cstring_with_len((text *)p, len);
	return data;
}
/*
