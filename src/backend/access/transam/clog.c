 ...
 */
static THREAD_LOCAL SlruCtlData ClogCtlData;
...
#else
		if ((int32)TransactionIdToPage(subxids[i]) != pageno)
...
#else
		while ((int32)TransactionIdToPage(subxids[i]) == pageno && i < nsubxids)
#endif //FOUNDER_XDB_SE
		{
			num_on_page++;
			i++;
						   ...
						   XLogRecPtr lsn, int pageno)
{
	...
	if (TransactionIdIsValid(xid))
	{
		...
		if (status == TRANSACTION_STATUS_COMMITTED)
		{
			for (i = 0; i < nsubxids; i++)
			{
				Assert(ClogCtl->shared->page_number[slotno] == (int)TransactionIdToPage(subxids[i]));
...
clog_redo(XLogRecPtr lsn, XLogRecord *record)
{
	...
	else if (info == CLOG_TRUNCATE)
	{
		...
		ereport(PANIC,
                ...
                errmsg("clog_redo: unknown op code %u", info)));
}
