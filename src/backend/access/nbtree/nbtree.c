...
btbuild(PG_FUNCTION_ARGS)
{
	...
	void* userData = (void *) PG_GETARG_POINTER(3);
	filter_func_t filterfunc = (filter_func_t) PG_GETARG_POINTER(4);
	...
	/* do the heap scan */
	if(index->mt_info.type == LSM_SUBTYPE){
		reltuples = 0;
	}else{
		reltuples = IndexBuildHeapScan(heap, index, indexInfo, true,
			btbuildCallback, (void *) &buildstate,userData,filterfunc);
	}
				...
				void *state)
{
	...
	if (values[0] == 0)// skip this tuple
	    return;
    if (values[0] == (Datum)-1) // is multikey
    {
        isMultiKey = true;
        pvalues = combineIndexKeys(buildstate->heapRel, index, htup, &nkeys);
		if(!pvalues||nkeys == 0)
			return;
    }
...
btinsert(PG_FUNCTION_ARGS)
{
	...
	itup = index_form_tuple(RelationGetDescr(rel), values, isnull, IS_CLUSTER_INDEX(rel->mt_info.type));
	if (!IS_CLUSTER_INDEX(rel->mt_info.type))
	    itup->t_tid = *ht_ctid;
	/*else  in this case this field is already assigned in index_form_tuple.
	    itup->t_tid.ip_blkid.bi_hi = IndexTupleSize(itup);*/
...
btgetbitmap(PG_FUNCTION_ARGS)
{
	...
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int64		ntids = 0;
	int64       *ptids = (int64 *)PG_GETARG_POINTER(2);
	ItemPointer heapTid;
...
btbeginscan(PG_FUNCTION_ARGS)
{
	...
	so = allocBTScanOpaqueStruct(CurrentTransactionState);// palloc0(sizeof(BTScanOpaqueData));
...
btendscan(PG_FUNCTION_ARGS)
{
    ...
    //if (so->currPos.datumBlock)
    //    pfree(so->currPos.datumBlock);
	...
	//if (so->markPos.datumBlock)
	//    pfree(so->markPos.datumBlock);
	so->markItemIndex = -1;
...
btrestrpos(PG_FUNCTION_ARGS)
{
	...
	else
	{
		...
		if (BTScanPosIsValid(so->markPos))
		{
		    //char *markdata = so->currPos.datumBlock;
		    //memcpy(markdata, so->markPos.datumBlock, BLCKSZ);
			/* bump pin on mark buffer for assignment to current buffer */
			IncrBufferRefCount(so->markPos.buf);
			memcpy(&so->currPos, &so->markPos,
				   offsetof(BTScanPosData, items[1]) +
				   so->markPos.lastItem * sizeof(BTScanPosItem));
	        //so->currPos.datumBlock = markdata;
		}
	}
...
restart:
	...
	if (delete_now)
	{
		...
		MemoryContextReset(vstate->pagedelcontext,true);
