...
_bt_initmetapage(Page page, BlockNumber rootbknum, uint32 level)
{
	 ...
	 */
	((PageHeader) page)->pd_lower =(LocationIndex)(
...
_bt_pagedel(Relation rel, Buffer buf, BTStack stack)
{
	...
	if (stack == NULL)
	{
		if (!InRecovery)
		{
			...
			itup_scankey = _bt_mkscankey(rel, targetkey, &keynum);
			...
			for (;;)
			{
				if (stack == NULL)
					ereport(ERROR,
                            (errcode(ERRCODE_NBTREE_NOT_ENOUGH_STACK_ITEM),
                            errmsg("not enough stack items")));
	...
	if (poffset >= maxoff)
	{
		if (poffset == P_FIRSTDATAKEY(opaque))
			...
			ereport(ERROR,
                    ...
                    errmsg( "failed to delete rightmost child %u of block %u in index \"%s\"",
