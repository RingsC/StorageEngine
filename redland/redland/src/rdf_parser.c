...
librdf_stream*
librdf_parser_parse_file_handle_as_stream(librdf_parser* parser, FILE *fh,
										  FILE *close_fh, librdf_uri* base_uri)
#endif
{
  LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(parser, librdf_parser, NULL);
...
/**
 * librdf_parser_set_multi_add_prepare:
 * @model: the model to use
 *
 * 准备批量插入
**/
void librdf_parser_set_multi_add_prepare(librdf_model* model)
{
	librdf_storage *storage = librdf_model_get_storage(model);

	if (storage->factory->set_multi_insert_prepare)
		storage->factory->set_multi_insert_prepare(storage);
}

/**
 * librdf_parser_set_multi_add_exec:
 * @model: the model to use
 *
 * 执行批量插入
**/
void librdf_parser_set_multi_add_exec(librdf_model* model)
{
	librdf_storage *storage = librdf_model_get_storage(model);

	if (storage->factory->set_multi_insert_execute)
		storage->factory->set_multi_insert_execute(storage);
}
