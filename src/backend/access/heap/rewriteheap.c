				   ...
				   TransactionId freeze_xid, bool use_wal)
{
	...
	state = (RewriteState)palloc0(sizeof(RewriteStateData));
...
end_heap_rewrite(RewriteState state)
{
	...
	while ((unresolved = (UnresolvedTup)hash_seq_search(&seq_status)) != NULL)
	{
	...
	if (state->rs_buffer_valid)
	{
		...
		smgrextend(state->rs_new_rel->rd_smgr, MAIN_FORKNUM, state->rs_blockno,
				   (char *) state->rs_buffer, true, BlockNumberThresholdByRel(state->rs_new_rel));
	}
				   ...
				   HeapTuple old_tuple, HeapTuple new_tuple)
{
							...
							&(old_tuple->t_data->t_ctid))))
	{
		...
		else
		{
			...
			unresolved = (UnresolvedTup)hash_search(state->rs_unresolved_tups, &hashkey,
									 HASH_ENTER, &found);
			Assert(!found);
	...
	for (;;)
	{
								   ...
								   state->rs_oldest_xmin))
		{
			...
			else
			{
				...
				mapping = (OldToNewMapping)hash_search(state->rs_old_new_tid_map, &hashkey,
									  HASH_ENTER, &found);
				Assert(!found);
...
raw_heap_insert(RewriteState state, HeapTuple tup)
{
	 ...
	 */
	if (state->rs_new_rel->rel_kind == RELKIND_TOASTVALUE)
	{
	...
	if (state->rs_buffer_valid)
	{
		...
		if (len + saveFreeSpace > pageFreeSpace)
		{
			...
			smgrextend(state->rs_new_rel->rd_smgr, MAIN_FORKNUM,
					   ...
					   BlockNumberThresholdByRel(state->rs_new_rel));
