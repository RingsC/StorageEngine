...
extern THREAD_LOCAL int	optreset;			/* might not be declared by system headers */
#endif
...
THREAD_LOCAL bool		Log_disconnections = false;

THREAD_LOCAL int			log_statement = LOGSTMT_NONE;

/* GUC variable for maximum stack depth (measured in kilobytes) */
THREAD_LOCAL int			max_stack_depth = 100;

/* wait N seconds to allow attach from a debugger */
THREAD_LOCAL int			PostAuthDelay = 0;
...
static THREAD_LOCAL long max_stack_depth_bytes = 100 * 1024L;
...
THREAD_LOCAL char	   *register_stack_base_ptr = NULL;
#endif
 ...
 */
static THREAD_LOCAL bool xact_started = false;
 ...
 */
static THREAD_LOCAL bool DoingCommandRead = false;

/*
 * Flags to implement skip-till-Sync-after-error behavior for messages of
 * the extended query protocol.
 */
static THREAD_LOCAL bool doing_extended_query_message = false;
static THREAD_LOCAL bool ignore_till_sync = false;
#ifndef FOUNDER_XDB_SE
/*
 ...
 */
static THREAD_LOCAL CachedPlanSource *unnamed_stmt_psrc = NULL;
#endif
/* workspace for building a new unnamed statement in */
static THREAD_LOCAL MemoryContext unnamed_stmt_context = NULL;


/* assorted command-line switches */
static THREAD_LOCAL const char *userDoption = NULL;	/* -D switch */

static THREAD_LOCAL bool EchoQuery = false;	/* -E switch */
...
static THREAD_LOCAL int	UseNewLine = 1;		/* Use newlines query delimiters (the default) */
#else
static THREAD_LOCAL int	UseNewLine = 0;		/* Use EOF as query delimiters */
#endif   /* TCOP_DONTUSENEWLINE */

/* whether or not, and why, we were canceled by conflict with recovery */
static THREAD_LOCAL bool RecoveryConflictPending = false;
static THREAD_LOCAL bool RecoveryConflictRetryable = true;
static THREAD_LOCAL ProcSignalReason RecoveryConflictReason;
			...
			default:
				ereport(FATAL,
                        (errcode(ERRCODE_CASE_NOT_FOUND),
                         errmsg("unrecognized conflict mode: %d",
                         (int) reason)));
		}
...
#ifndef FOUNDER_XDB_SE //FOUNDER_XDB_SE
		DisableNotifyInterrupt();
#endif
		DisableCatchupInterrupt();
#ifndef FOUNDER_XDB_SE //FOUNDER_XDB_SE
...
PostgresMain(int argc, char *argv[], const char *username)
{
	...
	{
		pg_signal(SIGHUP, SigHupHandler);		/* set flag to read config
												 * file */
		pg_signal(SIGINT, StatementCancelHandler);		/* cancel current query */
		pg_signal(SIGTERM, die); /* cancel current query and exit */
		 ...
		 */
		if (IsUnderPostmaster)
			pg_signal(SIGQUIT, quickdie);		/* hard crash time */
		else
			pg_signal(SIGQUIT, die);		/* cancel current query and exit */
		 ...
		 * backend
		 */
		pg_signal(SIGCHLD, SIG_DFL);		/* system() requires this on some
										 * platforms */
	}
...
get_stack_depth_rlimit(void)
{
	...
	static THREAD_LOCAL long val = 0;
...
static THREAD_LOCAL struct rusage Save_r;
static THREAD_LOCAL struct timeval Save_t;
