...
forkname_chars(const char *str, ForkNumber *fork)
{
	...
	{
		int			len = (int)strlen(forkNames[forkNum]);
	...
	else
	{
		...
		if (backend == InvalidBackendId)
		{
			pathlen = 9 + 1 + OIDCHARS + 1
				+ (int)strlen(TABLESPACE_VERSION_DIRECTORY) + 1 + OIDCHARS + 1
				+ OIDCHARS + 1 + FORKNAMECHARS + 1;
			path = (char *) palloc(pathlen);

			if (forknum != MAIN_FORKNUM)
				snprintf(path, pathlen, "fxdb_tblspc/%u/%s/%u/%u_%s",
						 rnode.spcNode, TABLESPACE_VERSION_DIRECTORY,
						 rnode.dbNode, rnode.relNode,
						 forkNames[forknum]);
			else
				snprintf(path, pathlen, "fxdb_tblspc/%u/%s/%u/%u",
						 rnode.spcNode, TABLESPACE_VERSION_DIRECTORY,
						 rnode.dbNode, rnode.relNode);
		}
		else
		{
			/* OIDCHARS will suffice for an integer, too */
			pathlen = 9 + 1 + OIDCHARS + 1
				+ (int)strlen(TABLESPACE_VERSION_DIRECTORY) + 1 + OIDCHARS + 2
				+ OIDCHARS + 1 + OIDCHARS + 1 + FORKNAMECHARS + 1;
			path = (char *) palloc(pathlen);

			if (forknum != MAIN_FORKNUM)
				snprintf(path, pathlen, "fxdb_tblspc/%u/%s/%u/t%d_%u_%s",
						 rnode.spcNode, TABLESPACE_VERSION_DIRECTORY,
						 rnode.dbNode, backend, rnode.relNode,
						 forkNames[forknum]);
			else
				snprintf(path, pathlen, "fxdb_tblspc/%u/%s/%u/t%d_%u",
						 rnode.spcNode, TABLESPACE_VERSION_DIRECTORY,
						 rnode.dbNode, backend, rnode.relNode);
		}
	}
...
GetDatabasePath(Oid dbNode, Oid spcNode)
{
	...
	{
		/* All other tablespaces are accessed via symlinks */
		pathlen = 9 + 1 + OIDCHARS + 1 + (int)strlen(TABLESPACE_VERSION_DIRECTORY) +
			1 + OIDCHARS + 1;
		path = (char *) palloc(pathlen);
		snprintf(path, pathlen, "fxdb_tblspc/%u/%s/%u",
...
IsToastRelation(Relation relation)
{
    ...
    extern Oid get_toast_table_id(Oid id);
	Oid toastId = get_toast_table_id(relation->rd_node.relNode);

	Relation toastrel = relation_open(toastId,NoLock); //try_relation_open(toastId, RowExclusiveLock);
	if (NULL == toastrel)
	{
		return false;
	}
...
#else
		Datum arg[1] ;
		arg[0] = fdxdb_string_formdatum((char*)(&newOid), 4);
...
#else
Oid
GetNewRelFileNode(Oid reltablespace, Oid dbid, char relpersistence)
#endif
{
	...
	switch (relpersistence)
	{
		case RELPERSISTENCE_TEMP:
			...
			backend = MyBackendId;
#else
			backend = CurrentTopLevelState->xid;
#endif //FOUNDER_XDB_SE
			break;
		...
		default:
			ereport(ERROR,
                    ...
                    errmsg("invalid relpersistence: %c", relpersistence)));
	...
	#else
	rnode.node.dbNode = (rnode.node.spcNode == GLOBALTABLESPACE_OID) ? InvalidOid : dbid;
	#endif
