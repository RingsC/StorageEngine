...
_hash_datum2hashkey(Relation rel, Datum key)
{
	...
	fcinfo.arg[0] = key;
	return DatumGetUInt32(hashtext(&fcinfo));
#endif
}
...
_hash_get_indextuple_hashkey(IndexTuple itup)
{
	...
	#ifdef FOUNDER_XDB_SE
	return *((uint32 *) VARDATA_ANY(attp));
	#else
	return *((uint32 *) attp);
	#endif
}
...
_hash_form_tuple(Relation index, Datum *values, bool *isnull)
{
	...
	else
	{
		...
		#else
		hashkeydatum = fdxdb_uint32_formdatum(hashkey);
		#endif
	}
	hashdesc = RelationGetDescr(index);
	Assert(hashdesc->natts == 1);
	itup = index_form_tuple(hashdesc, &hashkeydatum, isnull, false);
	#ifdef FOUNDER_XDB_SE
	if (NULL != DatumGetPointer(hashkeydatum))
		pfree(DatumGetPointer(hashkeydatum));
	#endif
	return itup;
}
