			...
			OffsetNumber offnum)
{
	...
	*/
	...
	for (i = 1; i <= keysz; i++)
	{
        ...
        rel->rd_colinfo->split_function(range, pstr0, scankey->sk_attno, len0);
...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
...
#else
			Fdxdb_ScanKeyInitWithCallbackInfo(scankeys + i, cur->sk_attno, InvalidStrategy, cur->sk_compfun, cur->sk_argument);
			}
#endif //FOUNDER_XDB_SE
	}
	...
	switch (strat_total)
	{
		...
		default:
			...
			ereport(ERROR,
                    ...
                    errmsg("unrecognized strat_total: %d", (int) strat_total)));
			return false;
	}
    ...
    if (IS_CLUSTER_INDEX(scan->indexRelation->mt_info.type)) {
	    Assert(so->currPos.itemIndex >= 0);
	    offset = so->currPos.itemIndex > 0 ? so->currPos.offsets[so->currPos.itemIndex - 1] : 0;
	    scan->current_dataitem = so->currPos.datumBlock + offset;
	    scan->curdi_len = so->currPos.offsets[so->currPos.itemIndex] - offset;
	} else
...
_bt_next(IndexScanDesc scan, ScanDirection dir)
{
	...
	{
		if ((IS_CLUSTER_INDEX(scan->indexRelation->mt_info.type) && ++so->currPos.itemIndex > so->currPos.lastItem) || 
		    (!IS_CLUSTER_INDEX(scan->indexRelation->mt_info.type) && --so->currPos.itemIndex < so->currPos.firstItem))
		{
			/* We must acquire lock before applying _bt_steppage */
			Assert(BufferIsValid(so->currPos.buf));
			LockBuffer(so->currPos.buf, BT_READ);
...
_bt_readpage(IndexScanDesc scan, ScanDirection dir, OffsetNumber offnum)
{
	...
	OffsetNumber maxoff;
	int			itemIndex;
	bool		continuescan, isClusterScan;

	/* we must have the buffer pinned and locked */
	Assert(BufferIsValid(so->currPos.buf));
    isClusterScan = IS_CLUSTER_INDEX(scan->indexRelation->mt_info.type);
    ...
    so->currPos.offsets[MaxIndexTuplesPerPage] = 0;// abandon previous page's items if any.
    scan->current_dataitem = 0;
	if (ScanDirectionIsForward(dir))
	{
	...
	else
	{
		...
		if (isClusterScan) {
            so->currPos.firstItem = 0;
		    so->currPos.lastItem = so->currPos.offsets[MaxIndexTuplesPerPage] - 1;
		    so->currPos.itemIndex = 0;// last entry is put at first and so on, go from 1st to last in dataBlock to scan backward.
		} else {
...
_bt_steppage(IndexScanDesc scan, ScanDirection dir)
{
	...
	if (so->markItemIndex >= 0)
	{
		...
		/*if (so->markPos.datumBlock == 0){
		    markdata = so->markPos.datumBlock = (char *)palloc(BLCKSZ);
		    so->markPos.offsets[MaxIndexTuplesPerPage] = 0;
		}*/
		...
		so->markItemIndex = -1;
		//so->markPos.datumBlock = markdata;
	}
...
_bt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	...
	else
	{
		ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("invalid scan direction: %d", (int) dir)));
