 ...
 * ----------
 */
static THREAD_LOCAL time_t last_pgarch_start_time;
static time_t last_sigterm_time = 0;
fxdb_thread_t *PgArchHandler = NULL;
/*
 * Flags set by interrupt handlers for later service in the main loop.
 */
static THREAD_LOCAL volatile sig_atomic_t got_SIGHUP = false;
static volatile sig_atomic_t got_SIGTERM = false;
static THREAD_LOCAL volatile sig_atomic_t wakened = false;
...
#else
static void* ArchiverFun( void * params)
{
	extern void* fxdb_SubPostmaster_Main(void *);
	extern void BackgroundWriterMain(void);
	extern void MemoryContextInit(bool);
...
pgarch_start(void)
{
	...
	*/
	...
	fxdb_save_thread_variables(param,4,archiver);
	PgArchHandler = fxdb_Thread_Create(ArchiverFun, param);
	if (NULL == PgArchHandler)
	{
		ereport(ERROR,
                (errcode(ERRCODE_THREAD_CREATE_FAILURE),
...
int pgarch_end()
{
    if (NULL != PgArchHandler)
    {
		return pgkill(PgArchHandler->thid,SIGQUIT);
    }
    return 0;
}
#endif
...
PgArchiverMain(int argc, char *argv[])
{
	 ...
	 */
	pg_signal(SIGHUP, ArchSigHupHandler);
	pg_signal(SIGINT, SIG_IGN);
	pg_signal(SIGTERM, ArchSigTermHandler);
	pg_signal(SIGQUIT, pgarch_exit);
	pg_signal(SIGALRM, SIG_IGN);
	pg_signal(SIGPIPE, SIG_IGN);
	pg_signal(SIGUSR1, pgarch_waken);
	pg_signal(SIGUSR2, pgarch_waken_stop);
...
//				ProcessConfigFile(PGC_SIGHUP);
			...
			else
			{
				...
				pg_sleep(1000000L);	/* wait a bit before retrying */
			}
		}
	}
}
...
#else
	rc = XLogArchiveOrRestoreFile(xlog, pathname, true);
#endif
...
/*
 * The codes is copied from pgarch_readyXlog.
 * filename: only file name ,not contain path.
 * 
 * Only the following file will return true:
 *   1 xlog file
 *   2 .backup file
 *   3 .history file
 */
bool
pgarch_fileCanArchive(const char *filename)
{
	int namelen = 0;
	...
	if (namelen >= MIN_XFN_CHARS &&
		namelen <= MAX_XFN_CHARS &&
		strspn(filename, VALID_XFN_CHARS) >= (size_t)namelen)
		return true;
