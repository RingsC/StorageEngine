...
#ifdef FOUNDER_XDB_SE
static void remove_from_mtinfo(MtInfo mt_info, Oid indid);
#endif //FOUNDER_XDB_SE
			 ...
			 bool concurrent)
{
	...
	indexRelation->rel_kind = RELKIND_INDEX;
...
static void 
remove_from_mtinfo(MtInfoData &mt_info, Oid indid)
{
	if(MtInfo_GetIndexCount(mt_info) <= 0)
	{
		if (NULL != mt_info.tableColInfo)
		{
			if (NULL != mt_info.tableColInfo->col_number)
			{
				pfree(mt_info.tableColInfo->col_number);
				mt_info.tableColInfo->col_number = NULL;
			}
			if (NULL != mt_info.tableColInfo->rd_comfunction)
			{
				pfree(mt_info.tableColInfo->rd_comfunction);
				mt_info.tableColInfo->rd_comfunction = NULL;
			}
			pfree(mt_info.tableColInfo);
			mt_info.tableColInfo = NULL;
		}
		return ;
	}
	...
	unsigned int *type = (unsigned int *)palloc0(sizeof(unsigned int));
	unsigned int *index_array = (unsigned int *)palloc0(sizeof(unsigned int));
	Colinfo *index_info = (Colinfo*)palloc(sizeof(Colinfo));

	uint32 count = 1;
	uint32 index_count = MtInfo_GetIndexCount(mt_info);
	bool has_index = false;

	for(unsigned int i = 0; i < index_count; ++i)
	{
		Oid tmpid = MtInfo_GetIndexOid(mt_info)[i];
		if(tmpid == indid)
		{
			--MtInfo_GetIndexCount(mt_info);
		} else {
			has_index = true;

			type = (unsigned int *)repalloc(type, sizeof(unsigned int) * count);
			index_array = (unsigned int *)repalloc(index_array, sizeof(unsigned int) * count);
			index_info = (Colinfo*)repalloc(index_info, sizeof(Colinfo) * count);

			type[count - 1] = MtInfo_GetIndexType(mt_info)[i];
			index_array[count - 1] = tmpid;
			index_info[count - 1] = MtInfo_GetIndexColinfo(mt_info)[i];

			++count;
		}
	}

	pfree(MtInfo_GetIndexType(mt_info));
	pfree(MtInfo_GetIndexOid(mt_info));
	pfree(MtInfo_GetIndexColinfo(mt_info));

	MtInfo_GetIndexType(mt_info) = NULL;
	MtInfo_GetIndexOid(mt_info) = NULL;
	MtInfo_GetIndexColinfo(mt_info) = NULL;

	if(has_index)
	{
		MtInfo_GetIndexType(mt_info) = type;
		MtInfo_GetIndexOid(mt_info) = index_array;
		MtInfo_GetIndexColinfo(mt_info) = index_info;
	} else {
		pfree(type);
		pfree(index_array);
		pfree(index_info);
 ...
 */
void
__index_drop(Oid indexId, Oid indrelId, Oid tableSpace)
{
#ifndef FOUNDER_XDB_SE
	...
	if(!IsTmpOid(indexId))
	{
		CheckStandbyUpdateError("Can not execute drop index during recovery.");
		GetCurrentTransactionId();
	} else 
	{
	...
	heapId = indrelId;
	userHeapRelation = heap_open(heapId, tableSpace, AccessExclusiveLock);

#endif //FOUNDER_XDB_SE

	userIndexRelation = index_open(indexId, AccessExclusiveLock);
    if (NULL == userIndexRelation)
    {
		return;
    }
...
#ifdef FOUNDER_XDB_SE
	CacheInvalidateRelcache(userIndexRelation);
#endif //FOUNDER_XDB_SE
	...
	* ensure other backends update their relcache lists of indexes.
	*/
	Relation userParentRelation = NULL;
	if(userIndexRelation->mt_info.type == LSM_SUBTYPE)
	{
		Oid parentRelId = userIndexRelation->mt_info.parentId;
		userParentRelation = index_open(parentRelId, AccessShareLock);
	}
	else
	{
		userParentRelation = heap_open(indrelId, AccessExclusiveLock);
	}

	CacheInvalidateRelcache(userParentRelation);

	remove_from_mtinfo(userParentRelation->mt_info, indexId);

	/*
	* Close owning rel, but keep lock
	*/
	heap_close(userParentRelation, NoLock);

#endif //FOUNDER_XDB_SE	
    fxdb_delete_meta(indexId);
	CommandCounterIncrement();
}

void
index_drop(Oid indexId, Oid indrelId, Oid tableSpace)
{
	...
	{
		for(uint32 i = 0; i < MtInfo_GetIndexCount(indexRel->mt_info); i++)
		{
			Oid subIdxId = MtInfo_GetIndexOid(indexRel->mt_info)[i];
			__index_drop(subIdxId, indrelId, tableSpace);
		}
	}
	index_close(indexRel, AccessExclusiveLock);

	return __index_drop(indexId, indrelId, tableSpace);
}
...
BuildIndexInfo(Relation index)
{
	...
	ii->ii_KeyAttrNumbers[0]=1;
	ii->ii_NumIndexAttrs = 1;
	ii->ii_Expressions = NIL;	/* for now */
	ii->ii_ExpressionsState = NIL;
	ii->ii_Predicate = NIL;
	ii->ii_PredicateState = NIL;
	ii->ii_ExclusionOps = 0;
	ii->ii_ExclusionProcs = 0;
	ii->ii_ExclusionStrats = 0;
	ii->ii_Unique = index->mt_info.type >= UINQUE_TYPE_BASE;
	/* In a concurrent build, mark it not-ready-for-inserts */
	ii->ii_ReadyForInserts = true;
	...
	if (numKeys < 1 || numKeys > INDEX_MAX_KEYS)
		ereport(ERROR,
                ...
                errmsg( "invalid indnatts %d for index %u",
...
#else
void
FormIndexDatum(Relation heapRelation,
			   Relation idexRelation,
			   IndexInfo *indexInfo,
			   HeapTuple tuple,
			   //EState *estate,
			   Datum *values,
			   bool *isnull)
{
	extern bool is_toast_table_id(Oid id);
	extern bool is_toast_index_id(Oid id);
	bool istoastrel = is_toast_table_id(heapRelation->rd_id);
	bool istoastindex = is_toast_index_id(idexRelation->rd_id);
	...
	Datum		iDatum = (Datum) 0;
	bool		isNull = false, isMultiKey = false;
	heap_deform_tuple(tuple, heapRelation->rd_att, &iDatum, isnull);
	
	if (istoastrel && istoastindex)
	{
		Chunk_data* pChunk = (Chunk_data*)iDatum;
		values[0] = fdxdb_string_formdatum(VARDATA(pChunk), 2*sizeof(Oid));
		return;
	}
	
	Datum iDatum2 = (Datum)pg_detoast_datum_packed((varlena *)iDatum);
	char *pstr0 = VARDATA_ANY(DatumGetCString(iDatum2));
	size_t len0 = VARSIZE_ANY_EXHDR(DatumGetCString(iDatum2));
	char *pstr1 = (char*)palloc0(len0 + VARHDRSZ);
	char *pstr = VARDATA(pstr1);
	size_t len1 = 0;

	for (i = 0; i < idexRelation->rd_colinfo->keys; i++)
	{
		size_t			keycol = idexRelation->rd_colinfo->col_number[i];
		...
		if (keycol != 0)
		{
			 ...
			 */
            RangeData range = {0};
			range.userData = idexRelation->user_data;
			...
			if (len2 == (size_t)-1) {
				pfree(pstr1);
				values[0] = 0;
				if (DatumGetPointer(iDatum2) != NULL && 
					 DatumGetPointer(iDatum2) != DatumGetPointer(iDatum))
					 pfree(DatumGetPointer(iDatum2));
				return;// don't insert index key for this heap tuple.
			}
	...
	if (isMultiKey) {
	    ...
	    values[0] = PointerGetDatum(pstr1);
	    SET_VARSIZE(pstr1, len1 + VARHDRSZ);
    }
	else  {
	    values[0] = 0;// skip building index key for this heap tuple.
		pfree(pstr1);
	}
	if (indexpr_item != NULL)
		ereport(ERROR,
				   ...
				   Oid reltoastidxid, double reltuples)
{
	...
	{
		Assert(rel_kind == RELKIND_TOASTVALUE);
...
index_build(Relation heapRelation,
			...
			IndexInfo *indexInfo,
			bool isprimary,
			bool isreindex,void* userData, filter_func_t filterfunc)
{
...
//	int			save_nestlevel;
	...
	if (heapRelation == 0)
	    procedure = indexRelation->rd_am->ambuildcluster;
	Assert(RegProcedureIsValid(procedure));

	ereport(LOG,
			(errmsg("start building index %d on heap %d",
					RelationGetRelid(indexRelation),
					heapRelation!=0?RelationGetRelid(heapRelation):0)));
	...
	stats = (IndexBuildResult *)
		DatumGetPointer(OidFunctionCall5(procedure,
										 ...
										 PointerGetDatum(indexInfo),PointerGetDatum(userData),PointerGetDatum(filterfunc)));
	 ...
	 */
	if (heapRelation && heapRelation->rd_rel->relpersistence == RELPERSISTENCE_UNLOGGED &&
	...
	index_update_stats(heapRelation,
					   true,
					   isprimary,
					   (heapRelation->rel_kind == RELKIND_TOASTVALUE) ?
...
//	SetUserIdAndSecContext(save_userid, save_sec_context);
	...
	pfree(stats);
	ereport(LOG,(errmsg(
			"finish building index %d on heap %d",
					RelationGetRelid(indexRelation),
					heapRelation!=0?RelationGetRelid(heapRelation):0)));
}
...
IndexBuildHeapScan(Relation heapRelation,
				   ...
				   void *callback_state,void* userData,
				   filter_func_t filterfunc)
{
	ereport(LOG,
			(errmsg("   index build heap scan begin for index %d on heap %d",
					RelationGetRelid(indexRelation),
					RelationGetRelid(heapRelation))));
	...
	scan = heap_beginscan_strat(heapRelation,	/* relation */
								...
								allow_sync, filterfunc, userData);	/* syncscan OK? */
	 ...
	 */
	while (indexInfo->ii_ReadyForInserts && (heapTuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		bool		tupleIsAlive;
...
//					   estate,
	...
	//ereport(LOG,
	//		(errmsg("   finish index build heap scan for index %d on heap %d",
	//				RelationGetRelid(indexRelation),
	//				RelationGetRelid(heapRelation))));
	return reltuples;
}
...
void
validate_index(Oid heapId, Oid indexId, Oid tableSpace, Snapshot snapshot)
{
	...
	heapRelation = heap_open(heapId, ShareUpdateExclusiveLock, tableSpace);
	/* And the target index relation */
	indexRelation = index_open(indexId, RowExclusiveLock, tableSpace, UNKNOWN_TYPE);
 ...
 */
void
reindex_index(Oid indexId, Oid tableSpace, bool skip_constraint_checks)
{
	Relation	iRel,
#ifndef FOUNDER_XDB_SE
				heapRelation,
	 ...
	 */
	heapId = IndexGetRelation(indexId);
	heapRelation = heap_open(heapId, tableSpace, ShareLock);

	/*
	 * Open the target index relation and get an exclusive lock on it, to
	 * ensure that no one else is touching this particular index.
	 */
	iRel = index_open(indexId, AccessExclusiveLock, tableSpace, UNKNOWN_TYPE);
 ...
 */
bool
reindex_relation(Oid relid, Oid tablespc, int flags)
{
	Relation	rel;
	Oid			toast_relid;
	List	   *indexIds;
	 ...
	 */
	rel = heap_open(relid, tablespc, ShareLock);
	...
	PG_TRY();
	{
		...
		foreach(indexId, indexIds)
		{
			...
			reindex_index(indexOid, rel->rd_node.spcNode, !(flags & REINDEX_REL_CHECK_CONSTRAINTS));
	...
	if ((flags & REINDEX_REL_PROCESS_TOAST) && OidIsValid(toast_relid))
		result |= reindex_relation(toast_relid, tablespc, flags);

	return result;
}
#else
void
reindex_index(Oid heapId, Oid indexId, Oid dbid)
{
	 ...
	 * we only need to be sure no schema or data changes are going on.
	 */
	heapRelation = heap_open(heapId, AccessExclusiveLock, dbid);

	/*
	 * Open the target index relation and get an exclusive lock on it, to
	 * ensure that no one else is touching this particular index.
	 */
	iRel = index_open(indexId, AccessExclusiveLock, dbid);
	...
	PG_TRY();
	{
		...
		/* Fetch info needed for index_build */
		indexInfo = BuildIndexInfo(iRel);
		for (unsigned int i = 0; i < MtInfo_GetIndexCount(heapRelation->mt_info); ++i) 
		{
			Oid index_id = MtInfo_GetIndexOid(heapRelation->mt_info)[i];
			if (indexId == index_id)
			{
				IndexType indexType = (IndexType)MtInfo_GetIndexType(heapRelation->mt_info)[i];
				indexInfo->ii_Unique = (indexType >= UINQUE_TYPE_BASE) ? UNIQUE_CHECK_YES : UNIQUE_CHECK_NO;
			}
		}
...
bool
reindex_relation(Oid relid, Oid dbid, int flags)
{
	Relation	rel;
	Oid			toast_relid;
	List	   *indexIds;
	bool		is_toastid = false;
	bool		result;
	 ...
	 * to prevent schema and data changes in it.
	 */
	if (relid < MIN_USER_OID)
		return false;
	
	rel = heap_open(relid, ShareLock, dbid);

	extern Oid get_toast_table_id(Oid relId);
	toast_relid = get_toast_table_id(relid);
	...
	PG_TRY();
	{
		...
		foreach(indexId, indexIds)
		{
			...
			reindex_index(relid, indexOid, rel->rd_node.dbNode);
	 ...
	 */
	extern bool is_toast_table_id(Oid id);
	is_toastid = is_toast_table_id(relid);
	if ((!is_toastid) && (flags & REINDEX_REL_PROCESS_TOAST) && OidIsValid(toast_relid))
		result |= reindex_relation(toast_relid, dbid,flags);
...
#define InsertRelDataBase(RelBase,Relid)	\
do{	\
	RelBase.RelDatabase[RelBase.nlen] = Relid;\
	++(RelBase.nlen);\
	if (RelBase.nSize <= RelBase.nlen)\
	{\
		RelBase.nSize = RelBase.nSize*2;\
		RelBase.RelDatabase = (Oid*)repalloc(RelBase.RelDatabase,RelBase.nSize);\
	}\
}while(0)

void ScanLORelTable(ReIndexBase& ScanBase,const Oid& LargeLOId)
{
	Relation relLO= heap_open(LargeLOId, AccessShareLock);
	HeapScanDesc ScanLO= heap_beginscan(relLO, SnapshotNow, 0, NULL, NULL, NULL);
	HeapTuple tupleLO = NULL;
	bool	is_toastid = true;
	while((tupleLO = heap_getnext(ScanLO, ForwardScanDirection)) != NULL)
	{
		Form_class_large_heap* metaData =(Form_class_large_heap*)(VARDATA_ANY((char*)tupleLO->t_data + tupleLO->t_data->t_hoff));
		if ((0 != metaData->lo_index_id) && (metaData->lo_heap_id >= MIN_USER_OID))
		{
			extern bool is_toast_table_id(Oid id);
			is_toastid = is_toast_table_id(metaData->lo_heap_id);
			if (!is_toastid)
			{
				InsertRelDataBase(ScanBase,metaData->lo_heap_id);
			}
		}
	}
	heap_endscan(ScanLO);
	heap_close(relLO, AccessShareLock);
}
void ScanSystemTable(ReIndexBase& ScanBase,Oid dbid)
{
	if (dbid != DEFAULTDATABASE_OID)
	{
		Relation relLargeMeta= heap_open(LargeObjMetaTableRelId, AccessShareLock);
		HeapScanDesc ScanLargeMeta = heap_beginscan(relLargeMeta, SnapshotNow, 0, NULL, NULL, NULL);
		HeapTuple tupleLargeMeta = NULL;
		bool	is_toastid = true;
		bool	is_toastid2 =true;
		while((tupleLargeMeta = heap_getnext(ScanLargeMeta, ForwardScanDirection)) != NULL)
		{
			Form_meta_large_table_rel* metaData =(Form_meta_large_table_rel*)(VARDATA_ANY((char*)tupleLargeMeta->t_data + tupleLargeMeta->t_data->t_hoff));
			if (metaData->dbid == dbid)
			{
				extern bool is_toast_table_id(Oid id);
				is_toastid = is_toast_table_id(metaData->relid);
				is_toastid2 = is_toast_table_id(metaData->relid2);
				if (!is_toastid)
				{
					InsertRelDataBase(ScanBase,metaData->relid);
				}
				if (!is_toastid2)
				{
					InsertRelDataBase(ScanBase,metaData->relid2);
					ScanLORelTable(ScanBase,metaData->relid2);
				}
			}
		}
		heap_endscan(ScanLargeMeta);
		heap_close(relLargeMeta, AccessShareLock);
	}
	else if (dbid == DEFAULTDATABASE_OID)
	{
		ScanLORelTable(ScanBase,LargeObjHeapId);
	}
}

void
reindex_database(Oid dbid)
{
	ReIndexBase ScanDatabase;
	ScanDatabase.nlen = 0;
	ScanDatabase.nSize = 512;
	ScanDatabase.RelDatabase = (Oid*)palloc0(ScanDatabase.nSize*sizeof(Oid));
	ScanSystemTable(ScanDatabase,dbid);
	
	ReIndexBase reDatabase;	
	reDatabase.nlen = 0;
	reDatabase.nSize = 512;
	reDatabase.RelDatabase = (Oid*)palloc0(reDatabase.nSize*sizeof(Oid));
	...
	{
		Form_meta_table metaData =(Form_meta_table)(VARDATA_ANY((char*)tuple->t_data + tuple->t_data->t_hoff));
		if ((metaData->database_id == dbid) && (0 == metaData->rel_pid) && (metaData->rel_id >= MIN_USER_OID))
		{
			extern bool is_toast_table_id(Oid id);
			is_toastid = is_toast_table_id(metaData->rel_id);
			if (!is_toastid)
			{
				for (unsigned int i=0; i<ScanDatabase.nlen; ++i)
				{
					if (metaData->rel_id == ScanDatabase.RelDatabase[i])
						goto End;
				}
				InsertRelDataBase(reDatabase,metaData->rel_id);
				End:
				;
			}
		}
	...
	PG_TRY();
	{
		for(unsigned int i=0; i<reDatabase.nlen; ++i)
			reindex_relation(reDatabase.RelDatabase[i],dbid,REINDEX_REL_PROCESS_TOAST);
	}
	PG_CATCH();
	{
		pfree(ScanDatabase.RelDatabase);
		pfree(reDatabase.RelDatabase);
		PG_RE_THROW();
	}
	PG_END_TRY();
...
extern Datum get_index_key(Relation heapRel, Relation indexRel, HeapTuple tup, bool min);
Datum index_get_max(Relation heapRelation, Relation indexRelation, Snapshot snapshot)
{
	if(indexRelation->idx_max == (Datum)0)
	{
		IndexScanDesc indscan = index_beginscan(heapRelation, indexRelation, snapshot, 0, 0, 0, 0);
		index_rescan(indscan, NULL, 0, NULL, 0);
		HeapTuple tuple = NULL;
		if((tuple = index_getnext(indscan, BackwardScanDirection)) != NULL)
		{
			indexRelation->idx_max = get_index_key(heapRelation, indexRelation, tuple, false);
		}
		index_endscan(indscan);
	}
	return indexRelation->idx_max;
}

Datum index_get_min(Relation heapRelation, Relation indexRelation, Snapshot snapshot)
{
	if(indexRelation->idx_min == (Datum)0)
	{
		IndexScanDesc indscan = index_beginscan(heapRelation, indexRelation, snapshot, 0, 0, 0, 0);
		index_rescan(indscan, NULL, 0, NULL, 0);
		HeapTuple tuple = NULL;
		if((tuple = index_getnext(indscan, ForwardScanDirection)) != NULL)
		{
			indexRelation->idx_min = get_index_key(heapRelation, indexRelation, tuple, true);
		}
		index_endscan(indscan);
	}
	return indexRelation->idx_min;
}
#endif
...
#else
#define currentlyReindexedHeap (CurrentTransactionState->mTopLevelState->ncurrentlyReindexedHeap)
#define currentlyReindexedIndex (CurrentTransactionState->mTopLevelState->ncurrentlyReindexedIndex)
#define pendingReindexedIndexes	(CurrentTransactionState->mTopLevelState->mpendingReindexedIndexes)
#endif
...
SetReindexProcessing(Oid heapOid, Oid indexOid)
{
	...
	if (OidIsValid(currentlyReindexedHeap))
		ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                errmsg("cannot reindex while reindexing")));
