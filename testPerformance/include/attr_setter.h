...
inline AttrInfo GetAttrInfo(std::string& strAttr)
{
	...
	{
	    attrInfo.first = PARAM_TYPE(1);
        attrInfo.second = offsetof(storage_params,workMem);	
	}
	else	 if(strAttr == std::string("mainTenanceWorkMem"))
	{
	    attrInfo.first = PARAM_TYPE(1);
        attrInfo.second = offsetof(storage_params,mainTenanceWorkMem);	
	}
	...
	else     if(strAttr == std::string("startvac"))
	{
		attrInfo.first = PARAM_TYPE(0);
		attrInfo.second = offsetof(storage_params,startVac);
	}
	else     if(strAttr == std::string("elevel"))
	{
		attrInfo.first = PARAM_TYPE(1);
		attrInfo.second = offsetof(storage_params,elevel);
	}
	else 
	{
