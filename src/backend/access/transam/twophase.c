 ...
 */
#define TWOPHASE_DIR "fxdb_twophase"

/* GUC variable, can't be changed after startup */
THREAD_LOCAL int			max_prepared_xacts = 64;
...
typedef struct GlobalTransactionData
{
	...
	TransactionStateData  transStateData;
	struct TopLevelState topLevelState;
...
#else
	TwoPhaseState = (TwoPhaseStateData *)ShmemInitStruct("Prepared Transaction Table",
#endif //FOUNDER_XDB_SE
									TwoPhaseShmemSize(),
									&found);
...
#else
	gxact->proc.mCurrentTransState = &gxact->transStateData;
	gxact->proc.mCurrentTransState->mTopLevelState = &gxact->topLevelState;
    gxact->xid = xid;
    gxact->topLevelState.xid = xid;
#endif
	gxact->proc.pid = 0;
	gxact->proc.mCurrentTransState->pid = 0;
...
#else
	gxact->proc.mCurrentTransState->mTopLevelState->inCommit = false;
//	gxact->proc.mCurrentTransState->mTopLevelState->lockProc->vacuumFlags = 0;
#endif
...
#else
	gxact->proc.mCurrentTransState->mTopLevelState->subxids.overflowed = false;
	gxact->proc.mCurrentTransState->mTopLevelState->subxids.nxids = 0;
#endif
...
void AddPrepareXact2Array(TransactionId xid)
{
	GlobalTransaction gxact;
	int i = 0;
	...
	for (i = 0; i < TwoPhaseState->numPrepXacts; i++)
	{
		...
		if(TransactionIdEquals(gxact->xid, xid)){
			AddTransation2ArrayInLock(&gxact->transStateData);
			break;
		}
	}
...
#else
		gxact->proc.mCurrentTransState->mTopLevelState->subxids.overflowed = true;
#endif
...
#else
		memcpy(gxact->proc.mCurrentTransState->mTopLevelState->subxids.xids, children,
			   nsubxacts * sizeof(TransactionId));
		gxact->proc.mCurrentTransState->mTopLevelState->subxids.nxids = nsubxacts;
#endif
	}
}
...
RemoveGXact(GlobalTransaction gxact)
{
	...
	ereport(ERROR,
            ...
            errmsg("failed to find %p in GlobalTransaction array", gxact)));
}
...
TwoPhaseGetDummyProc(TransactionId xid)
{
	...
	static THREAD_LOCAL TransactionId cached_xid = InvalidTransactionId;
	static  THREAD_LOCAL PGPROC *cached_proc = NULL;
	...
	if (result == NULL)			/* should not happen */
		ereport(ERROR,
                (errcode(ERRCODE_NO_ACTIVE_SQL_TRANSACTION),
                errmsg("failed to find dummy PGPROC for xid %u", xid)));
...
static THREAD_LOCAL struct xllist records
#else
#define records (CurrentTransactionState->mTopLevelState->mRecords)
#endif
...
#else
		records.tail->next = (XLogRecData *)palloc0(sizeof(XLogRecData));
#endif //FOUNDER_XDB_SE
...
#else
		records.tail->data = (char *)palloc(records.bytes_free);
#endif //FOUNDER_XDB_SE
	}
...
#else
	records.head = (XLogRecData *)palloc0(sizeof(XLogRecData));
#endif //FOUNDER_XDB_SE
...
#else
	records.head->data = (char *)palloc(records.bytes_free);
#endif //FOUNDER_XDB_SE
...
#else
		if ((write(fd, record->data, record->len)) != (int32)record->len)
#endif //FOUNDER_XDB_SE
		{
			close(fd);
			ereport(ERROR,
...
ReadTwoPhaseFile(TransactionId xid, bool give_warnings)
{
	...
	if (stat.st_size < (int)((MAXALIGN(sizeof(TwoPhaseFileHeader)) +
						MAXALIGN(sizeof(TwoPhaseRecordOnDisk)) +
						sizeof(pg_crc32))) ||
		stat.st_size > (int)MaxAllocSize)
	{
		close(fd);
		return NULL;
	}
...
#else
	if ((int32)crc_offset != MAXALIGN(crc_offset))
#endif //FOUNDER_XDB_SE
	{
		close(fd);
		return NULL;
...
#else
    if (hdr->magic != TWOPHASE_MAGIC || (int32)hdr->total_len != stat.st_size)
#endif //FOUNDER_XDB_SE	
    {
		pfree(buf);
		return NULL;
...
FinishPreparedTransaction(const char *gid, bool isCommit)
{
	 ...
	 */
	gxact = LockGXact(gid, 0/*GetUserId()*/);
#ifndef FOUNDER_XDB_SE
	xid = gxact->proc.xid;
#else
	xid = gxact->xid;
	...
	RemoveTransactonFromArray(&gxact->transStateData, InvalidTransactionId);
	//ProcArrayRemove(NULL, latestXid);
	...
	for (i = 0; i < ndelrels; i++)
	{
		...
		for (fork = 0; fork <= MAX_FORKNUM; fork++)
		{
			...
			if (smgrexists(srel, (ForkNumber)fork))
				smgrdounlink(srel, (ForkNumber)fork, false);
#endif //FOUNDER_XDB_SE
		}
		smgrclose(srel);
	}
...
RecoverPreparedTransactions(void)
{
	...
	while ((clde = ReadDir(cldir, dir)) != NULL)
	{
			...
			strspn(clde->d_name, "0123456789ABCDEF") == 8)
		{
			...
			AddTransation2Array(&gxact->transStateData);
							   ...
							   RelFileNode *rels)
{
	...
	if (nrels > 0){
		for(int i = 0; i < nrels; i++){
			RelationCacheDeleteIfAny(rels[i].relNode);
		}
	}
...
#ifdef FOUNDER_XDB_SE
bool GlobalTransactionIdIsPrepared(char *gid)
{
	bool found = false;
	int	i;
...
uint32 GetNextMaxPreparedGlobalTxnId()
{
	LWLockAcquire(TwoPhaseStateLock, LW_EXCLUSIVE);
...
int GetPreparedGlobalTxnId(GlobalPrepareTxnInfo** prepareList)
{
	LWLockAcquire(TwoPhaseStateLock, LW_SHARED);
	...
	GlobalPrepareTxnInfo* list_txn = (GlobalPrepareTxnInfo*)palloc(sizeof(GlobalPrepareTxnInfo) * num);

	for (int i = 0; i < num; i++)
	{
		uint32 gxid = atoi(TwoPhaseState->prepXacts[i]->gid);
		Oid dbid = TwoPhaseState->prepXacts[i]->proc.databaseId;
		list_txn[i].gxid = gxid;
		list_txn[i].dbid = dbid;
	}

	LWLockRelease(TwoPhaseStateLock);

	*prepareList = list_txn;
	return num;
}
