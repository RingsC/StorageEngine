...
typedef struct
{
	...
	pthread_t	pid;			/* PID of blocked backend */
} DEADLOCK_INFO;
...
static THREAD_LOCAL PGPROC **visitedProcs;	/* Array of visited procs */
static THREAD_LOCAL int	nVisitedProcs;

/* Workspace for TopoSort */
static THREAD_LOCAL PGPROC **topoProcs;		/* Array of not-yet-output procs */
static THREAD_LOCAL int *beforeConstraints;	/* Counts of remaining before-constraints */
static THREAD_LOCAL int *afterConstraints;	/* List head for after-constraints */

/* Output area for ExpandConstraints */
static THREAD_LOCAL WAIT_ORDER *waitOrders;	/* Array of proposed queue rearrangements */
static THREAD_LOCAL int	nWaitOrders;
static THREAD_LOCAL PGPROC **waitOrderProcs; /* Space for waitOrders queue contents */

/* Current list of constraints being considered */
static THREAD_LOCAL EDGE *curConstraints;
static THREAD_LOCAL int	nCurConstraints;
static THREAD_LOCAL int	maxCurConstraints;

/* Storage space for results from FindLockCycle */
static THREAD_LOCAL EDGE *possibleConstraints;
static THREAD_LOCAL int	nPossibleConstraints;
static THREAD_LOCAL int	maxPossibleConstraints;
static THREAD_LOCAL DEADLOCK_INFO *deadlockDetails;
static THREAD_LOCAL int	nDeadlockDetails;

/* PGPROC pointer of any blocking autovacuum worker found */
static THREAD_LOCAL PGPROC *blocking_autovacuum_proc = NULL;
	...
	if (DeadLockCheckRecurse(proc))
	{
		...
		if (!FindLockCycle(proc, possibleConstraints, &nSoftEdges))
			ereport(FATAL,
                    ...
                    errmsg("deadlock seems to have disappeared")));
	...
	for (i = 0; i < nEdges; i++)
	{
		if (!savedList && i > 0)
		{
			...
			if (nEdges != TestConfiguration(proc))
				ereport(FATAL,
                        (errcode(ERRCODE_INTERNAL_ERROR),
                        errmsg("inconsistent results during deadlock check")));
		}
...
PrintLockQueue(LOCK *lock, const char *info)
{
	...
	ereport(LOG, (errmsg("%s lock %p queue ", info, lock)));
	proc = (PGPROC *) waitQueue->links.next;
	for (i = 0; i < queue_size; i++)
	{
		ereport(LOG, (errmsg(" %d", proc->pid)));
		proc = (PGPROC *) proc->links.next;
	}
	fflush(stdout);
}
