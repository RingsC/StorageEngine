...
/**
 * librdf_model_get_all_graph:
 * @model: #librdf_model object
 *
 * 获取所有图信息
 **/
librdf_list*
librdf_model_get_all_graph(librdf_world *world) 
{
	librdf_list *list = librdf_new_list(world);
  unsigned int count = 0;
	RDFMeta_Models models = (RDFMeta_Models)librdf_storage_fxdb_get_all_graph(&count);

	if (models)
	{
		for (unsigned int i = 0; i < count; ++i)
		{
			librdf_graph_info *ginfo = LIBRDF_MALLOC(librdf_graph_info *, sizeof(librdf_graph_info));
			size_t strlens = strlen(models[i].name.data) + 1;
			ginfo->graph_name = LIBRDF_MALLOC(char *, strlens);
			strcpy(ginfo->graph_name, models[i].name.data);
			ginfo->db_id = models[i].db_id;
			ginfo->stid = models[i].stid;
			ginfo->index_spo_id = models[i].index_spo_id;
			ginfo->index_os_id = models[i].index_os_id;
			ginfo->index_po_id = models[i].index_po_id;
			ginfo->index_inf_id = models[i].index_inf_id;
			if (librdf_list_add(list, ginfo))
			{
				if (models)
					pfree(models);
				return list;
 ...
 **/
int
librdf_drop_model (librdf_model* model) 
{
	librdf_storage *storage = librdf_model_get_storage(model);
	return storage->factory->drop_storage(storage);
}

/**
 * librdf_model_remove_statement_any:
 * 
 * 与librdf_model_remove_statement函数一样，
 * 不过该函数允许接收非完整的statement
 **/
int
librdf_model_remove_statement_any(librdf_model* model, librdf_statement* statement)
{
  LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(model, librdf_model, 1);
  LIBRDF_ASSERT_OBJECT_POINTER_RETURN_VALUE(statement, librdf_statement, 1);
...
/**
 * librdf_model_counter_remove_rows:
 * 
 * 计算删除的statement条数
 **/
unsigned int
librdf_model_counter_remove_rows(librdf_model* model)
{
  librdf_storage *storage = librdf_model_get_storage(model);
	return storage->factory->counter_remove_rows(storage);
}
