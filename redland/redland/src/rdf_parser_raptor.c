...
#define RAPTOR_HANDLE_FREE(ptr)	mcxfree((void*)ptr)
#endif
...
#ifdef FOUNDER_XDB_SE
void raptor_free_handle(void* ptr)
{
	mcxfree((void*)ptr);
}
#endif
...
#endif
{
  ...
  if(pcontext->nspace_prefixes)
    raptor_free_sequence(pcontext->nspace_prefixes);
  #ifdef FOUNDER_XDB_SE
  pcontext->nspace_prefixes=raptor_new_sequence(raptor_free_handle, NULL);
  #else
                                            ...
                                            librdf_uri *base_uri)
{
  ...
  if(uri && librdf_uri_is_file_uri(uri)) {
    ...
    stream=librdf_parser_raptor_parse_file_handle_as_stream(context, fh, 1,
                                                            base_uri);
	#else
    stream=librdf_parser_raptor_parse_file_handle_as_stream(context, fh, (FILE *)1,
                                                            base_uri);	
	#endif
                                             ...
                                             librdf_model* model)
{
  ...
  librdf_parser_set_multi_add_prepare(model);
...
librdf_parser_raptor_serialise_finished(void* context)
{
  ...
  if(scontext) {
    ...
    if(scontext->model)
    {
    	librdf_parser_set_multi_add_exec(scontext->model);
    }
