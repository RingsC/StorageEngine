...
static THREAD_LOCAL int	spins_per_delay = DEFAULT_SPINS_PER_DELAY;
...
s_lock_stuck(volatile slock_t *lock, const char *file, int line)
{
	...
	exit(1);
#else
	elog(WARNING,"stuck spinlock (%p) detected at %s:%d",lock, file, line);
#endif
}
...
{
	/*
	 * We loop tightly for awhile, then delay using pg_sleep() and try again.
	 ...
	 * on that hardware.
	 *
	 * Once we do decide to block, we use randomly increasing pg_sleep()
	 ...
	 * spent.
	 *
	 * The pg_sleep() delays are measured in milliseconds because 1 msec is a
...
THREAD_LOCAL volatile struct test_lock_struct test_lock;
...
main()
{
	...
	{
		ereport(LOG, (errmsg("S_LOCK_TEST: failed, declared datatype is wrong size.")));
		return 1;
	}

	if (!S_LOCK_FREE(&test_lock.lock))
	{
		ereport(LOG, (errmsg("S_LOCK_TEST: failed, lock not initialized."));
		return 1;
	}
	...
	if (S_LOCK_FREE(&test_lock.lock))
	{
		ereport(LOG, (errmsg("S_LOCK_TEST: failed, lock not locked.")));
		return 1;
	}
	...
	if (!S_LOCK_FREE(&test_lock.lock))
	{
		ereport(LOG, (errmsg("S_LOCK_TEST: failed, lock not unlocked")));
		return 1;
	}
	...
	if (S_LOCK_FREE(&test_lock.lock))
	{
		ereport(LOG, (errmsg("S_LOCK_TEST: failed, lock not re-locked.")));
		return 1;
	}

	ereport(LOG, (errmsg("S_LOCK_TEST: this will print %d stars and then.", NUM_DELAYS)));
	ereport(LOG, (errmsg("             exit with a 'stuck spinlock' message.")));
	ereport(LOG, (errmsg("             if S_LOCK() and TAS() are working.")));
	fflush(stdout);
