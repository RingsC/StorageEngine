 ...
 * Return value: non 0 on failure
 **/
size_t __raptor_get_file_size(FILE *stream)
{
	size_t file_size = 0;

	int cur_pos = ftell(stream);
...
static int
__raptor_parser_parse_turtle_file_stream(raptor_parser* rdf_parser, FILE *stream)
{
	extern int raptor_init_parser_buffer(raptor_parser* rdf_parser, char *buffer, size_t len);
	size_t file_size = __raptor_get_file_size(stream);
	size_t buffer_len = file_size + 1;
	...
	while(!feof(stream))
	{
		...
		read_len = fread(buffer + read_len, 1, buffer_len - read_len, stream);
	}
  	buffer[read_len] = '\0';
  	if(raptor_init_parser_buffer(rdf_parser, buffer, read_len) == 0)
  	{
  		raptor_parser_parse_chunk(rdf_parser, NULL, 0, true);
		return 0;
  	}
                                ...
                                raptor_uri *base_uri)
{
  ...
  if(strstr(rdf_parser->factory->desc.label, "Turtle") != NULL) /* turtle */
  {
  	rc =__raptor_parser_parse_turtle_file_stream(rdf_parser, stream);
  }
  else
  {
	  while(!feof(stream)) {
