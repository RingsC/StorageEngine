...
static THREAD_LOCAL SlruErrorCause slru_errcause;
static THREAD_LOCAL int	slru_errno;
 ...
 */
int
SimpleLruReadPage(SlruCtl ctl, int pageno_, bool write_ok,
				  TransactionId xid)
{
	SlruShared	shared = ctl->shared;
	int pageno = pageno_;
	...
	switch (slru_errcause)
	{
		...
		default:
			...
			ereport(ERROR,
                    ...
                    errmsg("unrecognized SimpleLru error cause: %d",
				                     (int) slru_errcause)));
			break;
	}
}
 ...
 */
static int
SlruSelectLRUPage(SlruCtl ctl, int pageno_)
{
	SlruShared	shared = ctl->shared;
	int pageno = pageno_;
