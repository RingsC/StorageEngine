		...
		if (lt->pos >= BLCKSZ)
		{
			...
			else
			{
				...
				ereport(ERROR,
                        ...
                        errmsg("invalid logtape state: should be dirty")));
			}
			lt->numFullBlocks++;
			lt->curBlockNumber++;
			lt->pos = 0;
...
bool LogicalTapeSeekToEOF(LogicalTapeSet *lts, int tapenum)
{
	LogicalTape *lt;
	...
	return LogicalTapeSeek(lts, tapenum, lt->numFullBlocks, lt->lastBlockBytes);
}
