...
static BOOL pgwin32_get_dynamic_tokeninfo(HANDLE token,
							TOKEN_INFORMATION_CLASS class1, char **InfoBuffer,
...
{
	static THREAD_LOCAL int	_is_service = -1;
	HANDLE		AccessToken;
	char	   *InfoBuffer = NULL;
	char		errbuf[256];
	PTOKEN_GROUPS Groups;
 ...
 */
static BOOL
pgwin32_get_dynamic_tokeninfo(HANDLE token, TOKEN_INFORMATION_CLASS class1,
							  char **InfoBuffer, char *errbuf, int errsize)
{
	DWORD		InfoBufferSize;

	if (GetTokenInformation(token, class1, NULL, 0, &InfoBufferSize))
	{
...
#else
    *InfoBuffer = (char *)malloc(InfoBufferSize);
	...
	if (!GetTokenInformation(token, class1, *InfoBuffer,
