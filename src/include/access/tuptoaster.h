...
#define MIN_TOAST_ID 0x40000000
...
#ifdef FOUNDER_XDB_SE
/* The var byte array from heap tuple always has its header's size added to its var header size, so we should substract it. */
#define GetVarDataSize(htupdata, dataptr, datasize, mem2free) do {        \
    char *__dataptr__ = (htupdata);                                    \
    if (VARATT_IS_EXTERNAL(__dataptr__) ||                                  \
            VARATT_IS_COMPRESSED(__dataptr__)) {                            \
        __dataptr__ = (char *)heap_tuple_untoast_attr((varlena*)__dataptr__);\
        (mem2free) = __dataptr__;                                           \
    } else                                                                  \
        (mem2free) = NULL;                                                  \
    (datasize) = (VARATT_IS_SHORT(__dataptr__) ?                            \
        (VARSIZE_SHORT(__dataptr__) - VARHDRSZ_SHORT) :                     \
        (VARSIZE(__dataptr__) - VARHDRSZ));                                 \
    (dataptr) = (VARATT_IS_SHORT(__dataptr__) ?                             \
        VARDATA_SHORT(__dataptr__) : VARDATA(__dataptr__));                 \
} while (0)

#define GetHeapTupleDataSize(htup, dataptr, datasize, mem2free)   do {      \
    char *__htupdata__;                                                     \
    __htupdata__ = ((char *) ((htup)->t_data)) + (htup)->t_data->t_hoff;    \
    GetVarDataSize(__htupdata__, (dataptr), (datasize), (mem2free));      \
} while (0)

#define GetHeapTupleRawDatum(htup)                                 \
    PointerGetDatum(((char *) ((htup)->t_data)) + (htup)->t_data->t_hoff)
